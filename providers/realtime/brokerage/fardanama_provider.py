...
from providers.realtime.brokerage.fardanama_client import FardanamaClient

logger = get_logger(__name__)


class FardanamaProvider(BaseBrokerageProvider):
    def __init__(self, api_key: str = "") -> None:
        super().__init__(name="fardanama", base_url="")
        self.client = FardanamaClient(api_key=api_key)

    async def fetch(self, symbol: str | None = None, **kwargs: Any) -> Result[Any]:
        if symbol:
            return await self.client.get_quotes([symbol])
    ...
    async def health(self) -> dict[str, Any]:
        result = await self.client.get_quotes(["test"])
