...
class JSONArchiveProvider(BaseProvider):
    def __init__(self, archive_dir: str | Path | None = None) -> None:
        super().__init__(name="json_archive")
    ...
    def _symbol_path(self, symbol: str) -> Path:
        return self.archive_dir / f"{symbol}.json"
    ...
    async def fetch(self, symbol: str | None = None, **kwargs: Any) -> Result[Any]:
        ...
        try:
            with open(path, encoding="utf-8") as f:
                raw = json.load(f)
            parsed = self.parser.parse({"raw": raw, "format": "json"})
            return Result.ok(self.mapping.map(parsed))
        except Exception as e:
            logger.error("JSON read failed for %s: %s", symbol, e)
    ...
    async def save(self, symbol: str, data: list[dict[str, Any]]) -> Result[Path]:
        ...
        try:
            ...
            logger.error("JSON save failed for %s: %s", symbol, e)
            return Result.fail(str(e))
