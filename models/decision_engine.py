...
class DecisionArchitecture(TimestampMixin, Base):
    ...
    version: Mapped[str] = mapped_column(String(50), nullable=False, unique=True, index=True, comment="نسخه معماری (مثلاً Enterprise-Final-1.0)")
...
class DecisionResult(TimestampMixin, Base):
    ...
    decision: Mapped[str] = mapped_column(String(20), comment="تصمیم نهایی: BUY / WATCHLIST / HOLD / REDUCE / REJECT / NEUTRAL")
    ...
    # Metadata
    details: Mapped[dict | None] = mapped_column(JSONB, nullable=True, comment="جزئیات کامل (دلایل، reason_codes, report)")
