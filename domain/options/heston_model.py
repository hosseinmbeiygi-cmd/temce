...
def _characteristic_function_heston(
    u: complex, tau: float, params: HestonParams
) -> complex:
...
def heston_monte_carlo(params: HestonParams, seed: int = 42) -> OptionPrice:
    ...
    for i in range(n):
        ...
        # Stock price process
        S_t = S_t * np.exp(
            (r - q - 0.5 * v_t) * dt + sqrt_v * np.sqrt(dt) * Z1[i]
        )
    ...
    return OptionPrice(
        ...
        time_value=price - intrinsic,
        parameters={
            "v0": params.v0, "theta": params.theta_v if hasattr(params, 'theta_v') else params.theta,
            "kappa": params.kappa, "sigma_v": params.sigma_v, "rho": params.rho,
            "n_paths": m, "n_steps": n, "std_error": std_error,
...
def calibrate_heston(
    ...
    def objective(x):
        ...
        for K, T, mkt_price in market_prices:
            try:
                params = HestonParams(
                    S=S, K=K, T=T, r=r, q=q,
                    v0=v0, theta=theta_v, kappa=kappa, sigma_v=sigma_v, rho=rho,
                )
    ...
    return HestonParams(
        S=S, K=market_prices[0][0], T=market_prices[0][1], r=r, q=q,
        v0=v0, theta=theta_v, kappa=kappa, sigma_v=sigma_v, rho=rho,
    )
