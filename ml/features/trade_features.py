...
class TradeFeatures(BaseFeatureBuilder):
    ...
    def compute(self, data: pd.DataFrame) -> FeatureMatrix:
        ...
        total_value = (df["real_buy_value"] + df["real_sell_value"] + df["legal_buy_value"] + df["legal_sell_value"]).replace(0, np.nan)
    ...
    def get_feature_names(self) -> list[str]:
        return [
            "real_buy_volume_ratio", "legal_buy_volume_ratio",
            "real_net_flow", "legal_net_flow",
            "real_net_flow_pct", "legal_net_flow_pct",
            "buy_sell_pressure", "smart_money_signal",
            "real_net_flow_ma3", "legal_net_flow_ma3",
            "real_net_flow_delta3", "legal_net_flow_delta3",
            "real_net_flow_ma5", "legal_net_flow_ma5",
            "real_net_flow_delta5", "legal_net_flow_delta5",
            "real_buy_vol_concentration", "legal_buy_vol_concentration",
        ]
