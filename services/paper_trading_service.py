...
class PaperTradingService:
    ...
    def _signal_to_snapshot(self, sig: dict[str, Any], batch_id: str) -> PaperSignalSnapshotModel:
        ...
        return PaperSignalSnapshotModel(
            ...
            confidence=float(sig.get("confidence") or sig.get("confidence_score") or 0) if (sig.get("confidence") or sig.get("confidence_score")) else None,
            full_signal=sig,
        )
    ...
    async def close_trade(
        ...
        logger.info(
            ...
            trade.symbol, trade.entry_price, price, trade.pnl or 0, trade.pnl_pct or 0,
        )
    ...
    async def get_dashboard(self) -> dict[str, Any]:
        ...
        # Equity is always recomputed from realized + open P&L — never read back
        # from the equity table (that would freeze the curve at yesterday's value).
    ...
    async def list_snapshots(
        ...
        stmt = stmt.order_by(desc(PaperSignalSnapshotModel.generated_at)).offset((page - 1) * page_size).limit(page_size)
        rows = (await self.session.execute(stmt)).scalars().all()
