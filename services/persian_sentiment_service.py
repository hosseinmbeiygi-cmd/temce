...
POSITIVE_KEYWORDS = [
    "رشد", "افزایش", "صعود", "جهش", "مثبت", "سود", "بهره", "منفعت",
    "خرید", "تقاضا", "شکست مقاومت", "حمایت", "تجمع", "انباشت",
    "بهبود", "توسعه", "گسترش", "پیشرفت", "موفقیت", "رکوردشکنی",
    "بازدهی", "درآمد", "سودآوری", "افزایش سرمایه", "تقسیم سود",
    "پیشنهاد خرید", "توصیه خرید", "همراه با رشد", "روند صعودی",
    "کف‌سازی", "شکست سقف", "عبور از مقاومت", "سیگنال خرید",
    "پتانسیل رشد", "چشم‌انداز مثبت", "عملکرد قوی", "افزایش حجم",
    "ورود پول هوشمند", "جمع‌آوری", "تثبیت قیمت", "بازگشت صعودی",
]

NEGATIVE_KEYWORDS = [
    "کاهش", "افت", "سقوط", "ریزش", "منفی", "زیان", "ضرر", "خسارت",
    "فروش", "عرضه", "شکست حمایت", "مقاومت", "توزیع", "تخلیه",
    "تخریب", "فروپاشی", "بحران", "رکود", "تورم", "کمبود",
    "کاهش درآمد", "زیان‌دهی", "انحلال", "ورشکستگی", "تحریم",
    "پیشنهاد فروش", "توصیه فروش", "همراه با افت", "روند نزولی",
    "سقف‌سازی", "شکست کف", "سقوط از حمایت", "سیگنال فروش",
    "پتانسیل افت", "چشم‌انداز منفی", "عملکرد ضعیف", "کاهش حجم",
    "خروج پول هوشمند", "توزیع سنگین", "شکست قیمت", "بازگشت نزولی",
    "تهدید", "ریسک بالا", "هشدار", "فشار فروش", "عدم اطمینان",
]
...
class PersianSentimentService:
    ...
    def _ml_score(self, text: str) -> dict[str, Any] | None:
        ...
        score = sum(
            proba[i] * label_scores.get(classes[i], 0.0)
            for i in range(len(classes))
        )

        return {"score": round(score, 4), "probabilities": {classes[i]: round(float(proba[i]), 4) for i in range(len(classes))}}
