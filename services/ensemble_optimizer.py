...
class EnsembleOptimizer:
    ...
    def _accuracy_weighted(
        self, models: list[ModelPerformance], market: str,
        min_models: int, max_models: int,
    ) -> Result[OptimizedEnsemble]:
        ...
        return Result.ok(OptimizedEnsemble(
            market=market,
            models=selected,
    ...
    def _top_n(
        self, models: list[ModelPerformance], market: str, n: int,
    ) -> Result[OptimizedEnsemble]:
    ...
    def _default_ensemble(self, market: str, strategy: str) -> Result[OptimizedEnsemble]:
        ...
        performances = [
            ModelPerformance(
                model_name=name, market=market,
                accuracy_pct=55.0, total_signals=0,
                avg_return_pct=0.0, sharpe=0.0,
                weight=1.0 / max(len(model_names), 1), is_selected=True,
            )
    ...
    async def optimize_all_markets(
        self, strategy: str = "accuracy_weighted"
