...
class DiagnosticRunResult:
    ...
    __slots__ = ("run_id", "started_at", "finished_at", "total_symbols", "total_findings",
...
class DiagnosticsRunner:
    ...
    async def run(
        ...
        logger.info("Diagnostic run started: %s (limit=%d, skip_backtests=%s)",
                     self.result.run_id, limit, skip_backtests)
        ...
        logger.info("Diagnostic run finished: %s — %d findings (%d critical, %d errors)",
                     self.result.run_id, self.result.total_findings,
                     self.result.critical_count, self.result.error_count)
    ...
    async def _check_system(self) -> None:
        ...
        try:
            ...
            if async_session_factory is None:
                self.result.add_finding(DiagnosticFinding(
                    level="CRITICAL", category="system", symbol="__ALL__",
    ...
    async def _fetch_symbols(
        self, symbol_filter: list[str] | None, limit: int
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                try:
                    ...
                    if rows:
                        return [
                            {"symbol": r[0], "name": r[1] or "", "market_type": r[2] or ""}
                            for r in rows
                        ]
                except Exception as exc:
    ...
    def _validate_data(self, symbol: str, bars: list[dict[str, Any]], intraday_bars: list[dict[str, Any]] | None = None) -> bool:
        """Validate historical data quality. Returns True if data is usable for backtesting."""
        # Intraday check
        if intraday_bars is not None:
            if not intraday_bars:
                self.result.add_finding(DiagnosticFinding(
                    level="WARNING", category="data", symbol=symbol,
                    message="No intraday trade data found",
                ))
            else:
                self.result.add_finding(DiagnosticFinding(
                    level="INFO", category="data", symbol=symbol,
        ...
        nan_close = sum(1 for b in bars if b.get("close", 0) == 0 or (isinstance(b.get("close"), float) and b["close"] != b["close"]))
            ...
            for strat_name in strat_names:
                ...
                try:
                    ...
                    if result.total_trades == 0:
                        self.result.add_finding(DiagnosticFinding(
                            level="INFO", category="backtest", symbol=symbol,
                            strategy=strat_name,
                    ...
                    elapsed_ms = (time.perf_counter() - t0) * 1000
                    self.result.add_finding(DiagnosticFinding(
                        level="ERROR", category="backtest", symbol=symbol,
                        strategy=strat_name,
            ...
            async with async_session_factory() as session:
                ...
                with contextlib.suppress(Exception):
                    ...
                    result = await session.execute(
                        ...
                        .where(MlPredictionModel.symbol == symbol)
                        .limit(1)
                    )
                ...
                with contextlib.suppress(Exception):
                    ...
                    result = await session.execute(
                        select(func.count()).select_from(MlTrainingRunModel)
                    )
    ...
    async def _cross_checks(self) -> None:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                try:
                    result = await session.execute(
                        text("SELECT COUNT(*) FROM signal_accuracy")
                    )
                    count = result.scalar() or 0
                    if count > 0:
                        self.result.add_finding(DiagnosticFinding(
                            level="INFO", category="data", symbol="__ALL__",
                            message=f"Signal accuracy records: {count}",
                        ))
                except Exception:
                    self.result.add_finding(DiagnosticFinding(
                        level="WARNING", category="data", symbol="__ALL__",
    ...
    async def _persist_results(self) -> None:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                await session.commit()
                logger.info("Persisted %d findings for run %s",
                            self.result.total_findings, self.result.run_id)
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive system diagnostics for the trading platform"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limit", type=int, default=DEFAULT_SYMBOL_LIMIT,
        help=f"Max symbols to check (default: {DEFAULT_SYMBOL_LIMIT})",
    )
    parser.add_argument(
        "--skip-backtests", action="store_true",
