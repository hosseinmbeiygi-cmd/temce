...
def _compute_enhanced_metrics(result: Any, initial_capital: float) -> dict[str, Any]:
    ...
    if len(equity) < 2:
        return {"total_return_pct": 0, "sharpe_ratio": 0, "max_drawdown_pct": 100, "win_rate": 0, "total_trades": 0, "profit_factor": 0, "sortino_ratio": 0, "calmar_ratio": 0, "annualized_return_pct": 0, "winning_trades": 0, "losing_trades": 0, "avg_win": 0, "avg_loss": 0}
...
class StrategyGenerator:
    ...
    async def generate(
        ...
        try:
            ...
            logger.info("Phase 1: Grid search %d combinations × %d symbols = %d total runs",
                        len(all_combos), len(symbols), self._total)
            ...
            for batch_start in range(0, len(work_items), batch_size):
                ...
                futures = [
                    loop.run_in_executor(executor, self._run_single_sync, *item)
                    for item in batch
                ]
            ...
            self._history.append({
                ...
                "stats": self._stats,
                "top_strategies": [{"strategy": r["strategy"], "params": r["params"], "metrics": r["metrics"], "symbol": r.get("symbol", "")} for r in self._results[:10]],
            })
    ...
    def _run_single_sync(
        self, strategy_name: str, params: dict[str, Any], sym: str, data: list[dict], capital: float,
        timeout: float = 15.0,
    ...
    def export_csv(self) -> str:
        ...
        if not self._results:
            return ""
        headers = ["rank", "strategy", "symbol", "method", "return_pct", "sharpe", "sortino", "calmar", "max_dd", "win_rate", "profit_factor", "trades", "params"]
        rows = [",".join(headers)]
