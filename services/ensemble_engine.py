...
class StrategyEnsemble:
    ...
    REGIME_WEIGHTS = {
        0: {  # Bearish
            "trend": 0.1, "momentum": 0.1, "reversion": 0.6, "technical": 0.2,
        },
        1: {  # Neutral/Range
            "trend": 0.2, "momentum": 0.2, "reversion": 0.4, "technical": 0.2,
        },
        2: {  # Bullish
            "trend": 0.4, "momentum": 0.3, "reversion": 0.15, "technical": 0.15,
        },
    }
    ...
    def combine_signals(self, signals: list[dict[str, Any]], regime: int = 1,
...
class TSESimulator:
    ...
    def simulate_order(self, bar: dict, order_side: str, order_size: float,
                       limit_price: float, queue_position: float | None = None) -> dict:
        ...
        if remaining == 0:
            ...
            commission = order_size * price * (self.comm_buy if order_side == "BUY" else self.comm_sell)
            return {"status": "FILLED", "filled_qty": order_size, "price": price, "commission": commission, "reason": "Queue cleared"}
    ...
    def simulate(self, data: list[dict], signals: list[int], capital: float = 1_000_000_000) -> dict[str, Any]:
        ...
        for i, bar in enumerate(data):
            ...
            if signal == 1 and position == 0 and cash > 1000:
                ...
                if result["status"] == "FILLED":
                    ...
                    trades.append({"type": "BUY", "price": result["price"], "shares": result["filled_qty"], "fee": result["commission"]})
                ...
                if result["status"] == "FILLED":
                    ...
                    cash += result["filled_qty"] * result["price"] - result["commission"]
                    trades.append({"type": "SELL", "price": result["price"], "shares": position, "fee": result["commission"], "pnl": pnl})
                    position = 0
        ...
        returns = [(equity_curve[j] / equity_curve[j - 1]) - 1 for j in range(1, len(equity_curve)) if equity_curve[j - 1] > 0]
