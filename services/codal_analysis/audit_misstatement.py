...
class MisstatementEvaluator:
    ...
    def evaluate(
        ...
        total_judgmental = sum(ms.amount for ms in self.misstatements if ms.category == "judgmental" and not ms.is_corrected)
        total_projected = sum(ms.amount for ms in self.misstatements if ms.category == "projected" and not ms.is_corrected)
        ...
        if is_pervasive:
            rationale_parts.append(
                "CONCLUSION: Misstatements are pervasive - material and widespread"
            )
        elif is_material:
            rationale_parts.append(
                "CONCLUSION: Misstatements are material but not pervasive"
            )
        else:
            rationale_parts.append(
                "CONCLUSION: Misstatements are below materiality threshold"
            )
