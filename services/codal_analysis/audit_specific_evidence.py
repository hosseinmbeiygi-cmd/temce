...
class SpecificEvidenceAssessor:
    ...
    def inventory_observation(
        cls, location: str, date: str,
        test_counts: int = 100, test_differences: int = 0,
        count_method: str = "full",
    ) -> InventoryObservation:
        diff_rate = test_differences / test_counts if test_counts > 0 else 0
        accuracy = "satisfactory" if diff_rate <= 0.02 else "needs_improvement" if diff_rate <= 0.05 else "unsatisfactory"
        notes = []
        if diff_rate <= 0.02:
        ...
        return InventoryObservation(
            ...
            test_counts=test_counts, test_differences=test_differences,
    ...
    def litigation_assessment(
        cls, case_name: str, nature: str, claim_amount: float,
        likelihood: str = "possible",
    ) -> LitigationAssessment:
        la = LitigationAssessment(
            case_name=case_name, nature=nature,
            claim_amount=claim_amount, likelihood=likelihood,
        )
