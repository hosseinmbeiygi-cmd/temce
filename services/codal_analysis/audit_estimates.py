...
class EstimatesAuditor:
    ...
    def assess_warranty_provision(
        self, revenue: float, historical_claim_rate: float,
        avg_cost_per_claim: float, recorded_provision: float,
    ) -> ProvisionTestResult:
    ...
    def assess_doubtful_debts(
        self, receivables: float, historical_default_rate: float,
        aging_factors: dict[str, float] | None = None,
        recorded_allowance: float = 0,
    ) -> ProvisionTestResult:
        if aging_factors:
            expected = sum(
                amount * rate for amount, rate in aging_factors.items()
            )
        else:
