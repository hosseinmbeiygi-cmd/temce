...
def vertical_analysis(snap: FinancialSnapshot) -> VerticalAnalysis:
    va = VerticalAnalysis()
    if snap.revenue:
        va.pl_items["cost_of_goods_sold_pct"] = (snap.cost_of_goods_sold / snap.revenue * 100) if snap.cost_of_goods_sold else 0
        va.pl_items["gross_profit_pct"] = (snap.gross_profit / snap.revenue * 100) if snap.gross_profit else 0
        va.pl_items["operating_expenses_pct"] = (snap.operating_expenses / snap.revenue * 100) if snap.operating_expenses else 0
        va.pl_items["operating_profit_pct"] = (snap.operating_profit / snap.revenue * 100) if snap.operating_profit else 0
    ...
    if snap.total_assets:
        va.bs_items["current_assets_pct"] = (snap.current_assets / snap.total_assets * 100) if snap.current_assets else 0
        va.bs_items["non_current_assets_pct"] = (snap.non_current_assets / snap.total_assets * 100) if snap.non_current_assets else 0
        va.bs_items["current_liabilities_pct"] = (snap.current_liabilities / snap.total_assets * 100) if snap.current_liabilities else 0
        va.bs_items["non_current_liabilities_pct"] = (snap.non_current_liabilities / snap.total_assets * 100) if snap.non_current_liabilities else 0
        ...
        va.bs_items["accounts_receivable_pct"] = (snap.accounts_receivable / snap.total_assets * 100) if snap.accounts_receivable else 0
    return va
...
def compute_ratios(snap: FinancialSnapshot) -> RatioAnalysis:
    ...
    ratios.profitability["return_on_capital"] = _safe_div(snap.operating_profit, snap.total_equity + snap.non_current_liabilities) if (snap.total_equity + snap.non_current_liabilities) else None
    ...
    ratios.leverage["interest_coverage"] = _safe_div(snap.operating_profit, snap.financial_cost) if snap.financial_cost else None

    ratios.activity["inventory_turnover"] = _safe_div(snap.cost_of_goods_sold, snap.inventory or 1) if snap.inventory else None
    ratios.activity["receivables_turnover"] = _safe_div(snap.revenue, snap.accounts_receivable or 1) if snap.accounts_receivable else None
    ratios.activity["asset_turnover"] = _safe_div(snap.revenue, snap.total_assets) if snap.total_assets else None
    ratios.activity["fixed_asset_turnover"] = _safe_div(snap.revenue, snap.net_fixed_assets or 1) if snap.net_fixed_assets else None

    ratios.cash_flow["cfo_to_net_profit"] = _safe_div(snap.operating_cash_flow, snap.net_profit) if snap.net_profit else None
    ...
    if snap.prev:
        ...
        if rev_growth != 0:
            ...
            if eq.receivables_growth_vs_revenue > 1.2:
                eq.warnings.append(f"رشد مطالبات ({ar_growth:.1%}) بسیار بیشتر از رشد فروش ({rev_growth:.1%}) - احتمال Channel Stuffing")
