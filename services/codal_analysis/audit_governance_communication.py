...
class GovernanceCommunicator:
    ...
    def create_plan(cls, entity: str, period: str) -> GovernanceCommunicationPlan:
        ...
        plan.committee_meetings = [
            {"cycle": c, "status": "scheduled", "date": ""}
            for c in COMMITTEE_CYCLES
        ]
    ...
    @classmethod
    def add_record(
        cls, plan: GovernanceCommunicationPlan,
        topic: str, date: str, communicated_to: str,
        method: str = "meeting", key_points: str = "",
    ) -> None:
        plan.records.append(CommunicationRecord(
            topic=topic, date=date,
            communicated_to=communicated_to,
            method=method, key_points=key_points,
        ))
