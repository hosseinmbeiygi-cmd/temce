...
CANONICAL_ACCOUNTS: dict[str, dict[str, Any]] = {
    ...
    "COST_OF_GOODS_SOLD": {"code": "COGS", "name": "بهای تمام شده کالای فروش رفته", "category": "expense", "statement": "pl"},
    ...
    "NON_CURRENT_LIABILITIES": {"code": "NCUR_LIAB", "name": "بدهی‌های غیرجاری", "category": "liability", "statement": "bs"},
    ...
    "TOTAL_CURRENT_ASSETS": {"code": "TOT_CUR_ASSETS", "name": "جمع دارایی‌های جاری", "category": "asset", "statement": "bs"},
    "TOTAL_CURRENT_LIABILITIES": {"code": "TOT_CUR_LIAB", "name": "جمع بدهی‌های جاری", "category": "liability", "statement": "bs"},
}
...
class AccountMapper:
    def __init__(self, confidence_threshold: float = 0.85):
        ...
        self._compiled_rules: list[tuple[re.Pattern, str]] = [
            (re.compile(re.escape(kw), re.IGNORECASE), code)
            for kw, code in RULE_BASED_MAPPINGS.items()
        ]
    ...
    def get_unmapped(self, labels: list[str], industry: str | None = None) -> list[str]:
        return [
            lbl for lbl, res in zip(labels, self.batch_map(labels, industry), strict=False)
