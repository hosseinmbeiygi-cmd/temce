...
class IFRSComplianceEngine:
    ...
    def lease_classification(
        ...
        if term_percentage >= 0.75:
            lease_criteria += 1
            reasoning.append(f"Lease term ({lease_term_years}yrs) is major part of economic life ({economic_life_years}yrs) at {term_percentage:.0%} (IFRS 16.B3)")
        if pv_percentage >= 0.90:
            lease_criteria += 1
            reasoning.append(f"PV of payments ({pv_percentage:.1%} of fair value) substantially all of asset value (IFRS 16.B4)")
    ...
    def revenue_recognition(
        ...
        allocated = standalone_prices or [contract_price / max(performance_obligations_count, 1)] * max(performance_obligations_count, 1)
        if sum(allocated) > 0:
    ...
    def fair_value_measurement(
        ...
        if discounted_cashflows:
            ...
            pv = sum(
                ...
                for i, cf in enumerate(discounted_cashflows)
            )
            terminal_value = discounted_cashflows[-1] * (1 + 0.02) / (discount_rate - 0.02) if len(discounted_cashflows) > 0 else 0
