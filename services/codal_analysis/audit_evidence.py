...
EVIDENCE_TYPES = [
    "inspection_of_records", "inspection_of_assets", "observation",
    "external_confirmation", "recalculation", "reperformance",
    "analytical_procedure", "inquiry", "scanning",
]
    ...
    def assess_evidence(cls, account: str, assertions: list[str]) -> EvidenceCollection:
        ...
        for assertion in assertions:
            ...
            for proc in procedures:
                ...
                ec.items.append(EvidenceItem(
                    assertion=assertion,
                    procedure_type=proc,
                    description=f"Evidence for {assertion} using {proc}",
                    source="internal" if proc in ("recalculation", "reperformance", "analytical_procedure") else "external",
        ...
        for a in high_risk_assertions:
            max_rel = max(
                (e.reliability_score for e in ec.items if e.assertion == a), default=0
            )
            if max_rel < 0.6:
