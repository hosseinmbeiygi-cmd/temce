...
class CodalProfessionalAnalysisService:
    ...
    def analyze_symbol(self, symbol: str, industry: str | None = None) -> dict[str, Any]:
        ...
        benchmark = compare_with_industry(analysis, industry or "تولیدی") if industry else compare_with_industry(analysis, "تولیدی")
        ...
        going_concern = self.audit_workflow.going_concern_assessment(snap, consecutive_loss_years=1 if snap.net_profit < 0 else 0)
    ...
    def batch_map_accounts(self, labels: list[str], industry: str | None = None) -> dict[str, Any]:
        ...
        return {
            "results": [{"canonical_code": r.canonical_code, "canonical_name": r.canonical_name, "confidence": r.confidence, "method": r.method, "source_label": r.source_label} for r in results],
            "unmapped": unmapped,
        }
    ...
    def ifrs_lease_classify(
        ...
        result = self.ifrs_engine.lease_classification(annual_payment, lease_term, economic_life, asset_value, ownership_transfer)
        return {
    ...
    def ifrs_revenue_recognition(self, contract_price: float, obligations: int, standalone_prices: list[float] | None = None) -> dict[str, Any]:
    ...
    def tax_reconciliation(self, accounting_profit: float, permanent_additions: list[tuple[str, float]] | None = None) -> dict[str, Any]:
    ...
    def audit_going_concern(self, symbol: str) -> dict[str, Any]:
        ...
        result = self.audit_workflow.going_concern_assessment(snap, consecutive_loss_years=1 if snap.net_profit < 0 else 0)
        return {
    ...
    def audit_generate_program(
        self, symbol: str, specific_accounts: list[str] | None = None,
    ) -> dict[str, Any]:
        loaded = self._load_classified_data(symbol)
        if "error" in loaded:
            return {"symbol": symbol, **loaded}
        ...
        accounts = specific_accounts or [
            ...
            "property_plant_equipment", "payables", "cash_and_equivalents", "equity"
        ]
    ...
    def audit_add_misstatement(
        self, symbol: str, reference: str, description: str,
        amount: float, category: str, account: str,
    ) -> dict[str, Any]:
        ms = self.misstatement_evaluator.record_misstatement(
            reference, description, amount, category, account
        )
        return {
    ...
    def audit_sampling_attribute(
        self, population_size: int, expected_rate: float, tolerable_rate: float,
    ) -> dict[str, Any]:
    ...
    def audit_sampling_classical(
        self, sample_values: list[float], book_values: list[float],
    ...
    def audit_estimate_range(
        self, point_estimate: float, low: float, high: float,
    ) -> dict[str, Any]:
    ...
    def audit_provision_test(
        self, provision_type: str, recorded: float,
        estimated_low: float, estimated_high: float,
    ) -> dict[str, Any]:
        result = self.estimates_auditor.test_provision(
    ...
    def audit_form_opinion(
        self, uncorrected_misstatements: float = 0,
    ...
    def audit_obtain_representation(self, entity: str, period: str, obtain_all: bool = True, date: str = "") -> dict[str, Any]:
        letter = self.representation_manager.create_letter(entity, period)
        if obtain_all:
    ...
    def audit_inventory_observation(self, location: str, date: str, test_counts: int = 100, test_differences: int = 0) -> dict[str, Any]:
    ...
    def audit_litigation_assessment(self, case_name: str, nature: str, claim_amount: float, likelihood: str = "possible") -> dict[str, Any]:
    ...
    def audit_eqcr_review(self, engagement_partner: str, eqcr_partner: str, date: str, findings: list[dict[str, Any]] | None = None) -> dict[str, Any]:
