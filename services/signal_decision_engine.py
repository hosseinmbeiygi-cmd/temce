...
class SignalDecisionEngine:
    ...
    async def _gate_probability(
        self, candidate: SignalCandidate, effective_threshold: float, market: str,
    ) -> GateResult:
    ...
    async def _gate_risk_reward(self, candidate: SignalCandidate, market: str) -> GateResult:
        ...
        if rr < min_watchlist:
            return GateResult(
                ...
                details={"risk_reward": rr, "stop_loss_pct": candidate.stop_loss_pct, "target_pct": candidate.target_pct},
            )
    ...
    def signal_grade(self, decision: Decision) -> str:
        ...
        if (prob >= a_plus.get("min_probability", 0.70)
        ...
        if (prob >= a_grade.get("min_probability", 0.65)
        ...
        if (prob >= b_grade.get("min_probability", 0.60)
