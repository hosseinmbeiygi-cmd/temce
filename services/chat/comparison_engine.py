...
class ComparisonEngine:
    ...
    def _build_table(
        self, symbols: list[str], data_dict: dict[str, dict]
    ) -> dict[str, Any]:
        """Build comparison table data."""
        columns = [
            "symbol", "price", "change_pct", "volume", "value",
            "smc_score", "phase", "pe_ratio", "eps",
        ]
        ...
        for sym in symbols:
            ...
            row = {
                ...
                "phase": self._translate_phase(
                    data.get("phase", scores.get("phase", "neutral"))
                ),
    ...
    def _find_best_in_each(self, rows: list[dict]) -> dict[str, str]:
        ...
        best = {}
        numeric_cols = [
            "price", "change_pct", "volume", "value",
            "smc_score", "accumulation", "absorption",
            "breakout_readiness", "float_lock",
        ]
        higher_better = {
        ...
        for sym in symbols:
            ...
            for k, v in scores.items():
                if isinstance(v, (int, float)) and 0 <= v <= 1:
                    all_scores[k] = v
            for k in ["smc_score", "liquidity_score", "power_score",
                      "structure_score", "orderflow_score", "trigger_score"]:
