        ...
        for source_name, rss_url in self.RSS_SOURCES.items():
            try:
                ...
                for entry in feed.entries[:15]:
                    if not self._is_related_to_symbol(entry.get("title", ""), symbol) and \
                       not self._is_related_to_symbol(entry.get("summary", ""), symbol):
                        continue
    ...
    def _fetch_from_brsapi(self, symbol: str, days_back: int) -> list[dict[str, Any]]:
        ...
        try:
            if HAS_HTTPX:
                ...
                import requests
                url = self.API_ENDPOINTS["brsapi"]
                resp = requests.get(url, params={"key": self._brsapi_key, "l18": symbol, "days": days_back},
