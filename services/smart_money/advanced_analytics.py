...
class AdvancedAnalyticsEngine:
    ...
    def _mfi(self, closes: list[float], highs: list[float], lows: list[float], volumes: list[float], period: int = 14) -> float:
        if len(closes) < period + 1:
            return 50.0
    ...
    def _detect_patterns(self, closes: list[float], highs: list[float], lows: list[float], volumes: list[float]) -> list[PatternSignal]:
        patterns = []
        n = len(closes)
        if n < 3:
            return patterns
    ...
    def _find_support_resistance(self, closes: list[float], highs: list[float], lows: list[float]) -> list[SupportResistance]:
    ...
    def _analyze_volume(self, closes: list[float], highs: list[float], lows: list[float], volumes: list[float]) -> VolumeProfile:
        if not closes:
    ...
    def _compute_composite(self, ind: TechnicalIndicators, trend: TrendAnalysis, vol: VolatilityAnalysis, patterns: list[PatternSignal]) -> float:
        score = 0.0
