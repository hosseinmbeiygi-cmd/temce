...
class RiskAdjustedFilter:
    ...
    async def filter_signal(
        ...
        if risk_metrics.daily_volatility > max_vol:
            rejection_reason = f"نوسان بالا: {risk_metrics.daily_volatility*100:.1f}% (حداکثر مجاز: {max_vol*100:.1f}%)"
            ...
            rejection_reason = f"Drawdown بالا: {risk_metrics.max_drawdown_90d*100:.1f}% (حداکثر مجاز: {max_dd*100:.1f}%)"
        ...
        position_fraction = self._compute_position_size(
            risk_metrics, signal.get("score", 50) / 100.0,
            signal.get("confidence", 0.5), risk_tolerance
        )
    ...
    async def _compute_risk_metrics(
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                n = len(returns_arr)
                if n > 2:
                    skew = float(n * np.sum((returns_arr - mean_ret) ** 3) /
                                 ((n - 1) * (n - 2) * max(daily_vol ** 3, 0.0001)))
                    kurt = float((n * (n + 1) * np.sum((returns_arr - mean_ret) ** 4) /
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                if len(new_closes) < 20:
                    return 0.0
                new_returns = [(new_closes[i] - new_closes[i - 1]) / max(new_closes[i - 1], 0.001)
                ...
                for holding in holdings[:5]:  # check top 5 holdings
                    ...
                    if len(h_closes) < 20:
                        continue
                    h_returns = [(h_closes[i] - h_closes[i - 1]) / max(h_closes[i - 1], 0.001)
                                 for i in range(1, len(h_closes))]
    ...
    def _compute_position_size(
        ...
        return min(raw_size * scale * RiskAdjustedFilter.BASE_POSITION_FRACTION,
                   RiskAdjustedFilter.MAX_POSITION_FRACTION)
