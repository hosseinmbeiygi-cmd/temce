...
class TechnicalIndicators:
    ...
    @staticmethod
    def stochastic(highs: list[float], lows: list[float], closes: list[float], k_period: int = 14, d_period: int = 3) -> dict[str, list[float]]:
...
class CandlestickPatterns:
    @staticmethod
    def detect_all(opens: list[float], highs: list[float], lows: list[float], closes: list[float]) -> list[dict[str, Any]]:
        if len(opens) < 3:
            return []
        patterns = []
        i = len(opens) - 1
        ...
        if i >= 1:
            ...
            if c2 < o2 and c > o and o <= c2 and c >= o2 and body > body2 * 1.5:
                ...
                patterns.append({"name": "الگوی اینگالفینگ نزولی", "type": "bearish", "description": "شکست صعودی با قدرت"})
    ...
    def sector_correlation(sector_stocks: dict[str, list[float]]) -> dict[str, Any]:
        ...
        for i in range(len(symbols)):
            for j in range(i + 1, len(symbols)):
                corr = CorrelationAnalysis.pearson_correlation(
                    returns[symbols[i]],
                    returns[symbols[j]]
                )
...
class ReportGenerator:
    ...
    def daily_market_report(stocks: list[dict[str, Any]], alerts: list[dict[str, Any]] | None = None) -> str:
        ...
        if alerts:
            ...
            for a in alerts[:5]:
                lines.append(f"  • {a.get('symbol', '')}: {a.get('type', '')} {a.get('condition', '')} {a.get('threshold', '')}")
...
class StockAssistantService:
    ...
    async def _compare_stocks(self, sym1: str, sym2: str) -> dict[str, Any]:
        ...
        return {"text": "\n".join(lines), "type": "comparison", "data": {"sym1": sym1, "sym2": sym2, "score1": score1, "score2": score2}}
    ...
    async def _get_recommendation(self, symbol: str) -> dict[str, Any]:
        ...
        return {"text": "\n".join(lines), "type": "recommendation", "data": {"symbol": symbol, "score": score, "recommendation": rec}}
    ...
    async def _analyze_all(self) -> dict[str, Any]:
        ...
        return {"text": "\n".join(lines), "type": "analysis", "data": {"count": len(analyzed), "best": best["symbol"], "worst": worst["symbol"]}}
    ...
    async def _portfolio_handler(self, text: str) -> dict[str, Any]:
        ...
        if re.search(r"خلاصه|summary|وضعیت", text):
            ...
            if not summary["holdings"]:
                return {"text": "پرتفوی خالی است. با دستور «اضافه [نماد] [تعداد] [قیمت]» سهم اضافه کنید.", "type": "portfolio"}
    ...
    async def _alert_handler(self, text: str) -> dict[str, Any]:
        if re.search(r"اضافه|add|جدید", text):
            match = re.search(r"(\S+)\s+(rsi|price|قیمت|hajm|حجم)\s*(above|below|بالاتر|پایین‌تر)\s*(\d+(?:\.\d+)?)", text, re.IGNORECASE)
            if match:
                symbol = match.group(1)
