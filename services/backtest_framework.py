...
class VectorizedBacktest:
    ...
    def run(self, close_prices: list[float], signals: list[int],
...
class CPCV:
    ...
    def generate_combinations(self, n_total: int) -> list[dict[str, Any]]:
        ...
        groups = [list(range(i * group_size, min((i + 1) * group_size, n_total)))
        ...
        for test_indices in test_combos:
            ...
            embargo_set = list(range(max(0, min(test_set) - embargo_len), min(n_total, max(test_set) + embargo_len + 1)))
...
class RegimeEvaluator:
    ...
    def evaluate(self, equity_curve: list[float], regime_labels: list[int],
...
class LiquidityAnalyzer:
    ...
    def analyze(self, equity_curve: list[float], position_values: list[float],
                daily_volumes: list[float], participation_rate: float = 0.1) -> dict[str, Any]:
        days_to_liquidate = []
...
class StatisticalValidator:
    ...
    def deflated_sharpe(self, observed_sharpe: float, n_trials: int,
        ...
        e_max_sharpe = math.sqrt(2 * math.log(max(n_trials, 1))) * (1 - euler_mascheroni / (2 * math.log(max(n_trials, 1)))) + euler_mascheroni / (2 * math.sqrt(2 * math.log(max(n_trials, 1))))

        # Standard error of Sharpe
        se_sharpe = math.sqrt((1 + 0.5 * observed_sharpe ** 2 - skewness * observed_sharpe + (kurtosis - 3) / 4 * observed_sharpe ** 2) / max(n_observations - 1, 1))
    ...
    def reality_check(self, strategy_returns: list[list[float]],
