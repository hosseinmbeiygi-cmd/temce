...
class PostgreSQLClient:
    ...
    def _initialize_pool(self, min_conn: int, max_conn: int) -> None:
        if not self._pool:
            try:
                self._pool = SimpleConnectionPool(minconn=min_conn, maxconn=max_conn, **connection_params)
    ...
    def create_tables(self):
        with self._get_connection() as conn, conn.cursor() as cursor:
            for table, definition in self.table_definitions.items():
                try:
                    cursor.execute(
                        sql.SQL("CREATE TABLE IF NOT EXISTS {} ({})").format(sql.Identifier(table), sql.SQL(definition))
                    )
                    conn.commit()
        ...
        with self._get_connection() as conn, conn.cursor() as cursor:
            ...
            for table in tables:
                ...
                try:
                    ...
                    result[table] = {"path": csv_path, "size_bytes": file_size, "records": file_size > 0}
                except Exception as e:
...
if __name__ == "__main__":
    ...
    client.insert(
        "backtest_metrics", {"run_id": run_id, "metric_name": "total_return", "metric_value": 0.5, "annualized": False}
    )
