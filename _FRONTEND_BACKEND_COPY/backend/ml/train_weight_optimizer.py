...
class WeightOptimizer:
    ...
    async def _load_last_price_date(self) -> pd.Timestamp | None:
        ...
        async for session in get_session():
            ...
            val = (
                await session.execute(
                    text("SELECT max(gregorian_date) FROM brsapi_historical_daily WHERE gregorian_date IS NOT NULL")
                )
            ).scalar()
            break
    ...
    async def _load_forward_returns(
        ...
        df = pd.DataFrame([{"symbol": r.symbol, "date": r.gregorian_date, "close": r.price_close} for r in rows])
        if df.empty:
            return df
        df["date"] = pd.to_datetime(df["date"])
    ...
    async def train(self, output_dir: str | Path | None = None) -> dict[str, Any]:
        ...
        merged = scores.merge(returns, left_on=["trade_date", "symbol"], right_on=["date", "symbol"], how="inner")
        ...
        if unmapped_frac > 0.05:
            logger.warning(
                "%.1f%% of merged rows have no index regime (sparse index history) — they are labelled 'neutral'",
                unmapped_frac * 100,
            )
        ...
        for regime in REGIMES:
            ...
            summary["regimes"][regime] = {
                ...
                "top_features": dict(sorted(weights.items(), key=lambda kv: -abs(kv[1]))[:10]),
            }
    ...
    async def from_args(cls, argv: list[str] | None = None) -> None:
        ...
        parser.add_argument(
            "--wf-windows", type=int, default=4, help="purged walk-forward windows (0 = disable validation)"
        )
        parser.add_argument(
            "--embargo-pct", type=float, default=0.02, help="embargo fraction between train and test (F6 leak guard)"
        )
        parser.add_argument("--purge-pct", type=float, default=0.02, help="purge fraction at the train/test boundary")
        parser.add_argument(
            "--no-walk-forward",
            action="store_true",
            help="skip purged validation — write provisional in-sample weights",
        )
