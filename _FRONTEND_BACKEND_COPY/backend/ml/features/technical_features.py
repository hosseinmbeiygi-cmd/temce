...
class TechnicalFeatures(BaseFeatureBuilder):
    def compute(self, data: pd.DataFrame) -> FeatureMatrix:
        ...
        if high is not None and low is not None:
            ...
            df["sr_distance_to_resistance"] = [
                (c - r) / c if c and r else 0.0 for c, r in zip(c_list, sr_res, strict=False)
            ]
            df["sr_distance_to_support"] = [
                (c - s) / c if c and s else 0.0 for c, s in zip(c_list, sr_sup, strict=False)
            ]
    ...
    def get_feature_names(self) -> list[str]:
        return [
            ...
            "squeeze_squeeze_on",
            "squeeze_momentum",
            "sr_distance_to_resistance",
            "sr_distance_to_support",
            "sr_break_signal",
        ]
