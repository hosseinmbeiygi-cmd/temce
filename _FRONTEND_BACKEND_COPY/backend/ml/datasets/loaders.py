...
class DataLoader:
    ...
    async def _get_tsetmc_history(self, ins_code: str, start: str, end: str) -> list[dict] | None:
        ...
        try:
            async with aiohttp.ClientSession() as session, session.get(url, timeout=15) as response:
                ...
                filtered = [
                    item
                    for item in data["closingPriceHistory"]
    ...
    async def load_market_data(
        self, instrument_ids: list[str], start: str, end: str, source: str = "yahoo"
    ) -> Result[pd.DataFrame]:
        """Load market data with improved error handling and retry mechanism"""
        logger.info(
            "Loading market data for %d instruments from %s to %s (source: %s)", len(instrument_ids), start, end, source
        )

        if source.lower() == "yahoo":
            try:
                data = yf.download(tickers=" ".join(instrument_ids), start=start, end=end, group_by="ticker")
                if data.empty:
            ...
            try:
                ...
                for symbol in instrument_ids:
                    ...
                    # Get historical data
                    history = await self._get_tsetmc_history(stock_info["ins_code"], start, end)
