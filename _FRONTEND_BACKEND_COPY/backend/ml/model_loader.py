...
class ModelLoader:
    ...
    def __init__(
        ...
        self._load_cached = functools.lru_cache(maxsize=maxsize)(self._load_from_disk)
    ...
    def invalidate(self, symbol: str, algorithm: str | None = None) -> None:
        ...
        if algorithm:
            ...
            keys = [k for k, entry in self._loaded.items() if entry.symbol == symbol]
        ...
        self._load_cached = functools.lru_cache(maxsize=self._maxsize)(self._load_from_disk)
    ...
    def _load_from_disk(self, cache_key: str) -> Any:
        ...
        raise FileNotFoundError(f"No model file found in {artifact_dir} (tried: {[p.name for p in candidates]})")
