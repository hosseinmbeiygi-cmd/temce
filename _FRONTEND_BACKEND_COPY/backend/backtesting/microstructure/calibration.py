...
class MicrostructureCalibrator:
    ...
    def _calibrate_queue_metrics(
        self, params: SymbolMicrostructureParams, quotes: list[dict[str, Any]], total_seconds: float
    ) -> None:
        ...
        params.avg_queue = (
            int((params.avg_queue_bid + params.avg_queue_ask) / 2)
            if (params.avg_queue_bid + params.avg_queue_ask) > 0
            else 0
        )
