...
class MicrostructureEngine:
    ...
    def process_event(
        ...
        if event.event_type == EventType.QUOTE:
            ...
            impact_price = self.impact_model.get_execution_price(trade_vol, adv=1000000, price=trade_price, side="buy")
    ...
    def estimate_queue_lifetime(self, instrument_id: str) -> float:
        ...
        drain_rate = (
            (total_flow / max(queue.bid_trade_flow + queue.ask_trade_flow, 1))
            if queue.bid_trade_flow + queue.ask_trade_flow > 0
            else 0
        )
    ...
    def _update_cancel_rates(self, queue: QueueState, inst_id: str) -> None:
        ...
        cancel_bid = self.cancel_model.estimate_cancel(prev.get("bid", 0), queue.bid_queue_volume, queue.bid_trade_flow)
        cancel_ask = self.cancel_model.estimate_cancel(prev.get("ask", 0), queue.ask_queue_volume, queue.ask_trade_flow)
