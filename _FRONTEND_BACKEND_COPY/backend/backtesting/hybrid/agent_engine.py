...
class HybridNoiseTrader(SyntheticAgent):
    ...
    def observe(self, market_state: dict[str, Any]) -> None:
        self._last_price = (
            market_state.get("last_price", 0) or market_state.get("midpoint", 0) or self._last_price or 1000.0
        )
...
class HybridMeanReversion(SyntheticAgent):
    ...
    def decide(self, market_state: dict[str, Any]) -> list[OrderEvent] | None:
        ...
        if z > self.entry_z:
            ...
            return [
                OrderEvent(
                    instrument_id="", side="buy", quantity=self.order_size, price=round(current, 2), order_type="MARKET"
                )
            ]
        return None
