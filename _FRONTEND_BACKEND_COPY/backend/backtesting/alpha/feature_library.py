...
class FeatureSet:
    ...
    def update(self, market_state: dict[str, Any]) -> dict[str, float]:
        ...
        if "queue_imbalance" in self.features:
            computed["queue_imbalance"] = self.features["queue_imbalance"].update(
                FeatureLibrary.queue_imbalance(bid_vol, ask_vol)
            )

        if "microprice" in self.features:
            computed["microprice"] = self.features["microprice"].update(
                FeatureLibrary.microprice(bid, ask, bid_vol, ask_vol)
            )
