...
class FeatureDAGEngine:
    ...
    def register_builtins(self) -> None:
        builtins = [
            ...
            FeatureNode(
                ...
                ["trade_volume", "bid_volume", "ask_volume"],
                lambda tv, bv, av: (tv - (bv + av)) / (bv + av + tv) if (bv + av + tv) > 0 else 0.0,
                FeatureType.MICROSTRUCTURE,
                description="trade flow imbalance",
            ),
            FeatureNode(
                "trade_intensity", ["volume"], lambda v: v, FeatureType.VOLUME, description="trade volume per event"
            ),
            FeatureNode(
                "microprice",
                ...
                FeatureType.MICROSTRUCTURE,
                description="weighted mid price",
            ),
    ...
    def get_all_features(self, instrument_id: str = "") -> dict[str, float]:
        prefix = f"{instrument_id}:" if instrument_id else ""
        return {k.removeprefix(prefix): v for k, v in self._cache.items() if k.startswith(prefix) or not instrument_id}
