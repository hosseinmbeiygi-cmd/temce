...
class BacktestRunner:
    ...
    def resolve(self, engine: str | Any) -> str:
        ...
        if isinstance(engine, str):
            if engine not in self._ENGINES:
                raise ValueError(f"unknown backtest engine {engine!r} — choose from {list(self._ENGINES)}")
            return engine
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        if name in DEPRECATED_ENGINES:
            warnings.warn(
                f"Backtest engine {name!r} is deprecated (audit D1) — use 'simulator' (BacktestSimulator) instead",
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    async def _dispatch(
        ...
        strategy_name = getattr(strategy, "__class__", None).__name__ if strategy is not None else "Strategy"
        return self._to_backtest_result(raw, strategy_name, initial_capital)

    @staticmethod
    def _to_backtest_result(raw: Any, strategy_name: str, initial_capital: float) -> BacktestResult:
        ...
        if hasattr(obj, "to_backtest_result"):
            return obj.to_backtest_result(strategy_name=strategy_name, initial_capital=initial_capital)
        raise RuntimeError(
    ...
    def _summarize(self, bt: BacktestResult, engine: str, initial_capital: float) -> dict[str, Any]:
