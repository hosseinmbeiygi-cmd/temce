    ...
    def check_order(
        ...
        if side == "buy":
            ...
            if remaining_cash < required_reserve:
                result.allowed = False
                result.reason = (
                    f"Insufficient cash reserve: would have {remaining_cash:.0f}, need {required_reserve:.0f}"
                )
                return result
        ...
        if side == "buy":
            ...
            if portfolio_value > 0:
                ...
                if new_pct > self.policy.max_position_pct:
                    result.allowed = False
                    result.reason = (
                        f"Position limit breached for {symbol}: {new_pct:.2%} > {self.policy.max_position_pct:.2%}"
                    )
                    return result
        ...
        if side == "buy" and sector and sector_exposure:
            ...
            if portfolio_value > 0:
                ...
                if new_sector_pct > self.policy.max_sector_pct:
                    result.allowed = False
                    result.reason = (
                        f"Sector limit breached for {sector}: {new_sector_pct:.2%} > {self.policy.max_sector_pct:.2%}"
                    )
                    return result
