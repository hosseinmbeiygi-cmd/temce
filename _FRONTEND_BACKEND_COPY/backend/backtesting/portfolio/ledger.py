...
class PortfolioLedger:
    ...
    def buy(self, symbol: str, quantity: int, price: float, current_date: date) -> LedgerEntry | None:
        ...
        if total_cost > self.get_available_cash():
            logger.warning(
                "Insufficient cash for buy %s: need %.2f, have %.2f", symbol, total_cost, self.get_available_cash()
            )
            return None
    ...
    def sell(self, symbol: str, quantity: int, price: float, current_date: date) -> LedgerEntry | None:
