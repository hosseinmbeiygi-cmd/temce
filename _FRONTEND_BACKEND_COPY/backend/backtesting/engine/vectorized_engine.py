...
class VectorizedBacktestEngine:
    ...
    def calculate_metrics(
        ...
        sharpe = (
            float(excess.mean() / returns.std(ddof=0) * (ANNUAL_TRADING_DAYS**0.5)) if returns.std(ddof=0) > 0 else 0.0
        )

        downside = returns[returns < 0]
        downside_std = float(downside.std(ddof=0)) if len(downside) > 1 else 0.0
        sortino = float(returns.mean() / downside_std * (ANNUAL_TRADING_DAYS**0.5)) if downside_std > 0 else 0.0
    ...
    def plot_equity_curve(self, metrics: dict[str, Any], output: str = "equity_curve.png") -> str:
        ...
        try:
            ...
            path_d = " ".join(f"L{x},{100 - float(v) * 100}" for x, v in enumerate(curve.values))
            svg = (
