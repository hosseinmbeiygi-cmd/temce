...
class VisualizationEngine:
    ...
    def _build_monthly_returns(self, points: list[EquityPoint]) -> list[dict[str, Any]]:
        ...
        return [
            {"month": k, "return_pct": float(np.sum(v)), "n_observations": len(v)} for k, v in sorted(monthly.items())
        ]
    ...
    def _build_rolling_sharpe(
        self, points: list[EquityPoint], window: int = 60, risk_free_rate: float = 0.0
    ) -> list[dict[str, Any]]:
