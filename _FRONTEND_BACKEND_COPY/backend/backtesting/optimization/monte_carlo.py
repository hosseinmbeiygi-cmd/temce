...
class MonteCarloSimulator:
    ...
    def simulate(
        ...
        if var_pnl > 0 and len(pnls) > 1:
            autocorr_1 = sum((pnls[i] - mean_pnl) * (pnls[i - 1] - mean_pnl) for i in range(1, len(pnls))) / (
                (len(pnls) - 1) * var_pnl
            )
        ...
        return {
            ...
            "recommendation": self._generate_recommendation(prob_profit, prob_ruin_50, autocorr_1, kelly_fraction),
        }
