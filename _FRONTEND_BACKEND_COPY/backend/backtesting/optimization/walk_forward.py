...
class WalkForwardOptimizer:
    ...
    async def optimize(
        ...
        for w in range(self.windows):
            ...
            best_params, best_score = await self._optimize_window(cls, all_combos, keys, train_data, capital, simulator)

            # Validate on test data (out-of-sample)
            if best_params:
                try:
                    strategy = cls(
                        **{k: int(v) if isinstance(v, float) and v == int(v) else v for k, v in best_params.items()}
                    )
        ...
        # Compute aggregate metrics (v2:115 - 80% + CI 95%)
        ...
        # Primary criterion: expectancy >0 in OOS windows (not raw return), with CI
        expectancy_positive = sum(1 for r in oos_results if r.get("expectancy", r.get("return_pct", 0)) > 0)
        # also check return consistency for backward compat
        consistency = sum(1 for r in oos_results if r["return_pct"] > 0) / len(oos_results) * 100
        wf_win_rate = expectancy_positive / len(oos_results) * 100 if oos_results else 0
        # Bootstrap CI for OOS returns (95%)
        try:
            oos_returns = [r["return_pct"] for r in oos_results]
            oos_returns_sorted = sorted(oos_returns)
            n = len(oos_returns_sorted)
            ci_low = oos_returns_sorted[max(0, int(n * 0.025))] if n >= 4 else oos_returns_sorted[0]
            ci_high = oos_returns_sorted[min(n - 1, int(n * 0.975))] if n >= 4 else oos_returns_sorted[-1]
        except Exception:
            ci_low, ci_high = 0, 0
        passes_wf = wf_win_rate >= 80.0 and ci_low > 0  # v2:115 strict
        ...
        return {
            ...
            "wf_win_rate": round(wf_win_rate, 1),
            "wf_ci_low": round(ci_low, 2),
            "wf_ci_high": round(ci_high, 2),
            "passes_wf_strict": passes_wf,
    ...
    def _generate_recommendation(
        ...
        """Generate human-readable recommendation based on metrics (v2:115)."""
        if overfitting < 30 and consistency >= 80 and stability > 0.7:
            return "EXCELLENT (v2 strict): >=80% OOS windows positive with 95% CI >0. Ready for paper trading (30-trade rule)."
        elif overfitting < 30 and consistency > 70 and stability > 0.7:
            ...
            return (
                "GOOD: Strategy shows reasonable OOS performance. Consider additional validation before live trading."
            )
        elif overfitting < 70:
            return "CAUTION: Strategy may be overfit. Parameters are somewhat unstable. Increase validation period."
        else:
            return (
                "WARNING: Strategy shows signs of overfitting. Do NOT use for live trading without further validation."
            )
