...
REGISTRY: list[IndicatorSpec] = [
    ...
    IndicatorSpec(
        id="macd",
        name="MACD",
        name_fa="همگرایی واگرایی میانگین",
        group="momentum",
        ...
        conditions={
            ...
            "positive": "positive_histogram",
            "negative": "negative_histogram",
        },
    ),
    IndicatorSpec(
        id="squeeze_momentum",
        name="Squeeze Momentum",
        name_fa="فشردگی مومنتوم",
        group="momentum",
        compute_fn=lambda high, low, close, bb_period=20, bb_std=2.0, kc_period=20, kc_mult=1.5: _compute_squeeze(
            high, low, close, bb_period, bb_std, kc_period, kc_mult
        ),
        params={
            "bb_period": [15, 20, 25],
            "bb_std": [1.5, 2.0, 2.5],
    ...
    IndicatorSpec(
        ...
        name_fa="استوکاستیک",
        group="momentum",
        compute_fn=lambda high, low, close, k_period=14, d_period=3, smooth=3: _compute_stochastic(
    ...
    IndicatorSpec(
        ...
        name_fa="نیم‌رونده",
        group="trend",
        compute_fn=lambda high, low, close, amplitude=2, channel_deviation=2.0: _compute_half_trend(
    ...
    IndicatorSpec(
        ...
        name_fa="پارابولیک",
        group="trend",
        compute_fn=lambda high, low, close, step=0.02, max_af=0.2: _compute_parabolic_sar(
            high, low, close, step, max_af
        ),
    ...
    IndicatorSpec(
        ...
        name_fa="ایچیموکو",
        group="trend",
        compute_fn=lambda high, low, close, tenkan=9, kijun=26, senkou=52: _compute_ichimoku(
    ...
    IndicatorSpec(
        ...
        name_fa="پهنای بولینگر",
        group="volatility",
        compute_fn=lambda high, low, close, period=20, std=2.0: _compute_bollinger_bw(high, low, close, period, std),
    ...
    IndicatorSpec(
        ...
        name_fa="موقعیت کلتنر",
        group="volatility",
        compute_fn=lambda high, low, close, period=20, mult=2.0: _compute_keltner(high, low, close, period, mult),
    ...
    IndicatorSpec(
        ...
        name_fa="نسبت فشردگی",
        group="volatility",
        compute_fn=lambda high, low, close, short_p=5, long_p=20: _compute_compression(
            high, low, close, short_p, long_p
        ),
    ...
    IndicatorSpec(
        id="vwap",
        name="VWAP",
        name_fa="میانگین وزنی حجمی",
        group="volume",
        compute_fn=lambda close, high, low, volume, period=20: _compute_vwap(close, high, low, volume, period),
        params={"period": [10, 15, 20, 30]},
    ...
    IndicatorSpec(
        id="buyer_power",
        name="Buyer Power Ratio",
        name_fa="نسبت قدرت خریدار",
        group="flow",
        compute_fn=lambda buy_val, buy_cnt, sell_val, sell_cnt: _compute_buyer_power(
    ...
    IndicatorSpec(
        ...
        name_fa="حمایت/مقاومت",
        group="structure",
        compute_fn=lambda high, low, close, lookback=20: _compute_sr(high, low, close, lookback),
    ...
    IndicatorSpec(
        ...
        name_fa="کیفیت شکست",
        group="structure",
        compute_fn=lambda close, high, low, volume, period=20: _compute_breakout_quality(
            close, high, low, volume, period
        ),
    ...
    IndicatorSpec(
        ...
        name="Normalized Volatility",
        name_fa="نوسان نرمال‌شده",
        group="volatility",
    ...
    IndicatorSpec(
        ...
        name_fa="قدر نسبی",
        group="relative",
        compute_fn=lambda asset_price, benchmark_price, period=20: _compute_rs(asset_price, benchmark_price, period),
        params={"period": [10, 15, 20, 30]},
...
def _compute_macd(close: list[float], fast: int, slow: int, signal: int) -> dict[str, list]:
    ...
    return {
        ...
        "bullish_cross": bullish_cross,
        "bearish_cross": bearish_cross,
        "positive_histogram": positive_h,
        "negative_histogram": negative_h,
    }
...
def _compute_squeeze(high, low, close, bb_p, bb_s, kc_p, kc_m):
    ...
    return {
        ...
        "momentum": r["momentum"],
        "momentum_positive": r["momentum_positive"],
        "momentum_negative": [not v for v in r["momentum_positive"]],
...
def _compute_breakout_quality(close, high, low, volume, period):
    ...
    price_change = [
        (close[i] - close[i - 1]) / close[i - 1] * 100 if close[i - 1] > 0 else 0 for i in range(1, len(close))
    ]
...
def _compute_rs(asset_price, benchmark_price, period):
    ...
    rs = [
        a / b if a is not None and b is not None and b > 0 else None for a, b in zip(asset_sma, bench_sma, strict=False)
    ]
    ...
    return [
        {
            ...
            "group": s.group,
            "params": s.params,
            "conditions": list(s.conditions.keys()),
        }
        for s in REGISTRY
    ]
