...
class UniversalCollector:
    ...
    def collect_all(self, symbols: list[str] | None = None) -> dict[str, dict[str, list[CollectedData]]]:
        ...
        logger.info(
            "Starting universal data collection for %d symbols via %d adapters", len(symbols), len(self.adapters)
        )
    ...
    def get_status(self) -> dict[str, Any]:
        return {
            ...
            "errors": {name: len(adapter.get_errors()) for name, adapter in self.adapters.items()},
        }
