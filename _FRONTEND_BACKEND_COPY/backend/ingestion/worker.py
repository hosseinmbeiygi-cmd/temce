...
class IngestionWorker:
    ...
    async def ingest_all(self) -> dict[str, int]:
        ...
        for name, source in self._sources.items():
            try:
                if isinstance(
                    source, (TsetmcTradeSource, TsetmcOrderBookSource, TsetmcOptionSource, TsetmcFutureSource)
                ):
                    total = 0
                    instruments_to_fetch = (
                        self._instrument_ids[:5] if len(self._instrument_ids) > 5 else self._instrument_ids
                    )
        ...
        for payload in fetch_result.payloads:
            ...
            for event in events:
                ...
                if self._storage:
                    ...
                    # Canonical cross-market ticks use their own instrument
                    # namespace and do not need a row in the TSE symbol table.
                    target_id = ins_id or event.data.get("instrument_id")
                    if target_id and (ins_id or event.event_type == "market_tick"):
                        event.data["instrument_id"] = target_id
                        await self._write_event(event, str(target_id))
    ...
    async def _write_event(self, event: Any, instrument_id: str) -> None:
        ...
        try:
            if event_type == "trade_tick":
                ...
                await self._storage.write_market_tick(instrument_id, event.data, event.source)
        except Exception:
