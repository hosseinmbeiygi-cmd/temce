...
class RefreshMaterializedViewsJob(BaseJob):
    """Q1 P0: keep mv_latest_* fresh (CONCURRENT, no read lock).

    Schedule: every 60s via jobs/scheduler. Falls back to no-op pre-migration.
    """

    async def execute(self, context: JobContext) -> JobResult:
        from services.materialized_view_service import refresh_latest_views_with_new_session

        result = await refresh_latest_views_with_new_session()
        # Treat not_exists as success (pre-migration)
        has_error = any(v.startswith("error") for v in result.values())
        if has_error:
            return JobResult.failure_result(job_name=self.name, error=str(result))
