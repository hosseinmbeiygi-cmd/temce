...
class _BrsApiSyncBaseJob(BaseJob):
    ...
    async def execute(self, context: JobContext) -> JobResult:
        ...
        if endpoint is None:
            return JobResult.failure(f"Unknown endpoint: {self._endpoint_path}", job_name=self._name)
        ...
        try:
            async for session in get_session():
                ...
                return JobResult.failure(report.error or "unknown error", job_name=self._name)
