...
def _safe_drop_index(conn, name: str) -> None:
    row = conn.execute(sa.text("SELECT 1 FROM pg_indexes WHERE indexname = :n"), {"n": name}).fetchone()
    if row is not None:
...
def _safe_dedup_and_create_unique(conn, table: str, columns: list[str], name: str) -> None:
    ...
    # Count duplicates
    col_expr = " || '|-' || ".join(f"COALESCE({c}::text,'')" for c in columns)
    dup_sql = (
        "SELECT COUNT(*) FROM ("
    ...
    if dup_count > 500_000:
        print(f"  ! skipping {name}: {dup_count} duplicate groups (too many to dedup)")
        return
