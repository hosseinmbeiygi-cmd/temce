...
def upgrade() -> None:
    ...
    try:
        row = bind.execute(
            sa.text("SELECT hypertable_name FROM timescaledb_information.hypertables WHERE hypertable_name = :t"),
            {"t": _QUOTES},
        ).first()
    ...
    if is_hypertable:
        ...
        with contextlib.suppress(Exception):
            bind.execute(
                sa.text("SELECT create_hypertable(:t, :col, if_not_exists := TRUE)"),
                {"t": _QUOTES, "col": "time"},
            )
