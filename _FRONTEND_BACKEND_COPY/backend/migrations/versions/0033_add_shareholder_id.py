...
def upgrade() -> None:
    conn = op.get_bind()
    conn.execute(text("ALTER TABLE brsapi_shareholder_records ADD COLUMN IF NOT EXISTS shareholder_id BIGINT"))
    conn.execute(
        text(
...
def downgrade() -> None:
    conn = op.get_bind()
    conn.execute(text("DROP INDEX IF EXISTS ix_brsapi_shareholder_records_shareholder_id"))
    conn.execute(text("ALTER TABLE brsapi_shareholder_records DROP COLUMN IF EXISTS shareholder_id"))
