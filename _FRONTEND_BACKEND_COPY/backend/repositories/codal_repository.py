...
class _CodalDbRepo(DbRepository[Disclosure, CodalReportModel]):
    ...
    async def get_by_instrument(
        ...
        count_stmt = (
            select(sa_func.count()).select_from(CodalReportModel).where(CodalReportModel.instrument_id == instrument_id)
        )
    ...
    async def get_by_symbol(
        ...
        count_stmt = select(sa_func.count()).select_from(CodalReportModel).where(CodalReportModel.symbol == symbol)
        total_result = await self.session.execute(count_stmt)
        total = total_result.scalar() or 0
