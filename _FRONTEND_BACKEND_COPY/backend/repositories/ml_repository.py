...
class MlRepository:
    def __init__(self, session: AsyncSession | None = None) -> None:
        ...
        self._mem_version: InMemoryRepository[ModelVersion] | None = (
            None if session else InMemoryRepository[ModelVersion]()
        )
...
class _MlDbRepo(DbRepository):
    ...
    async def list_training_runs(self, page: int = 1, page_size: int = 100) -> Result[PaginatedResult[TrainingRun]]:
        ...
        stmt = (
            select(MlTrainingRunModel)
            .order_by(desc(MlTrainingRunModel.created_at))
    ...
    async def get_training_runs_by_status(self, status: str) -> Result[list[TrainingRun]]:
        stmt = (
            select(MlTrainingRunModel)
            .where(MlTrainingRunModel.status == status)
            .order_by(desc(MlTrainingRunModel.created_at))
        )
        result = await self.session.execute(stmt)
        rows = result.scalars().all()
