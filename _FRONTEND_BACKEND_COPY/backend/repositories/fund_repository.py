...
class FundRepository:
    ...
    async def get_by_fund_type(
        self, fund_type: str, page: int = 1, page_size: int = 100
    ) -> Result[PaginatedResult[Fund]]:
...
class _FundDbRepo(DbRepository[Fund, FundModel]):
    ...
    async def get_by_fund_type(
        ...
        stmt = select(FundModel).where(FundModel.fund_type == fund_type).offset((page - 1) * page_size).limit(page_size)
        result = await self.session.execute(stmt)
