...
class ScreenerProfile(TimestampMixin, Base):
    ...
    feedstock_change_pct: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="تغییرات نرخ خوراک در ۳ ماه آینده — ستون ۲۲"
    )
    capital_increase_type: Mapped[str | None] = mapped_column(
        String(30), comment="نوع افزایش سرمایه: نقدی/تجدید ارزیابی/ندارد — ستون ۲۴"
    )
    capital_increase_pct: Mapped[float | None] = mapped_column(
        Float, comment="مبلغ افزایش سرمایه (درصد سرمایه فعلی) — ستون ۲۵"
    )
    ...
    avg_50d_volume: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="میانگین حجم ۵۰ روزه — ستون ۳۷"
    )
    avg_daily_value: Mapped[float | None] = mapped_column(
        Float, comment="میانگین ارزش معاملات روزانه (تومان) — ستون ۳۹"
    )
    ...
    farabourse_volume: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="حجم معاملات فرابورس (هفته) — ستون ۴۳"
    )
    farabourse_price: Mapped[float | None] = mapped_column(
        Float, nullable=True, comment="قیمت سهم در فرابورس — ستون ۴۵"
    )

    # ── بخش پنجم: فیلترهای رویدادی و کیفی (ستون‌های ۵۶-۷۴) — مقدار ۰ یا ۱ ──
    ceo_change_success: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="تغییر مدیرعامل موفق در ۶ ماه اخیر؟ — ستون ۵۶"
    )
    ceo_change_fail: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="تغییر مدیرعامل ناموفق — ستون ۵۷"
    )
    annual_meeting_near: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="مجمع سالیانه نزدیکتر از ۴۵ روز — ستون ۵۸"
    )
    annual_meeting_passed: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="مجمع گذشته و ریسک ریزش — ستون ۵۹"
    )
    capital_increase_cash: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="افزایش سرمایه نقدی اعلام شده — ستون ۶۰"
    )
    capital_increase_reval: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="افزایش سرمایه تجدید ارزیابی — ستون ۶۱"
    )
    price_liberation_news: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="اخبار گشایش قیمتی در صنعت — ستون ۶۲"
    )
    gov_support_news: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="حمایت دولتی اعلام‌شده — ستون ۶۳"
    )
    heavy_legal_case: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="پرونده حقوقی سنگین (>۱۰٪ سود) — ستون ۶۴"
    )
    telegram_pump: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="پیشنهاد خرید در >۳ کانال تلگرامی — ستون ۶۵"
    )
    end_of_month: Mapped[int | None] = mapped_column(Integer, server_default="0", comment="روز پایانی ماه — ستون ۶۶")
    pre_holiday: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="روز قبل از تعطیلات طولانی — ستون ۶۷"
    )
    political_tension: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="رویداد سیاسی تنش‌زا در ۳ ماه آینده — ستون ۶۸"
    )
    political_relief: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="رویداد سیاسی گشایش‌زا — ستون ۶۹"
    )
    feedstock_meeting: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="جلسه تعیین نرخ خوراک در ۳ ماه آینده — ستون ۷۰"
    )
    big_ipo: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="IPO بزرگ در ۳۰ روز آینده — ستون ۷۱"
    )
    new_shareholder_capital: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="افزایش سرمایه از محل سهامداران جدید — ستون ۷۲"
    )
    positive_mgmt_news: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="اخبار مثبت مدیریتی — ستون ۷۳"
    )
    negative_mgmt_news: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="اخبار منفی مدیریتی — ستون ۷۴"
    )

    # ── بخش ششم: فیلترهای محاسباتی خودکار (ستون‌های ۷۷-۸۴) — با فرمول از داده‌های موجود محاسبه می‌شوند ──
    f77_dollar_eps_growth: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="نرخ رشد دلاری EPS > ۱۵٪ — ستون ۷۷"
    )
    f78_real_eps_growth: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="نرخ رشد واقعی EPS > ۱۵٪ — ستون ۷۸"
    )
    f79_pe_ratio_ok: Mapped[int | None] = mapped_column(Integer, server_default="0", comment="ضریب P/E < ۱.۲ — ستون ۷۹")
    f80_yield_gt_bank: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="بازده سهام > نرخ سود بانکی — ستون ۸۰"
    )
    f81_inst_ratio_ok: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="نسبت خرید خالص حقوقی > ۵٪ شناور — ستون ۸۱"
    )
    f82_volume_spike: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="جهش حجمی > ۳ برابر میانگین — ستون ۸۲"
    )
    f83_liquidity_ok: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="نقدشوندگی > ۰.۵٪ شناور در روز — ستون ۸۳"
    )
    f84_loss_ratio_ok: Mapped[int | None] = mapped_column(
        Integer, server_default="0", comment="نسبت زیان انباشته به سرمایه < ۵۰٪ — ستون ۸۴"
    )
...
class ScreenerSignal(TimestampMixin, Base):
    ...
    exit_reason: Mapped[str | None] = mapped_column(
        String(30), nullable=True, comment="دلیل خروج: stop_loss/target/manual"
    )
