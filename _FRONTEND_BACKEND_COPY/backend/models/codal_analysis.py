...
class DimAccount(Base):
    ...
    children: Mapped[list["DimAccount"]] = relationship(
        "DimAccount", backref="parent", remote_side="DimAccount.account_id"
    )
...
class FactFinancials(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "company_id", "date_id", "account_id", "document_id", "restatement_version", name="uq_fact_financials"
        ),
    )
...
class FactRatios(TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("company_id", "date_id", "ratio_code", name="uq_fact_ratios"),)
...
class FactGrowth(TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("company_id", "date_id", "metric_code", name="uq_fact_growth"),)
...
class AccountMapping(Base):
    ...
    __table_args__ = (UniqueConstraint("source_label", "industry_scope", name="uq_account_mapping"),)
