...
async def get_table_data(
    ...
    if not _SAFE_IDENTIFIER.match(table_name):
        return ApiResponse[dict[str, Any]](success=False, error={"message": "Invalid table name"})
    ...
    if not check.scalar():
        return ApiResponse[dict[str, Any]](success=False, error={"message": f"Table '{table_name}' not found"})
    ...
    columns = [{"name": row[0], "type": row[1]} for row in cols_result.fetchall() if row[0] not in _SENSITIVE_COLUMNS]
    ...
    rows = [
        {key: value for key, value in row._mapping.items() if key not in _SENSITIVE_COLUMNS}
