...
def _safe_error_message(exc: BaseException, default_message: str = 'Internal error') -> str:
 ...
 API — Enhanced endpoints with advanced analytics."""
...
async def screener_v2_filter(
    ...
    filters_key = hashlib.md5(
        json.dumps(body.filters, default=str).encode(),
        usedforsecurity=False,
    ).hexdigest()
    cache_key = hashlib.md5(
        f"v2f:{body.sort_by}:{body.sort_order}:{body.limit}:{body.min_score}:{body.market}:{filters_key}:{body.logic}:{body.include_details}:{body.timeframe}:{body.risk_tolerance}:{body.signal_filter}".encode(),
        usedforsecurity=False,
    ).hexdigest()
    cached = _cache_get(cache_key)
    if cached:
...
async def compare_symbols(
    ...
    try:
        ...
        return ApiResponse(success=False, data={"items": []}, error={"message": _safe_error_message(exc)})
...
async def sector_analysis(
    ...
    try:
        ...
        return ApiResponse(success=False, data={"sectors": []}, error={"message": _safe_error_message(exc)})
