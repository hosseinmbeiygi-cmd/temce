...
async def get_historical_daily(
    ...
    try:
        ...
        for hist_model in (GoldCoinHistoryModel, GoldCurrencyProDailyHistoryModel):
            ...
            g_total = (
                await session.execute(sa_select(sa_func.count()).select_from(hist_model).where(*g_conditions))
            ).scalar() or 0
            if g_total == 0:
                continue
            g_rows = (
...
async def list_sections(
    ...
    Uses three batched queries (pg_stat counts, DISTINCT ON sync log, UNION ALL
    max-date) instead of one query per section — keeps the endpoint fast and
    avoids concurrent use of the single request-scoped AsyncSession.
    """
    from brsapi.models.base import SyncLogModel

    # 1) Row estimates from PostgreSQL statistics (fast, may be slightly stale)
    ...
    # 2) Last successful sync per endpoint in one DISTINCT ON query
    endpoint_paths = sorted({cfg["endpoint"].path for cfg in SECTIONS.values()})
    last_sync_map: dict[str, Any] = {}
    try:
        sync_stmt = (
            select(
                SyncLogModel.endpoint,
                SyncLogModel.status,
                SyncLogModel.items_count,
                SyncLogModel.duration_ms,
                SyncLogModel.completed_at,
                SyncLogModel.error_message,
            )
            .where(SyncLogModel.endpoint.in_(endpoint_paths), SyncLogModel.status == "success")
            .distinct(SyncLogModel.endpoint)
            .order_by(SyncLogModel.endpoint, SyncLogModel.completed_at.desc())
        )
        for row in (await session.execute(sync_stmt)).all():
            last_sync_map[row.endpoint] = row
    except Exception:
        logger.warning("brsapi sections: batched last_sync query failed", exc_info=True)

    # 3) Last data date — one small MAX() query per table, sequentially.
    #    A single multi-table UNION statement locks all tables at once and
    #    deadlocks with the TRUNCATE-and-rewrite sync jobs; per-table queries
    #    hold a lock on one (indexed) table for milliseconds only.
    #    Table/column names come from the SECTIONS registry, not user input.
    max_date_map: dict[str, str | None] = {}
    seen_pairs: dict[tuple[str, str], str] = {}
    for sid in SECTION_DATE_COLUMNS:
        col = SECTION_DATE_COLUMNS[sid]
        table = SECTIONS[sid]["model"].__tablename__
        pair = (table, col)
        sid_for_pair = seen_pairs.setdefault(pair, sid)
        if sid_for_pair != sid:
            continue
        try:
            val = (
                await session.execute(
                    text(f"SELECT MAX({col})::text FROM {table}")
                )
            ).scalar()
        except Exception:
            logger.warning("brsapi sections: max-date query failed for %s", table, exc_info=True)
            val = None
        max_date_map[sid_for_pair] = str(val) if val else None
    # share the value across sections pointing at the same (table, column)
    for sid, col in SECTION_DATE_COLUMNS.items():
        table = SECTIONS[sid]["model"].__tablename__
        max_date_map[sid] = max_date_map.get(seen_pairs[(table, col)])

    results: list[dict[str, Any]] = []
    for section_id, cfg in SECTIONS.items():
        table_name = cfg["model"].__tablename__

        count = pg_stat_counts.get(table_name)
        if count is None:
            try:
                cnt_result = await session.execute(select(sa_func.count()).select_from(cfg["model"]))
                count = cnt_result.scalar() or 0
            except Exception:
                count = 0

        row = last_sync_map.get(cfg["endpoint"].path)
        results.append({
            "id": section_id,
            "name": cfg["name"],
            "name_en": cfg["name_en"],
            ...
            "record_count": count,
            "last_data_date": max_date_map.get(section_id),
            "last_sync": {
                "status": row.status,
...
async def sync_nav_all(
    ...
    Discovers fund symbols dynamically from the database (sector containing
    ``صندوق``/``fund``/``ETF``) plus the hardcoded fallback list. Fetches NAV
    data from BrsApi for each, respecting rate limits (6 req/min).
    """
    ...
    # Delegate to the service which dynamically discovers fund symbols,
    # skips today's already-synced NAVs, and respects rate limits.
    report = await sync_svc.sync_nav_all(session)
