...
@router.get(
    ...
    description="Get overall market summary including indices and sector performance",
)
...
async def market_watch(service: MarketService = Depends(get_market_service)) -> ApiResponse[PaginatedResult[Any]]:
    ...
    if not result.success:
        return ApiResponse[PaginatedResult[Any]](
            success=False, data=PaginatedResult(items=[], total=0, page=1, page_size=50, total_pages=1)
        )
...
async def market_indicator(
    ...
    indicator: str = Query(
        ..., description="Indicator: sma, ema, rsi, macd, bollinger, stochastic, atr, obv, williams_r, ichimoku"
    ),
...
@router.get(
    "/enriched-heatmap",
    summary="Enriched market heatmap",
    description="Symbol heatmap data enriched with price thresholds, free float, sector info",
)
...
@router.get(
    "/treemap", summary="Market treemap", description="Hierarchical treemap data grouped by sector for visualization"
)
