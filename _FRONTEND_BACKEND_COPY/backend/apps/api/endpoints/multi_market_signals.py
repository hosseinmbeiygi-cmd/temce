...
# Pipeline-wide minimum confidence. Signals below this are dropped at
# generation time (not just at display), so the cache, the signal_accuracy
# table, and downstream consumers all see the same filtered set.
#
# Set via env ``SIGNAL_PIPELINE_MIN_CONFIDENCE`` (default 0.40).
# Rationale (docs/accuracy-investigation-2026-08.md + dry-run):
#   threshold=0.0  → 29.4% acc, 100% coverage  (baseline)
#   threshold=0.35 → 46.2% acc, 37%  coverage  (+16.9pp)
#   threshold=0.40 → 46.9% acc, 32%  coverage  (+17.5pp)  ← chosen sweet spot
#   threshold=0.45 → 60.8% acc,  4%  coverage  (too aggressive)
#   threshold=0.55 → 92.0% acc,  1%  coverage  (almost no signals)
import os as _os

_PIPELINE_MIN_CONFIDENCE = float(_os.environ.get("SIGNAL_PIPELINE_MIN_CONFIDENCE", "0.40"))
...
@router.get(
    "",
    summary="Multi-market signals",
    description="Aggregated buy/sell signals from all markets with ML voting & calibrated confidence",
)
async def get_multi_market_signals(
    ...
    sort_by: str = Query(
        "boosted_score", description="boosted_score / confidence / rule_score / strength / change_pct"
    ),
    ...
    try:
        ...
        summary["avg_confidence"] = (
            round(sum((s.get("confidence") or 0) for s in signals_list) / max(len(signals_list), 1), 3)
            if signals_list
            else 0.0
        )
        ...
        # If the rolling-accuracy breaker is tripped, downgrade the result
        # to an empty signal list with a banner. We do NOT mutate the
        # cache — the breaker is a per-response read, not a write. This
        # way the cache stays useful for the next request once accuracy
        # recovers; the breaker simply suppresses the user-facing feed.
        try:
            from services.signal_circuit_breaker import evaluate_cached

            _breaker = await evaluate_cached()
        except Exception:
            logger.debug("circuit-breaker evaluation failed; skipping", exc_info=True)
            _breaker = None
        ...
        if _breaker is not None and _breaker.tripped:
            ...
            data["circuit_breaker"] = {
                ...
                "accuracy_pct": _breaker.accuracy_pct,
                "sample_size": _breaker.sample_size,
                "window_days": _breaker.window_days,
                "threshold_pct": _breaker.threshold_pct,
            }
        elif _breaker is not None:
            data["circuit_breaker"] = {
                "tripped": False,
                "reason": _breaker.reason,
                "accuracy_pct": _breaker.accuracy_pct,
...
@router.get(
    "/retrain",
    summary="Auto-retrain status",
    description="Trigger auto-retrain for markets with low accuracy. Note: triggers actual model retraining which may take 30+ seconds.",
)
async def trigger_retrain(
    market: str = Query(
        "all", description="stock / gold / currency / crypto / option / commodity / ime / all — retrain specific market"
    ),
...
@router.get(
    "/summary",
    summary="Signal summary by market",
    description="Quick summary of signal counts per market with confidence & cross-market insights",
)
