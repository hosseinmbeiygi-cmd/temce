...
def safe_error_message(exc: Exception) -> str:
    """Sanitized error message for API responses (no stack leak)."""
    msg = str(exc).strip()
    if not msg:
        return type(exc).__name__
    # Truncate to avoid leaking huge payloads
    return msg[:500]
...
async def _load_merged_funds(session: AsyncSession) -> dict[str, dict[str, Any]]:
    ...
    #    Q1 P0: try materialized view mv_latest_brsapi_symbol_snapshot first
    #    (CONCURRENT refreshed every 60s) — ~10x faster than DISTINCT ON on 836K rows.
    try:
        # Attempt MV fast path
        mv_ok = False
        try:
            mv_result = await session.execute(
                text(
                    "SELECT * FROM mv_latest_brsapi_symbol_snapshot "
                    "WHERE sector = :sector OR symbol = ANY(:etf_list)"
                ),
                {"sector": "صندوق سرمایه‌گذاری قابل معامله", "etf_list": list(BRSAPI_ETF_SYMBOLS)},
            )
            mv_rows = mv_result.fetchall()
            if mv_rows:
                # Map MV rows to dict via column names (fallback to ORM if needed)
                cols = mv_result.keys()
                for row in mv_rows:
                    d = dict(zip(cols, row, strict=False))
                    sym = d.get("symbol")
                    if sym and sym not in merged:
                        # Reuse _snapshot_to_dict shape by constructing a minimal object
                        merged[sym] = {
                            "symbol": d.get("symbol"),
                            "name": d.get("name") or sym,
                            "isin": d.get("isin") or "",
                            "fund_type": _infer_fund_type(d.get("name"), sym),
                            "market": "tse",
                            "nav": float(d.get("price_close") or d.get("price_last") or 0),
                            "nav_change": float(d.get("price_close_change") or d.get("price_last_change") or 0),
                            "nav_change_pct": round(float(d.get("price_close_change_pct") or d.get("price_last_change_pct") or 0), 2),
                            "price_last": float(d.get("price_last") or 0),
                            "price_close": float(d.get("price_close") or 0),
                            "price_yesterday": float(d.get("price_yesterday") or 0),
                            "price_max": float(d.get("price_max") or 0),
                            "price_min": float(d.get("price_min") or 0),
                            "trade_volume": int(d.get("trade_volume") or 0),
                            "trade_value": float(d.get("trade_value") or 0),
                            ...
                            "buy_real_volume": int(d.get("buy_real_volume") or 0),
                            "buy_legal_volume": int(d.get("buy_legal_volume") or 0),
                            "sell_real_volume": int(d.get("sell_real_volume") or 0),
                            "sell_legal_volume": int(d.get("sell_legal_volume") or 0),
                            "time": str(d.get("fetched_at") or d.get("updated_at") or ""),
                            "data_source": "tsetmc_mv",
                            "snapshot_date": str(d.get("fetched_at") or "")[:10],
                            "updated_at": str(d.get("fetched_at") or ""),
                        }
                mv_ok = len(mv_rows) > 0
        except Exception as mv_exc:
            if "does not exist" not in str(mv_exc) and "not exist" not in str(mv_exc).lower():
                logger.debug("MV fast path failed, falling back: %s", mv_exc)
            mv_ok = False
...
@router.post("/admin/refresh-views", summary="Refresh materialized views (admin)")
async def refresh_materialized_views(session: AsyncSession = Depends(get_db_session)):
    """Manually refresh mv_latest_* (CONCURRENT). Call after bulk ingest or via cron."""
    from services.materialized_view_service import refresh_latest_views

    result = await refresh_latest_views(session)
    await _invalidate_fund_cache()
...
@router.get("/{symbol}/score", summary="امتیاز + سیگنال ۵ سطحی + بردار دلایل (FundScoringEngine)")
async def get_fund_score(
    symbol: str,
    session: AsyncSession = Depends(get_db_session),
) -> dict[str, Any]:
    """خروجی موتور امتیازدهی قطعی FundScoringEngine + JSON ساختاریافته برای فرانت/Alert/Telegram.

    ضد هالوسیناسیون: این endpoint تنها جایی است که امتیاز تولید می‌شود.
    لایه LLM فقط تفسیر می‌کند.
    """
    funds = await _get_cached_funds(session)
    fund = funds.get(symbol)
    if not fund:
    ...
    from services.fund_scoring import FundMetrics, compute_fund_score
    ...
    metrics = FundMetrics(
        ...
        price_last=float(f.get("price_last") or 0),
        nav=float(f.get("nav") or 0),
        nav_source=f.get("nav_source", ""),
        nav_date=f.get("nav_date", ""),
        nav_change_pct=float(f.get("nav_change_pct") or 0),
        trade_volume=float(f.get("trade_volume") or 0),
        trade_value=float(f.get("trade_value") or 0),
        shares_count=float(f.get("shares_count") or 0),
        market_value=float(f.get("market_value") or 0),
        buy_real_volume=float(f.get("buy_real_volume") or 0),
        sell_real_volume=float(f.get("sell_real_volume") or 0),
    )
    result = compute_fund_score(metrics)
    return {"symbol": symbol, "found": True, **result.to_dict()}
