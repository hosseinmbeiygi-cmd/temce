...
async def _db_list_models(session: AsyncSession) -> list[dict] | None:
    ...
    all_versions = (
        (await session.execute(select(MlModelVersionModel).order_by(MlModelVersionModel.version))).scalars().all()
    )
...
async def _db_list_runs(session: AsyncSession, limit: int = 200) -> list[dict] | None:
    ...
    rows = (
        (await session.execute(select(MlTrainingRunModel).order_by(MlTrainingRunModel.created_at.desc()).limit(limit)))
        .scalars()
        .all()
    )
    if not rows:
...
async def predict_all(
    ...
    if instruments_result.success and instruments_result.value:
        db_symbols = [
            inst.symbol for inst in instruments_result.value.items if inst.symbol and inst.status.value == "active"
        ]
    else:
        db_symbols = []
