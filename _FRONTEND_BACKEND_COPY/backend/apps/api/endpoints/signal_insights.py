...
@router.get(
    "/accuracy",
    summary="Signal accuracy by market",
...
async def get_signal_accuracy(
    ...
    try:
        ...
        result = await tracker.get_accuracy_by_market(market=market_filter, days=days, page=page, page_size=page_size)
...
@router.get(
    "/accuracy/symbol/{symbol}",
    summary="Signal accuracy by symbol",
...
@router.get(
    "/ensemble", summary="Ensemble optimization", description="Get optimal ensemble configuration for each market"
)
    ...
    try:
        ...
        if market == "all":
            ...
            return ApiResponse(
                success=True,
                data={
                    "markets": {m: e.to_dict() for m, e in result.value.items()},
                    "generated_at": datetime.now().isoformat(),
                },
            )
        else:
...
@router.get(
    "/confidence", summary="Confidence scoring breakdown", description="Get confidence score breakdown for a signal"
)
...
@router.post(
    "/retrain", summary="Trigger model retrain", description="Trigger auto-retrain for a market or all markets"
)
...
@router.get(
    "/multi-timeframe",
    summary="Multi-timeframe confirmation",
...
@router.get(
    "/risk-filter",
    summary="Risk-adjusted signal filter",
