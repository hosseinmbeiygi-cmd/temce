...
def _build_stock_money_flow(snapshots: list[dict]) -> dict[str, list[dict[str, Any]]]:
    ...
    outflow = [
        {"symbol": s["symbol"], "name": s["name"], "value": abs(s["value"])} for s in stock_flow if s["value"] < 0
    ][:15]
...
def _empty_response(market: str) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": {
            ...
            "index_change": 0,
            "index_change_pct": 0,
            "index_equal_weight": 0,
            "index_equal_weight_change": 0,
            "index_equal_weight_change_pct": 0,
            "total_trade_value": 0,
            "total_volume": 0,
            "net_real_money_flow": 0,
            "buy_queue_value": 0,
            "sell_queue_value": 0,
            "equity_fund_value": 0,
            "equity_fund_net": 0,
