...
async def liquidity_flow(
    ...
    top_inflow = [
        {"sector": s, "inflow": d["inflow"], "symbols": d["symbols"]} for s, d in sorted_sectors[:3] if d["inflow"] > 0
    ]
    top_outflow = [
        {"sector": s, "outflow": d["outflow"], "symbols": d["symbols"]}
        for s, d in sorted(sector_flow.items(), key=lambda x: x[1]["outflow"], reverse=True)[:3]
        if d["outflow"] > 0
    ]
