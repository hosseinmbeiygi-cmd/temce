    ...
    try:
        ...
        if registry.scheduler:
            for job_info in jobs:
                ...
                if aps_job:
                    job_info["next_run_time"] = aps_job.next_run_time.isoformat() if aps_job.next_run_time else None
                else:
        ...
        return ApiResponse(
            success=False, error={"message": str(exc)}, data={"jobs": [], "total": 0, "enabled": 0, "disabled": 0}
        )
