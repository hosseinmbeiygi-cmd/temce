...
async def create_saved_filter(
    ...
    async for session in get_session():
        ...
        count_result = await session.execute(select(func.count()).where(SavedFilter.user_id == user_id))
...
async def update_saved_filter(
    ...
    async for session in get_session():
        ...
        if update_data:
            ...
            await session.execute(update(SavedFilter).where(SavedFilter.id == filter_id).values(**update_data))
            await session.commit()
