...
async def analyze_strategy(body: AnalyzeRequest) -> ApiResponse[dict[str, Any]]:
    ...
    try:
        ...
        if "call" in body.strategy or body.strategy in (
            "straddle",
            "strangle",
            "long_gut",
            "conversion",
        ...
        if "put" in body.strategy or body.strategy in (
            "straddle",
            "strangle",
            "long_gut",
            "conversion",
        ...
        if body.strategy == "covered_call":
            ...
            kwargs.update(
                {
                    ...
                    "put_premium": body.put_premium,
                    "call_strike": body.call_strike,
                    "call_premium": body.call_premium,
                }
            )
            ...
            kwargs.update(
                {
                    "lower_strike": body.strike,
                    "upper_strike": body.call_strike,
                    "lower_premium": body.call_premium,
                    "upper_premium": body.put_premium,
                }
            )
        elif body.strategy == "bull_put_spread" or body.strategy == "bear_put_spread":
            kwargs.update(
                {
                    "higher_strike": body.strike,
                    "lower_strike": body.put_strike,
                    "higher_premium": body.put_premium,
                    "lower_premium": body.call_premium,
                }
            )
...
async def calculate_greeks(
    S: float, K: float, T: float, r: float = 0.15, sigma: float = 0.35, option_type: str = "call"
...
async def pricing(S: float, K: float, T: float, r: float = 0.15, sigma: float = 0.35) -> ApiResponse[dict[str, Any]]:
...
async def check_parity(
    call_price: float, put_price: float, stock_price: float, strike: float, time_to_expiry: float = 0.25
) -> ApiResponse[dict[str, Any]]:
...
async def scan_arbitrage(options_chain: list[dict[str, Any]], stock_price: float) -> ApiResponse[list[dict[str, Any]]]:
    detector = get_arbitrage_detector()
...
async def analyze_volatility(
    options_chain: list[dict[str, Any]], stock_price: float, historical_vol: float = 0.35
) -> ApiResponse[dict[str, Any]]:
...
async def analyze_portfolio(positions: list[dict[str, Any]], stock_price: float) -> ApiResponse[dict[str, Any]]:
...
async def calculate_costs(body: CostCalcRequest) -> ApiResponse[dict[str, Any]]:
    ...
    result = engine.calculate_realistic_costs(
        body.entry_price, body.exit_price, body.quantity, body.is_option, body.is_short
    )
    return ApiResponse(success=True, data=result)
...
async def position_sizing(body: PositionSizingRequest) -> ApiResponse[dict[str, Any]]:
    ...
    result = engine.calculate_position_size(
        body.capital, body.risk_per_trade_pct, body.max_loss_per_contract, body.option_type
    )
    return ApiResponse(success=True, data=result)
...
async def live_options_chain(underlying: str, limit: int = 50) -> ApiResponse[dict[str, Any]]:
    ...
    chain_data = [
        {"type": c["type"], "strike": c["strike"], "price": c["price"], "volume": c["volume"], "open_interest": c["oi"]}
        for c in calls + puts
    ]
