...
# Required auth dependency â€” token is mandatory for sensitive endpoints
...
_ENDPOINTS = [
    ...
    {"path": "/forecast", "tag": "Forecast", "description": "Multi-horizon market forecasts"},
    ...
    },
    {
        "path": "/screener110",
        "tag": "Screener110",
        "description": "110-column CANSLIM screener â€” populate profiles & run model",
    },
    {
    ...
    },
    {
        ...
        "description": "Unified conversational assistant â€” execute commands across all features",
    },
...
class Router:
    def setup(self) -> APIRouter:
        ...
        # â”€â”€ Lazy imports: all endpoint routers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        from apps.api.endpoints.gold import router as gold_router
        ...
        from apps.api.endpoints.ime import router as ime_router
        ...
        from apps.api.endpoints.monitoring import router as monitoring_router
        ...
        from apps.api.endpoints.proposals import router as proposals_router
        ...
        # Sandooghyar â€” Ù¾Ø§ÛŒØ´ Ù‡ÙˆØ´Ù…Ù†Ø¯ ØµÙ†Ø¯ÙˆÙ‚â€ŒÙ‡Ø§ÛŒ Ø³Ø±Ù…Ø§ÛŒÙ‡â€ŒÚ¯Ø°Ø§Ø±ÛŒ Ø§ÛŒØ±Ø§Ù†
        from apps.funds.router import funds_router as sandooghyar_router

        # Health and auth routers are intentionally unprotected
        router.include_router(health_router, prefix="/health", tags=["Health"])
        router.include_router(auth_router, prefix="/auth", tags=["Authentication"])
        # Sandooghyar funds platform â€” peer-group percentile, bubble monitor, screening
        # Mounted at /sandooghyar (main /funds belongs to legacy funds_router)
        router.include_router(
            sandooghyar_router,
            prefix="/sandooghyar",
            tags=["Sandooghyar Funds"],
            dependencies=_optional_auth,
        )
        # Gold module (Iranian gold market) â€” optional auth like other data routers
        router.include_router(
            gold_router,
            prefix="/gold",
            tags=["Gold Module"],
        ...
        # All data routers have optional auth â€” token is checked if provided, but not required
        ...
        # DB-free static symbol catalog & search â€” works without PostgreSQL.
        router.include_router(
        ...
        router.include_router(ime_router, prefix="/ime", tags=["IME"], dependencies=_optional_auth)
        router.include_router(chat_router, prefix="/chat", tags=["Chat"], dependencies=_optional_auth)
        ...
        router.include_router(jobs_router, prefix="/jobs", tags=["Jobs"], dependencies=_require_admin)
        ...
        # System administration (Ø³Ù†Ø¯ Â§15.3 + Â§19.3): kill switch + RBAC endpoints
        router.include_router(
            system_router,
            prefix="",  # the router already declares its own /api/v1/system prefix
            tags=["System"],
        )
        router.include_router(
            analysis_router,
        ...
        router.include_router(alpha_router, prefix="/alpha", tags=["Alpha"], dependencies=_optional_auth)
        router.include_router(risk_router, prefix="/risk", tags=["Risk"], dependencies=_optional_auth)
        router.include_router(
            funds_router,
            prefix="/funds",
            tags=["Funds"],
        ...
        # Options market endpoints (strategies, chain, live, analytics)
        router.include_router(
            options_router,
            prefix="/options",
            tags=["Options"],
        ...
        # Queue Analysis (Phase 1 â€” real queue detection from TSETMC data)
        router.include_router(
