...
# â”€â”€ Notification callback for rate limit alerts â”€â”€
...
async def _rate_limit_notify(message: str) -> None:
    ...
    try:
        ...
        result = await sender.send(f"âš ï¸ <b>BrsApi Rate Limit</b>\n\n{message}")
...
# â”€â”€ Orchestrator hourly cron state â”€â”€
...
async def _save_cron_state_to_store() -> None:
    ...
    ``history`` is a ``deque`` â€” converted to a list so it round-trips
...
async def _send_cron_alert(title: str, message: str, icon: str = "ðŸ”´") -> None:
...
async def _check_cron_alerts() -> None:
    ...
    # â•â•â• 1. Three consecutive failures â•â•â•
    ...
    if all_failed:
        ...
        if (now_ts - _alert_state["consecutive_failures"]) >= ALERT_COOLDOWN_S:
            ...
            await _send_cron_alert(
                "3 Ø´Ú©Ø³Øª Ù…ØªÙˆØ§Ù„ÛŒ Ú©Ø±ÙˆÙ†",
                f"Ú©Ø±ÙˆÙ† orchestrator Ø¯Ø± Û³ Ø§Ø¬Ø±Ø§ÛŒ Ù…ØªÙˆØ§Ù„ÛŒ Ø´Ú©Ø³Øª Ø®ÙˆØ±Ø¯Ù‡ Ø§Ø³Øª.\n"
                f"Ø¢Ø®Ø±ÛŒÙ† Ø®Ø·Ø§: {errors[-1]}\n"
                f"Ø§Ø¬Ø±Ø§Ù‡Ø§ÛŒ: {last_three[0]['run']}ØŒ {last_three[1]['run']}ØŒ {last_three[2]['run']}\n"
                f"Ø²Ù…Ø§Ù† Ø¢Ø®Ø±ÛŒÙ† Ø´Ú©Ø³Øª: {last_three[-1].get('timestamp', '?')}",
            )
            _alert_state["consecutive_failures"] = now_ts
        return  # Don't send multiple alert types in the same check

    # â”€â”€ Recovery: cron succeeded after a failure streak â”€â”€
    if latest_ok and _alert_state["was_in_failure_streak"] > 0.0:
        ...
        await _send_cron_alert(
            ...
            f"Ú©Ø±ÙˆÙ† orchestrator Ù¾Ø³ Ø§Ø² Ø´Ú©Ø³Øªâ€ŒÙ‡Ø§ÛŒ Ù…ØªÙˆØ§Ù„ÛŒ Ø¨Ø§ Ù…ÙˆÙÙ‚ÛŒØª Ø§Ø¬Ø±Ø§ Ø´Ø¯.\n"
            f"Ù…Ø¯Øª Ø²Ù…Ø§Ù† Ø§Ø®ØªÙ„Ø§Ù„: {streak_duration / 60:.0f} Ø¯Ù‚ÛŒÙ‚Ù‡\n"
            f"Ø´Ù…Ø§Ø±Ù‡ Ø§Ø¬Ø±Ø§ÛŒ Ù…ÙˆÙÙ‚: {latest.get('run', '?')}\n"
            f"Ø³ÛŒÚ¯Ù†Ø§Ù„â€ŒÙ‡Ø§ÛŒ ØªÙˆÙ„ÛŒØ¯Ø´Ø¯Ù‡: {latest.get('signals', 0)}\n"
            f"Ø²Ù…Ø§Ù† Ø¨Ø§Ø²ÛŒØ§Ø¨ÛŒ: {latest.get('timestamp', '?')}",
            icon="ðŸŸ¢",
        )
        _alert_state["was_in_failure_streak"] = 0.0
        _alert_state["consecutive_failures"] = 0.0

    # â•â•â• 2. Overall accuracy dropped below 50% â•â•â•
    if latest_ok:
        ...
        if acc < 50:
            ...
            if (now_ts - _alert_state["accuracy_drop"]) >= ALERT_COOLDOWN_S:
                await _send_cron_alert(
                    "Ú©Ø§Ù‡Ø´ Ø¯Ù‚Øª Ú©Ù„ Ø¨Ù‡ Ø²ÛŒØ± ÛµÛ°Ùª",
                    f"Ø¯Ù‚Øª Ú©Ù„ Ø³ÛŒÚ¯Ù†Ø§Ù„â€ŒÙ‡Ø§ Ø¨Ù‡ {acc:.1f}Ùª Ú©Ø§Ù‡Ø´ ÛŒØ§ÙØªÙ‡ Ø§Ø³Øª (Ø²ÛŒØ± Ø¢Ø³ØªØ§Ù†Ù‡ ÛµÛ°Ùª).\n"
                    f"Ø³ÛŒÚ¯Ù†Ø§Ù„â€ŒÙ‡Ø§ÛŒ Ø¢Ø®Ø±ÛŒÙ† Ø§Ø¬Ø±Ø§: {latest.get('signals', 0)}\n"
                    f"Ø²Ù…Ø§Ù† Ø§Ø¬Ø±Ø§: {latest.get('timestamp', '?')}\n"
                    f"Ø¨Ø§Ø²Ø¢Ù…ÙˆØ²ÛŒ Ø®ÙˆØ¯Ú©Ø§Ø± Ø¯Ø± Ø§Ø¬Ø±Ø§ÛŒ Ø¨Ø¹Ø¯ÛŒ ÙØ¹Ø§Ù„ Ø®ÙˆØ§Ù‡Ø¯ Ø´Ø¯.",
                )
                _alert_state["accuracy_drop"] = now_ts

        # â”€â”€ Recovery: accuracy climbed back above 50% â”€â”€
        elif acc >= 50 and _alert_state["was_accuracy_below_50"] > 0.0:
            await _send_cron_alert(
                "Ø¨Ø§Ø²Ú¯Ø´Øª Ø¯Ù‚Øª Ø¨Ù‡ Ø¨Ø§Ù„Ø§ÛŒ ÛµÛ°Ùª",
                f"Ø¯Ù‚Øª Ú©Ù„ Ø³ÛŒÚ¯Ù†Ø§Ù„â€ŒÙ‡Ø§ Ø¨Ù‡ {acc:.1f}Ùª Ø¨Ø§Ø²Ú¯Ø´ØªÙ‡ Ø§Ø³Øª (Ø¨Ø§Ù„Ø§ÛŒ Ø¢Ø³ØªØ§Ù†Ù‡ ÛµÛ°Ùª).\n"
                f"Ø´Ù…Ø§Ø±Ù‡ Ø§Ø¬Ø±Ø§: {latest.get('run', '?')}\n"
                f"Ø³ÛŒÚ¯Ù†Ø§Ù„â€ŒÙ‡Ø§ÛŒ ØªÙˆÙ„ÛŒØ¯Ø´Ø¯Ù‡: {latest.get('signals', 0)}\n"
                f"Ø²Ù…Ø§Ù† Ø¨Ø§Ø²ÛŒØ§Ø¨ÛŒ: {latest.get('timestamp', '?')}",
                icon="ðŸŸ¢",
            )
...
async def _orchestrator_hourly_cron() -> None:
    ...
    while True:
        try:
            ...
            logger.info("=" * 50)
            logger.info("HOURLY ORCHESTRATOR CRON â€” run #%d", _cron_state["run_count"] + 1)
            logger.info("=" * 50)
        ...
        logger.debug("Cron run took %.1fs â€” sleeping %.1fs until next tick", elapsed, sleep_seconds)
        await asyncio.sleep(sleep_seconds)


# â”€â”€ Startup background tasks â”€â”€

# Distributed-lock ownership registry for startup tasks (key â†’ owner token).
...
async def _with_startup_lock(key: str, task: str) -> bool:
    ...
    in-memory fallback â€” returns ``False`` when another process already owns
    ...
    if locker is None:
        logger.warning("Startup lock unavailable for %s â€” running without lock", task)
        return True
    ...
    if not acquired:
        logger.info("Startup task %s skipped â€” another process is already running it", task)
        return False
...
async def _decision_engine_startup_seed() -> None:
    ...
    try:
        ...
        logger.info("Decision Engine â€” auto-seeding architecture data on startup...")
        session_obtained = False
        async for session in get_session():
            session_obtained = True
            await _auto_seed(session)
            logger.info("Decision Engine â€” startup auto-seed complete")
            break
        if not session_obtained:
            logger.warning("Decision Engine â€” could not obtain DB session for auto-seed")
    except Exception:
        logger.exception("Decision Engine â€” startup auto-seed failed")
    finally:
    ...
    while True:
        try:
            ...
            async with async_session_factory() as session:
                ...
                logger.info(
                    "Fund sync cycle: %s â€” next sync in %s",
                    report.summary,
                ...
                if report.errors:
                    for err in report.errors[:5]:  # First 5 only
                        logger.warning("  Fund sync error: %s â€” %s", err["symbol"], err["error"])

        except Exception:
            logger.exception("Fund sync cron failed â€” retrying in 60s")
            await asyncio.sleep(60)
            continue

        # â”€â”€ Next interval â”€â”€
        if _is_market_open():
            ...
            logger.info("Market closed â€” next fund sync at market open (in %.0f min)", delay / 60)
...
async def _ml_model_preload() -> None:
    ...
    needed â€” ``preload`` is read-only and idempotent (each worker warms its
    own in-memory cache).
    """
    try:
        ...
        async for session in get_session():
            ...
            if not symbols:
                logger.info("ML model preload: no active symbols found â€” skipping")
                break
        ...
        if not session_obtained:
            logger.warning("ML model preload: could not obtain DB session â€” skipping")
    except Exception:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.warning(
                        "Startup news fetch failed (attempt %d/%d) â€” retrying in %ds",
                        attempt,
                        MAX_RETRIES,
                        delay,
                    )
                    ...
                    logger.exception("Startup news fetch failed after %d attempts â€” giving up", MAX_RETRIES)
    finally:
...
async def _brsapi_startup_sync() -> None:
    ...
    workers â€” or the API alongside the SchedulerApp â€” never run the full
    ...
    try:
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                logger.info("=" * 60)
                logger.info(
                    "BrsApi STARTUP SYNC â€” attempt %d/%d â€” fetching ALL endpoints in order", attempt, MAX_RETRIES
                )
                logger.info("=" * 60)
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.warning(
                        "BrsApi startup sync failed (attempt %d/%d) â€” retrying in %ds",
                        attempt,
                        MAX_RETRIES,
                        delay,
                    )
                    ...
                    logger.exception("BrsApi startup sync failed after %d attempts â€” giving up", MAX_RETRIES)
    finally:
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # â”€â”€ Start BrsApi usage recorder (daily usage report for admin) â”€â”€
    try:
        from brsapi.usage_recorder import get_usage_recorder

        get_usage_recorder().start()
        logger.info("BrsApi usage recorder started â€” flushing daily usage every 60s")
    except Exception:
        logger.warning("BrsApi usage recorder failed to start")

    # â”€â”€ Start accuracy outcome flusher (audit S2: no silent outcome loss) â”€â”€
    try:
        from services.accuracy_outcome_queue import get_accuracy_outcome_queue

        get_accuracy_outcome_queue().start()
        logger.info("Accuracy outcome queue flusher started â€” flushing every 30s")
    except Exception:
        logger.warning("Accuracy outcome flusher failed to start")

    # â”€â”€ Register rate limit notification callback â”€â”€
    try:
        from brsapi.rate_limiter import get_rate_limiter
    ...
    # â”€â”€ Start BrsApi scheduler (periodic sync jobs) â”€â”€
    try:
        ...
        logger.exception("BrsApi scheduler startup failed â€” periodic syncs disabled")

    # â”€â”€ Initial full sync (ALL endpoints, ordered) â”€â”€
    ...
    # â”€â”€ Start RealtimeService (WebSocket broadcasting) â”€â”€
    try:
        ...
        logger.exception("RealtimeService startup failed â€” WebSocket broadcasting disabled")

    # â”€â”€ Hourly orchestrator cron (signal generation + outcome tracking + auto-retrain) â”€â”€
    _track_background_task(_orchestrator_hourly_cron(), "orchestrator-hourly-cron")
    logger.info("Orchestrator hourly cron started â€” generating signals every 3600s")

    # â”€â”€ Pre-warm the multi-market signals cache so the first page load is
    #    instant instead of blocking on a ~60s pipeline rebuild â”€â”€
    try:
    ...
    # â”€â”€ Fund sync cron (every 15 min during market hours, daily at 2 AM) â”€â”€
    _track_background_task(_fund_sync_cron(), "fund-sync-cron")
    logger.info("Fund sync cron started â€” updating fund data every 15 min")

    # â”€â”€ Warm the ModelLoader LRU cache for active watchlist/screener symbols â”€â”€
    _track_background_task(_ml_model_preload(), "ml-model-preload")
    logger.info("ML model preload scheduled at startup â€” warming cache for active symbols")
...
def create_app() -> FastAPI:
    ...
    # RequestContextMiddleware LAST â†’ OUTERMOST layer so every downstream
    ...
    # â”€â”€ Orchestrator cron status endpoint â”€â”€
    ...
    # â”€â”€ Rate limit status endpoint â”€â”€
    ...
    # â”€â”€ Cache health / stats endpoint â”€â”€
    ...
    async def prometheus_metrics():
        ...
        return PlainTextResponse(get_prometheus_exporter().export_text())
