...
class SentimentAnalyzer:
    ...
    def analyze(self, text: str) -> dict[str, Any]:
        ...
        for word, weight in self.POSITIVE_WEIGHTS.items():
            ...
            if count > 0:
                ...
                negated_count = sum(
                    1
                    for _ in range(len(text_lower) - len(word) + 1)
                    if text_lower[_ : _ + len(word)] == word
                    and any(text_lower[max(0, _ - 15) : _].strip().endswith(nw) for nw in self.NEGATION_WORDS)
                )
    ...
    def analyze_market_sentiment(self, texts: list[str]) -> dict[str, Any]:
    ...
    def analyze_symbol_sentiment(self, symbol: str, news_list: list[dict[str, Any]]) -> dict[str, Any]:
