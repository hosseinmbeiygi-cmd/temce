...
class ChatEngine:
    ...
    async def process(
        ...
        # ═══ Override heuristic: if user mentions "صندوق" (fund), force fund_* intent ═══
        msg_lower = message.lower()
        if "صندوق" in msg_lower:
            if intent in ("analyze_symbol", "compare_symbols", "find_best"):
                if "مقایسه" in msg_lower or "بهترین" in msg_lower:
                    intent = "fund_compare"
                elif "حباب" in msg_lower or "آربیتراژ" in msg_lower:
                    intent = "fund_bubble"
                else:
                    intent = "fund_analyze"
        ...
        if not symbols:
            context_symbols = self._dialog.resolve_symbol(user_id, message, symbols)
            if context_symbols:
        ...
        # ═══ Level 9: Compound Splitter ═══
        # برای intentهای fund_* از compound عبور نکن (متن «X و Y» مقایسه است، نه ۲ سؤال)
        if intent not in ("fund_analyze", "fund_compare", "fund_bubble") and self._compound_splitter.is_compound(
            message
        ):
            parts = self._compound_splitter.split(message)
            if len(parts) > 1:
                response = await self._handle_compound(user_id, parts, intent, entities)
                self._dialog.add_turn(user_id, message, intent, entities, response, confidence)
                return response
        ...
        if "suggestions" not in response or not response["suggestions"]:
            response["suggestions"] = self._suggestions.get_suggestions(user_id, intent, entities, max_suggestions=4)
    ...
    async def _handle_compound(
        ...
        for part in parts[:3]:  # Max 3 sub-queries
            ...
            part_response = await self._route_intent(user_id, part_intent, part_entities, part)
        ...
        return {
            "text": combined,
            "type": "compound",
            "suggestions": self._suggestions.get_suggestions(user_id, primary_intent, entities, max_suggestions=3),
        }
    ...
    async def _route_intent(
        ...
        # ── Fund: scoring engine + explainer ──
        if intent == "fund_analyze":
            return await self._handle_fund_analyze(entities)
        if intent == "fund_compare":
            return await self._handle_fund_compare(entities)
        if intent == "fund_bubble":
            return await self._handle_fund_bubble()
    ...
    async def _handle_analyze(self, entities: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._stock_assistant:
            try:
                result = await self._stock_assistant.process_message(f"تحلیل {symbol}")
    ...
    async def _handle_dynamic_filter(self, entities: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._stock_assistant:
            filter_text = " ".join(f"{c['field'].upper()}{c['operator']}{c['value']}" for c in conditions)
            try:
                result = await self._stock_assistant.process_message(f"فیلتر {filter_text}")
                if result and result.get("type") != "error":
                    return {
                        "text": result.get("text", ""),
                        "type": "filter",
    ...
    async def _handle_compare(self, entities: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._stock_assistant:
            for sym in all_symbols[:5]:
                try:
                    result = await self._stock_assistant.process_message(f"تحلیل {sym}")
                    if result:
    ...
    # ════════════════════════════════════════════
    # Fund Handlers (FundScoringEngine + Explainer)
    # ════════════════════════════════════════════

    async def _handle_fund_analyze(self, entities: dict[str, Any]) -> dict[str, Any]:
        """تحلیل یک صندوق از طریق FundScoringEngine + لایه توضیح فارسی.

        ضد هالوسیناسیون: امتیاز توسط موتور قطعی پایتون ساخته می‌شود؛
        این handler فقط شکل گفت‌وگویی به آن می‌دهد.
        """
        raw_text = entities.get("raw_text", "")
        symbols = entities.get("symbols", [])

        # استخراج صریح نماد بعد از «صندوق X» (regex چون entity_extractor سهامی است)
        if not symbols:
            import re

            m = re.search(r"صندوق\s+([\u0600-\u06FF\w]+)", raw_text)
            if m:
                candidate = m.group(1)
                # اگر کلمه عمومی است (طلا/اهرمی/...) به جای نماد، skip
                if candidate not in (
                    "طلا",
                    "سهامی",
                    "اهرمی",
                    ...
                    fund = await self._get_fund_dict(candidate)
                    if fund:
                        symbols = [candidate]

        # fallback: از کلیدواژه گروه صندوق نمونه بگیر
        if not symbols:
            picked = await self._pick_fund_by_keyword(raw_text)
            if picked:
                symbols = [picked]
        ...
        if not symbols:
            return {
                "text": (
                    "🎯 **تحلیل صندوق**\n\n"
                    "لطفاً نام یا نوع صندوق را بگویید، مثلاً:\n"
                    "  • «تحلیل صندوق آگاس»\n"
                    "  • «صندوق طلا چطوره؟»\n"
                    "  • «سیگنال صندوق درآمد ثابت»\n"
                    "  • «بهترین صندوق اهرمی کدام است؟»"
                ),
                "type": "clarification",
                "suggestions": ["صندوق طلا", "صندوق اهرمی", "صندوق درآمد ثابت"],
            }

        symbol = symbols[0]
        try:
            from services.fund_explainer import explain_fund
            from services.fund_scoring import compute_fund_score

            fund = await self._get_fund_dict(symbol)
            if not fund:
                return {
                    "text": f"⚠️ صندوق «{symbol}» در دیتابیس یافت نشد. (نمادهای صندوق از فیپیران همگام می‌شوند.)",
                    "type": "error",
                    "suggestions": ["بهترین صندوق طلا", "لیست صندوق‌ها"],
                }

            metrics = self._fund_dict_to_metrics(fund)
            result = compute_fund_score(metrics)
            explained = explain_fund(result)
            return {
                **explained,
                "actions": [
                    {"type": "link", "label": f"📊 {symbol}", "url": f"/funds/{symbol}"},
                    {"type": "link", "label": "🫧 مانیتور حباب", "url": "/funds?tab=bubble"},
                ],
            }
        except Exception as e:
            logger.exception("Fund analyze failed: %s", e)
            return {
                "text": f"⚠️ خطا در تحلیل صندوق «{symbol}»: {e}",
                "type": "error",
            }

    async def _pick_fund_by_keyword(self, text: str) -> str | None:
        """از روی کلیدواژه (طلا/اهرمی/درآمد ثابت/...) اولین صندوق هم‌گروه فعال را برمی‌گرداند."""
        text_l = text.lower()
        # اولویت: کلمه دقیق‌تر
        kw_map = {
            "اهرمی": ["اهرمی", "اهرم"],
            "طلا": ["طلا", "سکه"],
            "درآمد ثابت": ["درآمد ثابت"],
            "مختلط": ["مختلط"],
            "شاخصی": ["شاخصی", "شاخص"],
            "بخشی": ["بخشی"],
            "سهامی": ["سهامی", "سهام"],
        ...
        gen = None
        try:
            from apps.api.dependencies import get_db_session  # type: ignore
            from apps.api.endpoints.funds import _get_cached_funds  # type: ignore

            gen = get_db_session()
            session = await gen.__anext__()
            funds_map = await _get_cached_funds(session)
            # فیلتر: فقط صندوق‌هایی که fund_type شامل target است
            matches = [f for f in funds_map.values() if target in (f.get("fund_type", "") or "")]
            if not matches:
                return None
            # فعال‌ترین بر اساس trade_value
            matches.sort(key=lambda f: float(f.get("trade_value") or 0), reverse=True)
            return matches[0].get("symbol")
        except Exception as e:
            logger.warning("pick_fund_by_keyword failed: %s", e)
            return None
        finally:
            if gen is not None:
                try:
    ...
    async def _handle_fund_compare(self, entities: dict[str, Any]) -> dict[str, Any]:
        """مقایسه شاخص‌محور چند صندوق با استفاده از موتور امتیازدهی."""
        symbols = entities.get("symbols", [])
        raw_text = entities.get("raw_text", "")

        # اگر entity extractor کلمه «صندوق» را بلعیده، خودمان با regex استخراج می‌کنیم
        if len(symbols) < 2:
            import re

            # الگو: کلمه‌ای که بعد از «صندوق» و قبل از «و/با/،» آمده
            pattern = r"صندوق\s+([\u0600-\u06FF\w]+)"
            candidates = re.findall(pattern, raw_text)
            # حذف تکراری و نگه‌داشتن حداکثر ۴
            seen = set()
            symbols = []
            for c in candidates:
        ...
        # اگر «بهترین X» بود، همه صندوق‌های هم‌گروه را بر اساس امتیاز rank کن و ۲ تای اول برگردان
        if "بهترین" in raw_text and len(symbols) < 2:
            return await self._handle_fund_best_in_group(raw_text)

        if len(symbols) < 2:
            return {
                "text": "⚖️ **مقایسه صندوق‌ها**\n\nبرای مقایسه لطفاً حداقل دو صندوق نام ببرید، مثلاً:\n  • «مقایسه صندوق آگاس و صندوق پالایش»\n  • «بهترین صندوق طلا» (بر اساس امتیاز مقایسه می‌شود)",
                "type": "clarification",
                "suggestions": ["مقایسه دو صندوق طلا", "مقایسه صندوق اهرمی و سهامی", "بهترین صندوق طلا"],
            }

        try:
            from services.fund_explainer import explain_compare_funds
            from services.fund_scoring import compute_fund_score

            results = []
            for sym in symbols[:4]:
                fund = await self._get_fund_dict(sym)
                if fund:
                    results.append(compute_fund_score(self._fund_dict_to_metrics(fund)))

            if len(results) < 2:
                return {
                    "text": "⚠️ صندوق‌های کافی برای مقایسه یافت نشد.",
                    "type": "error",
                }

            return explain_compare_funds(results)
        except Exception as e:
            logger.exception("Fund compare failed: %s", e)
            return {"text": f"⚠️ خطا در مقایسه: {e}", "type": "error"}

    async def _handle_fund_best_in_group(self, raw_text: str) -> dict[str, Any]:
        """وقتی کاربر «بهترین صندوق X» می‌پرسد، ۲ صندوق برتر گروه X را مقایسه می‌کند."""
        gen = None
        try:
            from apps.api.dependencies import get_db_session  # type: ignore
            from apps.api.endpoints.funds import _get_cached_funds  # type: ignore
            ...
            if not target:
                return {
                    "text": "برای «بهترین صندوق» لطفاً گروه را مشخص کنید: طلا، اهرمی، درآمد ثابت، سهامی.",
                    "type": "clarification",
                }
            ...
            if len(matches) < 2:
                return {
                    "text": f"⚠️ صندوق کافی در گروه «{target}» یافت نشد.",
                    "type": "error",
                }
            # امتیاز همه را محاسبه کن
            scored = [(f["symbol"], compute_fund_score(self._fund_dict_to_metrics(f))) for f in matches]
            # حذف آنهایی که bubble>3% (طبق override نباید «بهترین» باشند)
            scored_filtered = [s for s in scored if s[1].bubble.get("status") != "high_bubble"]
            if not scored_filtered:
                scored_filtered = scored
            scored_filtered.sort(key=lambda x: x[1].score, reverse=True)
            top = scored_filtered[:2]
            results = [s[1] for s in top]
            explained = explain_compare_funds(results)
            explained["text"] = (
                f"🏆 **بهترین صندوق‌های {target}** (بر اساس امتیاز کوانت، بدون حباب):\n\n" + explained["text"]
            )
            return explained
        except Exception as e:
            logger.exception("best in group failed: %s", e)
            return {"text": f"⚠️ خطا: {e}", "type": "error"}
        finally:
            if gen is not None:
                try:
                    await gen.aclose()
    ...
    async def _handle_fund_bubble(self) -> dict[str, Any]:
        """مانیتور حباب و آربیتراژ گروهی."""
        try:
            from services.fund_explainer import explain_bubble_monitor
            from services.fund_scoring import compute_fund_score
            from services.fund_service import FundService

            fs = FundService()
            all_funds_result = await fs.list_all(page=1, page_size=200)
            funds = []
            if all_funds_result.success and all_funds_result.value:
                # value: PaginatedResult[Fund] — items
                funds = getattr(all_funds_result.value, "items", []) or []
                ...
                try:
                    metrics_list.append(
                        (
                            f.symbol,
                            compute_fund_score(
                                self._fund_dict_to_metrics(
                                    {
                                        "symbol": f.symbol,
                                        "name": f.name,
                                        "fund_type": getattr(f, "fund_type", "سهامی"),
                                        "price_last": getattr(f, "price_last", 0),
                                        "nav": f.nav,
                                        "nav_source": getattr(f, "nav_source", ""),
                                    }
                                )
                            ),
                        )
            ...
            top_bubbles = sorted(
                [
                    {
                        "symbol": s,
                        "bubble": r.bubble.get("p_nav", "—"),
                        "score": r.score,
                        "signal": r.signal_label,
                    }
                    for s, r in metrics_list
                    if r.bubble.get("status") == "high_bubble"
                ],
                key=lambda x: x["score"],
            )[:5]

            # آربیتراژ ساده: گروه‌بندی بر اساس fund_type
            ...
            for s, r in metrics_list:
                bubble_val = 0.0
                try:
                    pnav_str = r.bubble.get("p_nav", "0")
                    bubble_val = float(pnav_str.replace("٪", "").replace("+", "").replace("—", "0") or 0)
                except Exception:
                    bubble_val = 0.0
                by_group[r.fund_type].append((s, bubble_val))
            ...
            for g, lst in by_group.items():
                ...
                spread = sorted_lst[-1][1] - sorted_lst[0][1]
                if spread >= 1.5:
                    arbitrage.append(
                        {
                            "group": g,
                            "rich": {"symbol": sorted_lst[-1][0], "bubble": sorted_lst[-1][1]},
                            "cheap": {"symbol": sorted_lst[0][0], "bubble": sorted_lst[0][1]},
                            "spread": spread,
                        }
                    )
            ...
            explained = explain_bubble_monitor(arbitrage, top_bubbles)
            return {**explained, "actions": [{"type": "link", "label": "🫧 مشاهده هیت‌مپ", "url": "/funds?tab=bubble"}]}
        except Exception as e:
            logger.exception("Fund bubble failed: %s", e)
            return {"text": f"⚠️ خطا در مانیتور حباب: {e}", "type": "error"}

    async def _get_fund_dict(self, symbol: str) -> dict[str, Any] | None:
        """دریافت دیکشنری صندوق از cache یا DB."""
        gen = None
        try:
            from apps.api.dependencies import get_db_session  # type: ignore
            from apps.api.endpoints.funds import _get_cached_funds  # type: ignore
            ...
            session = await gen.__anext__()
            funds_map = await _get_cached_funds(session)
            return funds_map.get(symbol)
        except Exception as e:
            logger.debug("_get_fund_dict(%s) failed: %s", symbol, e)
            return None
        finally:
            if gen is not None:
                try:
    ...
    def _fund_dict_to_metrics(self, f: dict[str, Any]) -> FundMetrics:
        """تبدیل dict صندوق (از API) به FundMetrics."""
        from services.fund_scoring import FundMetrics
        ...
        return FundMetrics(
            symbol=f.get("symbol", ""),
            name=f.get("name", ""),
            fund_type=f.get("fund_type", "سهامی"),
            market=f.get("market", "tse"),
            price_last=_f("price_last"),
            nav=_f("nav"),
            nav_source=f.get("nav_source", ""),
            nav_date=f.get("nav_date", ""),
            nav_change_pct=_f("nav_change_pct"),
            trade_volume=_f("trade_volume"),
            trade_value=_f("trade_value"),
            shares_count=_f("shares_count"),
            market_value=_f("market_value"),
            buy_real_volume=_f("buy_real_volume"),
            sell_real_volume=_f("sell_real_volume"),
        )
