...
INTENT_PATTERNS: dict[str, list[str]] = {
    ...
    "analyze_symbol": [
        ...
        r"(تحلیل|بررسی)\s*(کامل|فنی|تکنیکال|بنیادی)?\s*(\S+)(?!\s*صندوق)",
        r"(?<!\bصندوق\s)(?<!مقایسه\s)(\S+)\s*(چطوره|تحلیل|بررسی|وضعیت)",
    ],
    "analyze_all": [
    ...
    "fund_analyze": [
        r"تحلیل\s+صندوق",
        r"صندوق\s+[\u0600-\u06FF\w]+\s+(رو|را)?\s*(تحلیل|بررسی|چطور|چگونه)",
        r"صندوق\s+[\u0600-\u06FF\w]+\s+(می‌ارزد|می\u200cارزد|می\u200cارزه|بخرم|ارزش|چطوره)",
        r"سیگنال\s+صندوق",
        r"صندوق\s+(طلا|سهام|اهرم|درآمد|اختصاصی|بخشی|شاخصی|کالایی|مختلط)\s+(چطور|چگونه|بخرم|چی)",
    ],
    "fund_compare": [
        r"مقایسه\s+صندوق",
        r"دو\s+صندوق\s+.*مقایسه",
        r"بهترین\s+صندوق",
    ],
    "fund_bubble": [
        r"(حباب\s+صندوق|حباب\s+[\u0600-\u06FF]+)",
        r"(مانیتور\s+حباب)",
        r"(آربیتراژ|arbitrage)",
        r"(فرصت\s+.*آربیتراژ)",
    ],
}
...
DEFAULT_TRAINING_DATA: list[tuple[str, str]] = [
    ...
    # Fund analyze (موتور امتیازدهی ۵ لایه + سیگنال)
    ("تحلیل صندوق طلا", "fund_analyze"),
    ("صندوق آگاس رو تحلیل کن", "fund_analyze"),
    ("صندوق طلا چطوره", "fund_analyze"),
    ("آیا صندوق X بخرم", "fund_analyze"),
    ("سیگنال صندوق درآمد ثابت", "fund_analyze"),
    ("صندوق اهرمی بخرم", "fund_analyze"),
    ("ارزش خرید صندوق سهامی", "fund_analyze"),
    # Fund compare
    ("مقایسه صندوق طلا", "fund_compare"),
    ("دو صندوق سهامی مقایسه کن", "fund_compare"),
    ("بهترین صندوق اهرمی", "fund_compare"),
    # Fund bubble
    ("حباب صندوق چطوره", "fund_bubble"),
    ("مانیتور حباب", "fund_bubble"),
    ("آربیتراژ صندوق‌ها", "fund_bubble"),
    ("فرصت آربیتراژ", "fund_bubble"),
]
