...
class Personalizer:
    ...
    def update_from_intent(self, user_id: str, intent: str, entities: dict[str, Any], text: str) -> None:
        ...
        if conditions:
            filter_desc = " ".join(f"{c['field']}{c['operator']}{c['value']}" for c in conditions[:3])
            self._store.add_recent_filter(user_id, filter_desc)

        profile["activity_count"] += 1
        profile["last_seen"] = time.time()

    def boost_results(self, user_id: str, results: list[dict[str, Any]]) -> list[dict[str, Any]]:
