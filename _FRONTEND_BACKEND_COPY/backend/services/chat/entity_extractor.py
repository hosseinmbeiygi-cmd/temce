...
class EntityExtractor:
    ...
    def extract(self, text: str) -> dict[str, Any]:
        ...
        alert_match = re.search(
            r"هشدار\s*(\S+)\s*(rsi|price|قیمت|حجم|volume)\s*(above|below|بالاتر|پایین‌تر)\s*(\d+)", text, re.IGNORECASE
        )
        if alert_match:
        ...
        for pattern in compare_patterns:
            ...
            if match:
                ...
                for g in groups:
                    if (
                        g
                        and g not in ("و", "با", "مقایسه", "کدوم بهتره")
                        and g not in PERSIAN_STOPWORDS
                        and len(g) >= 2
                    ):
                        targets.append(g)
