    ...
    async def run_backtest(
        ...
        try:
            ...
            if strategy_cls is None:
                registry = get_strategy_registry()
                available = registry.list_names() or [
            ...
            try:
                ...
                trade_like = [
                    {"price": bar.get("close", 0), "volume": bar.get("volume", 0), "timestamp": bar.get("timestamp")}
                    for bar in data
                ]
                quality_report = validator.validate_trades(trade_like)
                if quality_report.quality_score < 0.5:
                    logger.warning(
                        "Low data quality for %s: score=%.2f, warnings=%s",
                        symbols[0],
                        quality_report.quality_score,
                        quality_report.warnings[:5],
                    )
                data_quality_report = {
            ...
            if (
                ...
                or slippage_bps is not None
                or stop_loss_pct is not None
            ...
            response = BacktestResultResponse(
                ...
                data_source_warning="SYNTHETIC DATA USED — Results are UNRELIABLE. No real historical data available for this symbol."
                if is_synthetic
                else None,
            )
