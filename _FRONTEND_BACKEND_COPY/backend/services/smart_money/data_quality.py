    ...
    def validate(
        ...
        if report.issues:
            ...
            if report.quality_score > 0.3:
                ...
                logger.info(
                    "Data quality partial for %s: score=%.2f issues=%s", symbol, report.quality_score, report.issues
                )
            else:
                logger.warning(
                    "Data quality too low for %s: score=%.2f issues=%s", symbol, report.quality_score, report.issues
                )
