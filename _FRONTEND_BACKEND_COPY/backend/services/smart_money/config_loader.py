...
def classify_phase_from_config(
    ...
    for rule in config.phase_rules:
        ...
        if rule.logic == "all":
            ...
            if all(_apply_condition(scores.get(c.field_name, 0.0), c) for c in rule.conditions):
                return rule.name
        elif rule.logic == "any":
            # OR logic: any condition matches
            if any(_apply_condition(scores.get(c.field_name, 0.0), c) for c in rule.conditions):
                return rule.name
