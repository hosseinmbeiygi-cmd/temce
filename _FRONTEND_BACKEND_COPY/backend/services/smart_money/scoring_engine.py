...
class ScoringEngine:
    ...
    def analyze(
        ...
        feature_vector = self._feature_store.compute_all_features(
            quote=quote,
            history=history,
            layer_results={"l1": l1, "l2": l2, "l3": l3, "l4": l4, "l5": l5, "l6": l6, "l7": l7, "l8": l8, "l9": l9},
    ...
    def _compute_ahm(self, history: list[dict[str, Any]], index_history: list[float] | None) -> float:
        ...
        for q in history[-ahm_window:]:
            try:
                ahm_vals.append(self.layer2.compute(q, history, index_return=index_ret).get("abs", 0.0))
            except Exception:
                ahm_vals.append(0.5)
    ...
    def _build_minimal_result(self, symbol: str, dq_report: DataQualityReport) -> dict[str, Any]:
        ...
        return {
            ...
            "scores": dict.fromkeys(
                ["accumulation", "absorption", "float_lock", "breakout_readiness", "buyer_power", "microstructure"], 0.0
            ),
