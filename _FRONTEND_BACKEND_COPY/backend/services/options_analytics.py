...
class ArbitrageDetector:
    ...
    def check_put_call_parity(
        self, call_price: float, put_price: float, stock_price: float, strike: float, time_to_expiry: float
    ) -> dict[str, Any]:
        ...
        for _expiry, opts in by_expiry.items():
            ...
            for call in calls:
                ...
                if matching_put and call.get("price", 0) > 0 and matching_put.get("price", 0) > 0:
                    ...
                    result = self.check_put_call_parity(call["price"], matching_put["price"], stock_price, strike, T)
...
class VolatilityAnalyzer:
    ...
    def analyze(
        self, options_chain: list[dict[str, Any]], stock_price: float, historical_vol: float = 0.35
    ) -> dict[str, Any]:
        ...
        for opt in options_chain:
            if opt.get("price", 0) > 0 and opt.get("strike", 0) > 0:
                ...
                iv = implied_volatility(opt["price"], stock_price, opt["strike"], T, 0.15, opt.get("type", "call"))
                iv_data.append(
                    {
                        "strike": opt["strike"],
...
class OptionsPortfolioAnalyzer:
    ...
    def analyze_portfolio(self, positions: list[dict[str, Any]], stock_price: float) -> dict[str, Any]:
