...
class InferenceService:
    ...
    async def predict_real(self, model_id: str, symbol: str) -> Result[dict[str, Any]]:
        ...
        try:
            ...
            prediction_val = (
                float(result.predictions[0]) if hasattr(result.predictions, "__len__") else float(result.predictions)
            )
        except Exception as e:
    ...
    async def _mock_predict(self, model_id: str, symbol: str) -> Result[dict[str, Any]]:
        ...
        return Result.fail(
            f"No trained model found for {model_id}/{symbol}. Please train a model first via /ml/train endpoint."
        )
    ...
    async def predict(self, model_id: str, features: dict[str, Any]) -> Result[PredictionResult]:
        ...
        return Result.fail(
            f"No trained model available for {model_id}. Please train a model first via /ml/train endpoint."
        )
