...
_CHAR_TRANSLATION = str.maketrans({"ي": "ی", "ك": "ک", "ۀ": "ه", "ة": "ه", "ؤ": "و"})
_DIACRITICS_RE = re.compile(r"[\u064b-\u065f\u0670\u06d6-\u06ed]")
_SPACE_RE = re.compile(r"\s+")


def normalize_financial_text(text: str) -> str:
    """Normalize Persian/Arabic financial text before intent and filter parsing.

    Users commonly mix Persian and Arabic characters, Persian digits, zero-width
    joiners, English operators and inconsistent whitespace in the same query.
    Keeping normalization central makes every intent and vocabulary entry work
    across those spelling variants without duplicating the whole lexicon.
    """
    normalized = (text or "").translate(_DIGIT_TRANSLATION).translate(_CHAR_TRANSLATION).lower()
...
def detect_intent(text: str) -> tuple[str | None, dict[str, str]]:
    ...
    """
    text_lower = normalize_financial_text(text)
    text_variants = (text.strip().lower(), text_lower)

    # Try patterns in order (more specific first)
    for intent, patterns in _INTENT_PATTERNS.items():
        for pattern in patterns:
            match = next((candidate for variant in text_variants if (candidate := re.search(pattern, variant))), None)
            if match:
                groups = match.groups()
                params: dict[str, str] = {}
                ...
                if intent == "analyze_symbol":
                    params["symbol"] = (
                        groups[0]
                        if groups[0] and groups[0] not in ("تحلیل", "بررسی", "چطوره", "وضعیت", "حالش")
                        else (groups[1] if len(groups) > 1 else "")
                    )
                    # Clean up
                    ...
                    if alert_match:
                        ...
                        params["condition"] = (
                            "above"
                            if "above" in alert_match.group(3).lower() or "بالاتر" in alert_match.group(3)
                            else "below"
                        )
...
_OR_PATTERN = re.compile(r"(?:\b(?:or|یا)\b|هر\s*کدام|یکی\s*از)", re.IGNORECASE)
...
# Compositional vocabulary for numeric financial questions.  Each alias can be
# combined with Persian/English operators, Persian/Arabic/English digits,
# magnitude units and range expressions.  This covers thousands of useful
# phrasings without maintaining thousands of fragile one-off sentences.
_FIELD_ALIASES: dict[str, tuple[str, ...]] = {
    "smc_score": ("smc", "امتیاز smc", "امتیاز پول هوشمند", "نمره پول هوشمند"),
    "liquidity_score": ("امتیاز نقدشوندگی", "نمره نقدشوندگی", "نقدشوندگی"),
    "power_score": ("امتیاز قدرت خرید", "قدرت خریدار", "قدرت خرید", "سرانه خرید"),
    "structure_score": ("امتیاز ساختار", "ساختار قیمت", "ساختار بازار"),
    "orderflow_score": ("امتیاز جریان سفارش", "جریان سفارش", "اوردر فلو", "order flow"),
    "trigger_score": ("امتیاز تریگر", "آمادگی شکست", "تریگر", "trigger"),
    "technical_score": ("امتیاز تکنیکال", "نمره تکنیکال", "technical score"),
    "momentum_score": ("امتیاز مومنتوم", "نمره شتاب", "momentum score", "مومنتوم"),
    "risk_score": ("امتیاز ریسک", "نمره ریسک", "risk score"),
    "composite_score": ("امتیاز ترکیبی", "نمره ترکیبی", "composite score"),
    "pattern_confidence": ("اطمینان الگو", "اعتبار الگو", "pattern confidence"),
    "rsi": ("شاخص قدرت نسبی", "آر اس آی", "rsi 14", "rsi"),
    "macd_histogram": ("هیستوگرام مکدی", "هیستوگرام macd", "مکدی", "macd"),
    "bb_pct": ("درصد باند بولینگر", "موقعیت بولینگر", "bollinger", "bb"),
    "atr_pct": ("دامنه واقعی", "نوسان atr", "atr"),
    "adx": ("قدرت روند adx", "شاخص adx", "adx"),
    "cci": ("شاخص کانال کالا", "سی سی آی", "cci"),
    "mfi": ("شاخص جریان نقدینگی", "ام اف آی", "mfi"),
    "williams_r": ("ویلیامز درصد آر", "ویلیامز", "williams %r", "williams"),
    "stochastic_k": ("استوکاستیک k", "استوکاستیک", "stochastic", "sto"),
    "pe_ratio": ("نسبت قیمت به سود", "پی بر ای", "پی ای", "p/e", "pe"),
    "eps": ("سود هر سهم", "ای پی اس", "eps"),
    "roe": ("بازده حقوق صاحبان سهام", "بازده حقوق سهامداران", "roe"),
    "debt_to_equity": ("نسبت بدهی به حقوق صاحبان سهام", "بدهی به حقوق", "debt to equity", "d/e"),
    "net_margin": ("حاشیه سود خالص", "نت مارجین", "net margin"),
    "market_value": ("ارزش بازار", "مارکت کپ", "market cap"),
    "last_price": ("آخرین قیمت", "قیمت پایانی", "قیمت آخر", "price", "قیمت"),
    "change_pct": ("درصد تغییر قیمت", "بازده روزانه", "تغییر روزانه", "تغییر قیمت", "change"),
    "volume": ("حجم معاملات", "حجم معامله", "trade volume", "volume", "حجم"),
    "value": ("ارزش معاملات", "ارزش معامله", "گردش معاملات", "turnover", "trade value"),
    "trade_count": ("تعداد معاملات", "تعداد معامله", "trade count", "trades"),
    "support_level": ("سطح حمایت", "قیمت حمایت", "support"),
    "resistance_level": ("سطح مقاومت", "قیمت مقاومت", "resistance"),
    "distance_to_support": ("فاصله تا حمایت", "distance to support"),
    "distance_to_resistance": ("فاصله تا مقاومت", "distance to resistance"),
    "poc_price": ("نقطه کنترل حجم", "قیمت poc", "poc"),
    "value_area_high": ("سقف ناحیه ارزش", "value area high", "vah"),
    "value_area_low": ("کف ناحیه ارزش", "value area low", "val"),
}
...
_INDUSTRY_TERMS: dict[str, str] = {
    "خودرویی": "خودرو و ساخت قطعات",
    "خودرو و قطعات": "خودرو و ساخت قطعات",
    "دارویی": "دارویی",
    "فلزات اساسی": "فلزات اساسی",
    "فلزی": "فلزات اساسی",
    "سیمانی": "سیمان، آهک و گچ",
    "بانکی": "بانک و موسسات اعتباری",
    "بانک ها": "بانک و موسسات اعتباری",
    "پالایشی": "فرآورده های نفتی",
    "فرآورده نفتی": "فرآورده های نفتی",
    "سرمایه گذاری": "سرمایه گذاری",
    "حمل و نقل": "حمل و نقل",
    "نیروگاهی": "عرضه برق، گاز، بخار",
    "برق و نیروگاه": "عرضه برق، گاز، بخار",
    "بیمه ای": "بیمه و صندوق بازنشستگی",
    "مخابراتی": "مخابرات",
    "لاستیکی": "لاستیک و پلاستیک",
    "رایانه ای": "رایانه و فعالیت های وابسته",
    "ماشین آلات": "ماشین آلات و تجهیزات",
    "شیمیایی": "محصولات شیمیایی",
    "غذایی": "غذایی و آشامیدنی",
    "کشاورزی": "کشاورزی",
}
...
_MAGNITUDE_MULTIPLIERS = {
    ...
    "میلیون": 1_000_000.0,
    "m": 1_000_000.0,
    "میلیارد": 1_000_000_000.0,
    "b": 1_000_000_000.0,
    "تریلیون": 1_000_000_000_000.0,
    "همت": 10_000_000_000_000.0,
}


def _alias_pattern(alias: str) -> str:
    return re.escape(normalize_financial_text(alias)).replace(r"\ ", r"\s+")


def _parse_financial_amount(
    number: str,
    magnitude: str | None,
    currency: str | None,
    percent: str | None,
    ...
    value = float(number.replace(",", "")) * _MAGNITUDE_MULTIPLIERS.get(magnitude or "", 1.0)
    if currency == "تومان" and magnitude != "همت":
        value *= 10.0
    if percent and field in _SCORE_FIELDS and abs(value) > 1:
        value /= 100.0
    return value


def _extract_compositional_filters(text: str) -> list[dict[str, Any]]:
    """Parse free-form numeric conditions such as «حجم بالای ۱۰ میلیون».

    This parser deliberately works from field aliases + operators instead of
    full sentences, so new combinations are understood automatically.
    """
    filters: list[dict[str, Any]] = []
    number = r"-?\d+(?:\.\d+)?"
    magnitude = r"(?:هزار|میلیون|میلیارد|تریلیون|همت|k|m|b)?"
    currency = r"(?:ریال|تومان)?"
    percent = r"(?:درصد|%)?"
    operator_pattern = "|".join(
        sorted((_alias_pattern(op) for op in _OPERATOR_WORDS), key=len, reverse=True)
    )
    ...
    for field, aliases in _FIELD_ALIASES.items():
        for alias in sorted(aliases, key=len, reverse=True):
            ...
            between_pattern = re.compile(
                rf"{alias_re}\s*(?:بین|از)\s*(?P<a>{number})\s*(?P<am>{magnitude})\s*(?P<ac>{currency})\s*(?P<ap>{percent})\s*(?:تا|الی|و)\s*(?P<b>{number})\s*(?P<bm>{magnitude})\s*(?P<bc>{currency})\s*(?P<bp>{percent})"
            )
                ...
                filters.append(
                    {
                        "field": field,
                        "operator": "between",
                        "value": _parse_financial_amount(
                            match.group("a"), match.group("am"), match.group("ac"), match.group("ap"), field
                        ),
                        "value_to": _parse_financial_amount(
                            match.group("b"), match.group("bm"), match.group("bc"), match.group("bp"), field
                        ),
                        "_explicit": True,
                    }
                )
            ...
            comparison_patterns = (
                re.compile(
                    rf"{alias_re}\s*(?P<op>{operator_pattern})\s*(?P<n>{number})\s*(?P<m>{magnitude})\s*(?P<c>{currency})\s*(?P<p>{percent})"
                ),
                re.compile(
                    rf"(?P<op>{operator_pattern})\s*{alias_re}\s*(?P<n>{number})\s*(?P<m>{magnitude})\s*(?P<c>{currency})\s*(?P<p>{percent})"
                ),
            )
            for pattern in comparison_patterns:
                for match in pattern.finditer(text):
                    op_text = normalize_financial_text(match.group("op"))
                    filters.append(
                        {
                            "field": field,
                            "operator": _OPERATOR_WORDS.get(op_text, "eq"),
                            "value": _parse_financial_amount(
                                match.group("n"), match.group("m"), match.group("c"), match.group("p"), field
                            ),
                            "_explicit": True,
                        }
                    )
...
def _extract_screener_filters(text: str) -> list[dict[str, Any]]:
    ...
    for phrase, criterion in sorted(_SCREENER_SYNONYMS.items(), key=lambda entry: len(entry[0]), reverse=True):
        normalized_phrase = normalize_financial_text(phrase)
        for match in re.finditer(re.escape(normalized_phrase), text_lower):
            span = match.span()
            if any(span[0] >= start and span[1] <= end for start, end in claimed_spans):
                continue
            claimed_spans.append(span)
            filters.append(dict(criterion))

    # 2. Market and industry vocabulary maps colloquial sector names to the
    # exact values stored by the screener pipeline.
    for phrase, market in sorted(_MARKET_TERMS.items(), key=lambda entry: len(entry[0]), reverse=True):
        if normalize_financial_text(phrase) in text_lower:
            filters.append({"field": "market", "operator": "eq", "value": market})
            break
    for phrase, industry in sorted(_INDUSTRY_TERMS.items(), key=lambda entry: len(entry[0]), reverse=True):
        if normalize_financial_text(phrase) in text_lower:
            filters.append({"field": "industry", "operator": "eq", "value": industry})
            break

    # 3. Compositional Persian/English expressions with words, units and ranges.
    filters.extend(_extract_compositional_filters(text_lower))

    # 4. Compact numeric patterns like RSI<30, MACD>0, P/E<8, حجم>1000
    _OP = r"([<>]=?|=)"
    _NUM = r"(\d+(?:\.\d+)?)"
    _NUM_NEG = r"(-?\d+(?:\.\d+)?)"
    numeric_patterns = [
    ...
    # Explicit numeric conditions override a qualitative default for the same
    # field (for example «ارزنده با P/E کمتر از ۱۰» must use ۱۰, not preset ۸).
    explicit_fields = {f["field"] for f in filters if f.get("_explicit")}
    if explicit_fields:
        filters = [f for f in filters if f.get("_explicit") or f["field"] not in explicit_fields]

    # Deduplicate while preserving order (normalize numeric values to float for comparison)
    seen: set[str] = set()
    unique: list[dict[str, Any]] = []
    for f in filters:
        f.pop("_explicit", None)
        value = f["value"]
        try:
            value_key = (
                float(value)
            ...
            value_key = str(value)
        key = f"{f['field']}:{f['operator']}:{value_key}:{f.get('value_to')}"
        if key not in seen:
            seen.add(key)
            unique.append(f)
    return unique
...
def _detect_filter_logic(text: str) -> str:
    ...
    """
    if _OR_PATTERN.search(normalize_financial_text(text)):
        return "or"
    return "and"
...
# Every stable section exposed by the shared application navigation is also
# addressable from natural language.  Dynamic routes such as /symbol/[symbol]
# are generated by their dedicated handlers and are intentionally excluded.
_PAGE_MAP.update(
    {
        "داشبورد شخصی": "/personal",
        "بازار طلا": "/markets/gold",
        "بازار نقره": "/markets/silver",
        "شاخص کل": "/markets/indices",
        "شاخص هم وزن": "/markets/indices",
        "نرخ ارز": "/markets/fx",
        "بازار جفت ارز": "/crypto-market",
        "حق تقدم": "/markets/stocks/rights",
        "نمادهای متوقف": "/markets/stocks/suspended",
        "پرداخت سود نقدی": "/markets/stocks/dividends",
        "معاملات پایانی": "/markets/stocks/closing",
        "اوراق تال": "/markets/stocks/tal",
        "اوراق بدهی": "/markets/bonds",
        "تسهیلات مسکن": "/markets/stocks/maskan",
        "بازار آتی": "/markets/futures",
        "قرارداد آتی": "/markets/futures",
        "سلف موازی": "/markets/futures/salaf",
        "صندوق درون روز": "/funds/intraday",
        "اختیار معامله": "/options",
        "آپشن": "/options",
        "بازار آپشن": "/markets/options-market",
        "بازار کالا": "/markets/commodities-market",
        "بورس کالا": "/markets/commodity",
        "جستجوی آپشن": "/markets/search",
        "تابلویی": "/markets/tabaei",
        "فروش بازار": "/markets/sales",
        "تحلیل تکنیکال طلا": "/gold/technical",
        "سکه ها": "/gold/coins",
        "صندوق طلا": "/gold/funds",
        "طلای فیزیکی": "/gold/physical",
        "آپشن طلا": "/gold/options",
        "آتی طلا": "/gold/futures",
        "پرتفوی طلا": "/gold/portfolio",
        "خرید پله ای طلا": "/gold/dca",
        "dca طلا": "/gold/dca",
        "توکن طلا": "/gold/tokens",
        "هشدار طلا": "/gold/alerts",
        "تحلیل طلا": "/gold/analytics",
        "تحلیل بنیادی": "/fundamental",
        "ترازنامه": "/fundamental/balance-sheet",
        "صورت سود و زیان": "/fundamental/income",
        "جریان نقدی": "/fundamental/cashflow",
        "نسبت های مالی": "/fundamental/ratios",
        "افزایش سرمایه": "/fundamental/capital-increase",
        "سود هر سهم": "/fundamental/eps",
        "سود نقدی": "/fundamental/dps",
        "تولید و فروش": "/fundamental/production",
        "دیده بان آلفا": "/alpha",
        "دیده بان بازار": "/market-watch",
        "عمق بازار": "/market-depth",
        "بینش بازار": "/market-insights",
        "غربالگر 110": "/screener110",
        "غربالگر ساده": "/screener",
        "سیگنال چند بازاره": "/multi-market-signals",
        "سیگنال سهام": "/signals/stocks",
        "همه سیگنال ها": "/signals/all",
        "ثبت سیگنال": "/signals/register",
        "معامله آزمایشی": "/paper-trading",
        "اسکن کامل": "/backtest/full-scan",
        "تولید خودکار": "/backtest/generate",
        "ترکیب استراتژی": "/backtest/compose",
        "walk forward": "/backtest/walk-forward",
        "مونت کارلو": "/backtest/monte-carlo",
        "monte carlo": "/backtest/monte-carlo",
        "روش های بک تست": "/backtest/methods",
        "مرکز تصمیم": "/backtest/decision",
        "سیستم انطباقی": "/backtest/adaptive",
        "کاسکد": "/backtest/cascade",
        "موتور استراتژی": "/backtest/engine",
        "موتور تصمیم": "/decision-engine",
        "صرافی رمزارز": "/crypto-exchange",
        "api تبدیل": "/tabdeal-api",
        "پیش بینی ها": "/forecasts",
        "همبستگی": "/correlations",
        "توصیه ها": "/recommendations",
        "چت هوشمند": "/chat",
        "تقویم اقتصادی": "/economic-calendar",
        "ساعت بازار": "/economic-calendar",
        "نتایج": "/results",
        "واردات نماد": "/instruments/import",
        "تابلوی معاملات": "/trades",
        "قیمت ها": "/quotes",
        "واردات قیمت": "/quotes/import",
        "حسابرسی کدال": "/codal/audit",
        "واردات کدال": "/codal/import",
        "انبار داده": "/data",
        "همگام سازی": "/sync",
        "مدیریت همگام سازی": "/sync-manager",
        "تنظیمات همگام سازی": "/sync-settings",
        ...
        "ویجت ها": "/widgets",
        "افزونه ها": "/plugins",
        "رهاورد": "/plugins/rahavard",
        "کیفیت داده": "/data-import",
        "سلامت داده": "/data-health",
        "انتقال داده": "/data-transfer",
        "اندیکاتورها": "/indicators",
        "brsapi کدال": "/brsapi/codal",
        "brsapi": "/brsapi",
        "جداول": "/tables",
    }
)
...
def resolve_page(text: str) -> str | None:
    ...
    text_lower = normalize_financial_text(text)
    # Longest aliases win, so «بازار طلا» resolves to /markets/gold rather
    # than the generic /markets route.
    for keyword, path in sorted(_PAGE_MAP.items(), key=lambda entry: len(entry[0]), reverse=True):
        if normalize_financial_text(keyword) in text_lower:
            return path
    return None
...
class UnifiedAssistantService:
    ...
    async def process(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            response_time = (time.time() - start_time) * 1000
            log_query(
                text, intent=intent, params=params, response_type="error", success=False, response_time_ms=response_time
            )
                ...
                if intent == "gainers":
                    ...
                    if result.success and result.value:
                        ...
                        return {
                            "text": "\n".join(lines),
                            "type": "market",
                            "suggestions": ["کاهشی‌ها", "خلاصه بازار", "بهترین سهم‌ها"],
                        }
                ...
                if intent == "losers":
                    ...
                    if result.success and result.value:
                        ...
                        return {
                            "text": "\n".join(lines),
                            "type": "market",
                            "suggestions": ["افزایشی‌ها", "خلاصه بازار", "تحلیل فولاد"],
                        }
            ...
            try:
                ...
                if result.success and result.value:
                    ...
                    return {
                        ...
                        "suggestions": [
                            f"اضافه {await self._random_symbol()} به دیده‌بان",
                            "تحلیل فولاد",
                            "خلاصه بازار",
                        ],
                    }
    ...
    async def _handle_portfolio_remove(self, params: dict[str, Any], text: str) -> dict[str, Any]:
        ...
        return {
            "text": result.get(
                "text", "✅ نماد از پرتفوی حذف شد." if result.get("type") == "portfolio" else "خطا در حذف از پرتفوی"
            ),
            "type": "portfolio",
    ...
    async def _handle_screener(self, text: str) -> dict[str, Any]:
        ...
        if extracted_filters:
            if len(extracted_filters) > 1:
                ...
                logic_part = ""
            filters_text = "\n".join(f"  • {f['field']} {f['operator']} {f['value']}" for f in extracted_filters)
            response_text = (
