...
class NewsIngestionService:
    ...
    async def ingest(
        ...
        if sources:
            ...
            provider.feeds = {
                k: v for k, v in provider.feeds.items() if k in sources or any(k.startswith(f"{s}_") for s in sources)
            }
        ...
        if not skip_dedup:
            ...
            if verbose:
                print(
                    f"  ✅ {len(raw_articles)} unique articles remain ({stats['duplicates_removed']} duplicates removed)"
                )
        else:
