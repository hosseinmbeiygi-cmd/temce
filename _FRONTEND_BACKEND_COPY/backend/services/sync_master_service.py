...
class MasterReport:
    ...
    def print_summary(self) -> None:
        ...
        for phase in self.phases:
            ...
            print(f"  {icon} {phase.phase_name:<30s} {phase.total_items:>6} items  {phase.duration_ms / 1000:>6.1f}s")
            if phase.error:
                print(f"     Error: {phase.error[:100]}")
        print(f"{'=' * 70}")
        print(f"  🏁 Total: {self.total_items:,} items in {self.total_duration_ms / 1000:.1f}s")
...
class MasterSyncService:
    ...
    async def _phase_3_transactions(
        ...
        symbols = await self._get_all_symbols(session)
        symbols = symbols[: self.max_symbols]
        logger.info("Phase 3: processing %d symbols (recent %d days)", len(symbols), self.transaction_days_back)
        ...
        dates_to_sync = [
            (jdatetime.date.today() - timedelta(days=d)).strftime("%Y-%m-%d") for d in range(self.transaction_days_back)
        ]
    ...
    async def _phase_5_codal(
        ...
        try:
            ...
            await self._add_operation(
                phase,
                "Codal Download & Parse",
                True,
                dl_summary.parsed,
                (time.monotonic() - dl_start) * 1000,
            )
