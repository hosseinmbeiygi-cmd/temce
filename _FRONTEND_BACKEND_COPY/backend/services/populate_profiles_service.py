...
class PopulateProfilesService:
    ...
    async def _populate_one_with_savepoint(self, symbol: str, sym_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            logger.warning("Skipped %s (savepoint rollback): %s", symbol, exc)
            return "skipped"
    ...
    async def _populate_one(self, symbol: str, sym_data: dict[str, Any]) -> str:
        ...
        try:
            ...
            """)
            daily_rows = [dict(r._mapping) for r in (await self._session.execute(q, {"sym": symbol})).fetchall()]
        ...
        try:
            ...
            """)
            legal_rows = [dict(r._mapping) for r in (await self._session.execute(q, {"sym": symbol})).fetchall()]

            _inst_buy_30d = sum(float(r.get("legal_buy_volume", 0) or 0) for r in legal_rows)
            _inst_sell_30d = sum(float(r.get("legal_sell_volume", 0) or 0) for r in legal_rows)
        except Exception:
            pass
