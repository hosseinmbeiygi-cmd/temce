...
MARKET_SECTORS: frozenset[str] = frozenset({"سهام", "طلا و سکه", "ارز", "رمزارز", "کامودیتی", "بورس کالا", "صندوق"})
...
def sector_summary() -> list[dict[str, str | int]]:
    ...
    return [
        {"sector": sector, "count": count} for sector, count in sorted(counts.items(), key=lambda kv: (-kv[1], kv[0]))
    ]
