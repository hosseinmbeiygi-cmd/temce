...
class AuditPlanner:
    ...
    def assess_scope(
        ...
        if scope["total_locations"] > 0:
            scope["coverage_pct"] = round(scope["significant_locations"] / scope["total_locations"] * 100, 1)
        return scope
...
def format_planning_for_response(strategy: AuditStrategy) -> dict[str, Any]:
    return {
        ...
        "team": [{"role": m.role, "name": m.name, "responsibilities": m.responsibilities} for m in strategy.team],
        "timeline": [{"phase": t.phase, "key_deliverables": t.key_deliverables} for t in strategy.timeline],
