...
class FraudAssessor:
    ...
    def assess_asset_fraud(self, snap: FinancialSnapshot) -> list[FraudIndicatorResult]:
        ...
        for ind in ASSET_FRAUD_INDICATORS:
            ...
            if ind["code"] == "AF1" and revenue > 0 and other_expenses > revenue * 0.3:
            ...
            elif ind["code"] == "AF2" and prev_inventory is not None and prev_cogs is not None and prev_cogs > 0:
                ...
                if inv_change < -prev_cogs * 0.2:
                    ...
                    severity = "medium"
                    note = (
                        f"Inventory dropped {abs(inv_change):,.0f} ({abs(inv_change) / prev_cogs * 100:.0f}% of COGS)"
                    )
    ...
    def assess_corruption(self, snap: FinancialSnapshot) -> list[FraudIndicatorResult]:
        ...
        for ind in CORRUPTION_INDICATORS:
            ...
            if ind["code"] == "CR1" and revenue > 0 and other_expenses > revenue * 0.2:
                ...
                severity = "medium"
                note = (
                    f"High other expenses ({other_expenses / revenue * 100:.0f}% of revenue) — possible consulting fees"
                )
                ...
                rev_growth = (
                    (snap.revenue - snap.prev.revenue) / snap.prev.revenue if snap.prev and snap.prev.revenue else 0
                )
    ...
    def assess_all(self, snap: FinancialSnapshot) -> FraudAssessment:
        ...
        revenue_score = (
            sum(ind.weight for ind in revenue_indicators if ind.is_present)
            / sum(ind["weight"] for ind in REVENUE_FRAUD_INDICATORS)
            * 100
            if REVENUE_FRAUD_INDICATORS
            else 0
        )

        asset_score = (
            sum(ind.weight for ind in asset_indicators if ind.is_present)
            / sum(ind["weight"] for ind in ASSET_FRAUD_INDICATORS)
            * 100
            if ASSET_FRAUD_INDICATORS
            else 0
        )

        corruption_score = (
            sum(ind.weight for ind in corruption_indicators if ind.is_present)
            / sum(ind["weight"] for ind in CORRUPTION_INDICATORS)
            * 100
        ...
        "revenue_indicators": [
            {
                ...
                "severity": i.severity,
                "evidence": i.evidence_note,
            }
            for i in fa.revenue_indicators
        ],
        "asset_indicators": [
            {"code": i.code, "description": i.description, "is_present": i.is_present} for i in fa.asset_indicators
        ],
        "corruption_indicators": [
            {"code": i.code, "description": i.description, "is_present": i.is_present} for i in fa.corruption_indicators
        ],
