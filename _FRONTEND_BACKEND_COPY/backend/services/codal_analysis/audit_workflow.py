...
class ProfessionalAuditWorkflow:
    ...
    def perform_analytical_procedures(
        self, snap: FinancialSnapshot, prev: FinancialSnapshot | None = None
    ) -> list[AnalyticalProcedureResult]:
        ...
        results.append(
            AnalyticalProcedureResult(
                ...
                status="unusual"
                if abs(gm_deviation) > self.analytical_thresholds["gross_margin"]["threshold"]
                else "expected",
...
class AnalyticalReview:
    ...
    def ratio_analysis_procedure(
        self, snap: FinancialSnapshot, ratio_name: str, value: float
    ) -> AnalyticalProcedureResult:
    ...
    def substantive_analytical_procedure(
        self, snap: FinancialSnapshot, prev: FinancialSnapshot | None
    ) -> list[AnalyticalProcedureResult]:
        results = []
        if not prev:
            return results
