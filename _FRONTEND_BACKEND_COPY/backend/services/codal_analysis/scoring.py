...
def compute_health_score(analysis: ComprehensiveAnalysis) -> FinancialHealthScore:
    ...
    overall = sum(category_scores.get(cat, 0) * cfg["weight"] for cat, cfg in SCORING_CONFIG.items())
    ...
    return FinancialHealthScore(
        ...
        financial_strength=round(
            category_scores.get("profitability", 0) * 0.5
            + category_scores.get("efficiency", 0) * 0.3
            + category_scores.get("earnings_quality", 0) * 0.2,
            4,
        ),
