...
class OpinionFormulator:
    ...
    def determine_opinion(
        ...
        if not scope_limitation:
            key = ("material" if is_material else "not_material", "pervasive" if is_pervasive else "not_pervasive")
        else:
            key = ("unable_to_obtain", "pervasive" if is_pervasive else "not_pervasive")
    ...
    def form_opinion(
        ...
        # Basis paragraphs
        if is_material:
            opinion.basis_paragraphs.append(
                OpinionBasisParagraph(
        ...
        for em in emphasis_matters:
            opinion.emphasis_of_matter.append(
                EmphasisOfMatter(
...
def format_opinion_for_response(opinion: AuditOpinion) -> dict[str, Any]:
    return {
        ...
        "basis_paragraphs": [
            {"section": p.section, "description": p.description, "amount": p.amount} for p in opinion.basis_paragraphs
        ],
        "emphasis_of_matter": [
            {"description": e.description, "standard_ref": e.standard_ref} for e in opinion.emphasis_of_matter
        ],
        "other_matters": [{"description": o.description} for o in opinion.other_matters],
