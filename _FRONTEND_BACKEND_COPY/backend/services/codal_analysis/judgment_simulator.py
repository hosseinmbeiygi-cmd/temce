...
class ProfessionalJudgmentSimulator:
    ...
    def impairment_testing(
        ...
        if is_impaired:
            reasoning.append(
                f"IMPAIRED: Carrying amount exceeds recoverable amount by {impairment_loss:,.0f} (IAS 36.59)"
            )
        else:
    ...
    def tax_reconciliation(
        ...
        reconciliation_items = [
            ...
            {
                "description": f"Expected tax expense at {statutory:.0%}",
                "amount": accounting_profit * statutory,
                "type": "expected",
            },
        ]
        ...
        for desc, val in permanent_add:
            reconciliation_items.append(
                {"description": f"Permanent addition: {desc}", "amount": val, "type": "permanent_add"}
            )
        for desc, val in permanent_ded:
            reconciliation_items.append(
                {"description": f"Permanent deduction: {desc}", "amount": -val, "type": "permanent_ded"}
            )
    ...
    def consolidation_worksheet(
        ...
        for i, sub in enumerate(subsidiaries):
            ...
            ownership = (
                (ownership_percentages or [1.0] * len(subsidiaries))[i] if i < len(ownership_percentages or []) else 1.0
            )
        ...
        for t in interco:
            interco_entry = {
                ...
                "type": t.get("type", "revenue"),
            }
            ws.intercompany_transactions.append(interco_entry)
            if t.get("type") == "revenue":
                eliminations_revenue += t.get("amount", 0)
                ws.elimination_entries.append(
                    {
                        "account": "Revenue",
                        "debit": t.get("amount", 0),
                        "credit": 0,
                        "description": f"Eliminate intercompany revenue: {t.get('description', '')}",
                    }
                )
                ...
                ws.elimination_entries.append(
                    {
                        ...
                        "debit": t.get("amount", 0),
                        "credit": 0,
                        "description": f"Eliminate intercompany profit: {t.get('description', '')}",
                    }
                )
        ...
        ws.elimination_entries.append(
            {
                ...
                "debit": 0,
                "credit": parent_investment,
                "description": "Eliminate parent investment against subsidiary equity",
            }
        )
