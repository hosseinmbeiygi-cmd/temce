...
class AccountRiskProfile:
    ...
    def calculate_overall(self) -> None:
        if not self.assertion_risks:
            return
        self.overall_risk_score = max(r.combined_risk for r in self.assertion_risks.values())
        ...
        self.key_assertions = [a for a, r in self.assertion_risks.items() if r.risk_level == "high"]
...
class AuditRiskAssessor:
    ...
    def _assess_revenue_assertions(self, snap: FinancialSnapshot, entity_factor: float) -> AccountRiskProfile:
    ...
    def _assess_receivables_assertions(self, snap: FinancialSnapshot, entity_factor: float) -> AccountRiskProfile:
    ...
    def _assess_inventory_assertions(self, snap: FinancialSnapshot, entity_factor: float) -> AccountRiskProfile:
    ...
    def _assess_ppe_assertions(self, snap: FinancialSnapshot, entity_factor: float) -> AccountRiskProfile:
    ...
    def _assess_payables_assertions(self, snap: FinancialSnapshot, entity_factor: float) -> AccountRiskProfile:
    ...
    def assess_all(self, snap: FinancialSnapshot) -> AuditRiskAssessment:
        ...
        account_risk_scores = [p.overall_risk_score for p in account_profiles.values()] if account_profiles else [0.5]
        max_account_risk = max(account_risk_scores)
        overall_audit_risk = max(entity_risk_level_val, max_account_risk)

        high_risk_accounts = [name for name, p in account_profiles.items() if p.overall_risk_level == "high"]
        key_assertions_by_account = {name: p.key_assertions for name, p in account_profiles.items()}
