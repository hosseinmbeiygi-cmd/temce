...
def compute_total_combinations(
    strategies: list[str],
...
class GeneticOptimizer:
    ...
    def __init__(
        self, population_size: int = 50, generations: int = 12, mutation_rate: float = 0.15, crossover_rate: float = 0.7
    ):
...
class WalkForwardValidator:
    ...
    async def validate(
        self, strategy_name: str, params: dict[str, Any], data: list[dict], capital: float, simulator: BacktestSimulator
    ) -> dict[str, Any]:
...
class CascadeEngine:
    ...
    async def run(
        ...
        try:
            ...
            logger.info("Phase 1: Grid search %d combinations across %d symbols", len(all_combos), len(symbols))
            ...
            for i in range(0, len(all_combos), batch_size):
                ...
                for sym in symbols:
                    tasks = [
                        self._run_single(name, params, symbol_data[sym], capital, stop_loss) for name, params in batch
                    ]
                    results = await asyncio.gather(*tasks, return_exceptions=True)
                    for r in results:
                        if isinstance(r, Exception) or r is None:
            ...
            if use_genetic:
                ...
                for strategy_name in seen_strategies:
                    ...
                    for sym in symbols:
                        ga_results = await ga.optimize(strategy_name, bounds, symbol_data[sym], capital, self.simulator)
                        for r in ga_results:
                            r["symbol"] = sym
                            r["method"] = "genetic"
            ...
            if use_walk_forward:
                ...
                for r in self._results[:100]:  # Validate top 100
                    ...
                    wf_result = await wf.validate(r["strategy"], r["params"], data, capital, self.simulator)
            ...
            logger.info("Engine complete: %d strategies passed, %d validated", len(self._results), validated_count)
            return Result.ok(
                {
    ...
    async def _run_single(
        self, strategy_name: str, params: dict[str, Any], data: list[dict], capital: float, stop_loss: float = 8.0
    ):
        try:
            registry = get_strategy_registry()
            cls = registry.get(strategy_name)
