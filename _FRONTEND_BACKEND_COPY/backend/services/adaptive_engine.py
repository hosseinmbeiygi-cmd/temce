...
class CoEvolutionOptimizer:
    ...
    def evaluate_fitness(self, alpha: AlphaChromosome, omega: OmegaChromosome, data: list[dict]) -> float:
        ...
        for i in range(lookback, len(data)):
            ...
            buy_signal = momentum > alpha.genes["momentum_threshold"] and rsi_val < 70 and rsi_val > 30
        ...
        fitness = total_return * 0.01 + sharpe * 0.3 + calmar * 0.2 + win_rate * 0.1 - max_dd * 10
    ...
    def _compute_rsi(self, data: list[dict], idx: int, period: int) -> float:
        """RSI at *idx* over the trailing *period* bars (canonical core.indicators)."""
...
class AdaptiveTradingSystem:
        ...
        for bar in data[-500:]:
            features.append(
                [
                    ...
                    bar.get("volume", 0)
                    / max(
                        sum(b.get("volume", 0) for b in data[max(0, data.index(bar) - 20) : data.index(bar)]) / 20, 1
                    ),
                    1.0,
                ]
            )
    ...
    def decide(self, market_data: dict[str, Any], portfolio_returns: list[float]) -> dict[str, Any]:
        ...
        return {
            ...
            "risk_check": "SAFE"
            if self.safe_agent.calculate_cvar(portfolio_returns) <= self.safe_agent.risk_limit
            else "CAUTION",
        }
...
class TSDEngineDSS:
    ...
    def generate_signal(
        self, market_features: list[float], portfolio_returns: list[float] | None = None
    ) -> dict[str, Any]:
