...
class BatchLoader:
    ...
    async def load_all(self) -> dict[str, Any]:
        ...
        profile_map: dict[str, dict[str, Any]] = {p_["symbol"]: p_ for p_ in profiles_raw if p_.get("symbol")}
        snapshot_map: dict[str, dict[str, Any]] = {sp_["symbol"]: sp_ for sp_ in snaps_raw if sp_.get("symbol")}
...
class VectorCalculator:
    ...
    def trend_label(self, closes: list[float]) -> str:
        ...
        NOTE: Requires at least 5 data points. If fewer than 20 points are
        available, the MA20 is computed on the available data (not a true 20-day
        average) — callers should be aware of this limitation for short-history
        symbols.
        """
        if len(closes) < 5:
            return "flat"
    ...
    def _compute_atr(rows: list[dict[str, Any]], price: float) -> float:
        """ATR(14) over daily rows (rows[0] = newest; delegated to core.indicators)."""
        n = min(15, len(rows))
        if n < 2:
            return price * 0.02
        # Rows are newest-first; reverse into chronological order for core.compute_atr.
        chronological = list(reversed(rows[:n]))
        highs = [float(r.get("price_max", 0) or 0) for r in chronological]
        lows = [float(r.get("price_min", 0) or 0) for r in chronological]
        closes = [float(r.get("price_close", 0) or 0) for r in chronological]
        atr = compute_atr(highs, lows, closes, 14)
        return atr if atr is not None else price * 0.02
    ...
    def score_gov(self, lr: float, av: float, ir: float, cap_type: str | None, gov_news: int) -> float:
        s = 0.0
        if lr > 1.0:
            s -= 10.0
        elif lr > 0.5:
    ...
    def score_feedstock(self, has_fs: bool, change_pct: float, feedstock_price: float) -> float:
        """Score based on feedstock price trend.

        Args:
            has_fs: whether feedstock_price data exists
            change_pct: feedstock_change_pct (columns 22) — % change in feedstock price
            feedstock_price: feedstock_price (column 21) — absolute feedstock price

        Returns:
            Score 0-10 based on feedstock conditions.
        """
        if not has_fs or feedstock_price == 0:
            return 0.0
        # Favorable: feedstock price decreasing (change_pct < 0) is good for producers
        if change_pct < -5:
            return 10.0
        if change_pct < 0:
            return 5.0
        # Unfavorable: feedstock price increasing
        if change_pct > 10:
            return 0.0
        return 2.5  # Neutral/slight change
    ...
    def calculate(
        ...
        if daily_rows:
            ...
            price = float(daily_rows[0].get("price_close") or daily_rows[0].get("price_last", 0) or 0)
        ...
        # Column 21-22: feedstock price and change
        feedstock_price = float(profile.get("feedstock_price", 0) or 0) if profile else 0.0
        feedstock_change = float(profile.get("feedstock_change_pct", 0) or 0) if profile else 0.0
        ...
        # ── 7. Final score (96-97) ──
        raw = (
            s_f * WEIGHTS["fundamental"]
            + s_v * WEIGHTS["valuation"]
            + s_i * WEIGHTS["institutional"]
            + s_t * WEIGHTS["technical"]
            + s_m * WEIGHTS["macro"]
            + s_g * WEIGHTS["gov_support"]
            + s_l * WEIGHTS["liquidity"]
            + s_fb * WEIGHTS["farabourse"]
        ...
        # Stop loss (101)
        # Use min() to pick the tighter (closer-to-price) stop loss for
        # conservative position sizing — this ensures risk per position stays
        # within the intended 1% of capital.
        sl = min(price - (2 * atr14), price * 0.93) if price > 0 else 0
...
class BulkWriter:
    ...
    async def write_snapshots(self, rows: list[dict[str, Any]]) -> None:
        ...
        for i in range(0, len(rows), CHUNK_SIZE):
            ...
            stmt = stmt.on_conflict_do_nothing(constraint="screener_snapshots_pkey")
            # NOTE: no explicit begin() — the session from get_session()/
            # endpoint dependency already holds an open transaction.
            await self._session.execute(stmt)

    async def write_signals(self, results: list[dict[str, Any]], now: datetime) -> list[dict[str, Any]]:
        ...
        for i in range(0, len(rows), CHUNK_SIZE):
            ...
            stmt = stmt.on_conflict_do_nothing(constraint="screener_signals_pkey")
            # NOTE: no explicit begin() — the session from get_session()/
            # endpoint dependency already holds an open transaction.
            await self._session.execute(stmt)
...
class Screener110Service:
    ...
    def __init__(
        self, session: AsyncSession, total_capital: float = DEFAULT_CAPITAL, brsapi: Any | None = None
    ) -> None:
        self._session = session
        self._brsapi = brsapi
    ...
    async def run_full_cycle(self) -> list[dict[str, Any]]:
        ...
        for sym_data in symbols:
            ...
            if sl > 0 and cp > sl:
                result["position_size"] = int((0.01 * self.total_capital) / (cp - sl))
            ...
            snapshot_rows.append(
                {
                    ...
                    if daily_map.get(sym)
                    else 0,
                    "volume_ma_50": float(last_snap.get("volume_ma_50", 0) or 0) if last_snap else 0,
                    "farabourse_volume": float(last_snap.get("farabourse_volume", 0) or 0) if last_snap else 0,
                    "farabourse_price": float(last_snap.get("farabourse_price", 0) or 0) if last_snap else 0,
        ...
        # ── Phase 3: Save ────────────────────────────────────────────────
        logger.info("Phase 3/3: Saving %d snapshots + %d signals...", len(snapshot_rows), len(results))
        t0 = time.perf_counter()
    ...
    async def run_symbol(self, symbol: str) -> dict[str, Any] | None:
        ...
        """)
        daily = [dict(r._mapping) for r in (await self._loader._session.execute(q, {"sym": symbol})).fetchall()]
        ...
        """)
        legal = [dict(r._mapping) for r in (await self._loader._session.execute(q, {"sym": symbol})).fetchall()]
...
async def run_cycle(session: AsyncSession, total_capital: float = DEFAULT_CAPITAL) -> list[dict[str, Any]]:
