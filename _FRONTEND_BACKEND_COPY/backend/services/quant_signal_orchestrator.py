...
class CrossMarketCorrelator:
    ...
    async def _fetch_price_changes(
        ...
        try:
            ...
            if len(prices) >= 2:
                changes["change_pct_5d"] = (
                    round((prices[0] - prices[min(4, len(prices) - 1)]) / prices[min(4, len(prices) - 1)] * 100, 2)
                    if len(prices) >= 5
                    else round((prices[0] - prices[-1]) / prices[-1] * 100, 2)
                )
            if len(prices) >= 21:
                changes["change_pct_20d"] = round((prices[0] - prices[-1]) / prices[-1] * 100, 2)
            return changes
        except Exception:
            return {}
    ...
    def _analyze_market_regime(
        ...
        if gold_bias["bias"] == "bullish" and stock_bias["bias"] == "bearish":
            result.append(
                _signal_description(
                    "Risk-Off",
                    "Ø³Ù†Ø§Ø±ÛŒÙˆÛŒ Ø±ÛŒØ³Ú©â€ŒÚ¯Ø±ÛŒØ²ÛŒ",
                    "DEFENSIVE",
            ...
            result.append(
                _signal_description(
                    "Risk-On",
                    "Ø³Ù†Ø§Ø±ÛŒÙˆÛŒ Ø±ÛŒØ³Ú©â€ŒÙ¾Ø°ÛŒØ±ÛŒ",
                    "AGGRESSIVE",
        ...
        if commodity_bias["bias"] == "bullish" and currency_bias["bias"] == "bullish":
            ...
            result.append(
                _signal_description(
                    "Import-Inflation",
                    "ÙØ´Ø§Ø± ØªÙˆØ±Ù…ÛŒ ÙˆØ§Ø±Ø¯Ø§ØªÛŒ",
                    "CAUTION",
        ...
        bearish_count = sum(
            1 for b in [gold_bias, stock_bias, crypto_bias, currency_bias, commodity_bias] if b["bias"] == "bearish"
        )
        if all_bearish and bearish_count >= 4:
            result.append(
                _signal_description(
                    "Systemic-Risk",
                    "Ø±ÛŒØ³Ú© Ø³ÛŒØ³ØªÙ…ÛŒ",
                    "CASH",
        ...
        if gold_bias["bias"] == "bullish" and gold_bias["strength"] == "strong" and stock_bias.get("bias") != "bearish":
            result.append(
                _signal_description(
                    "Safe-Haven",
                    "Ù¾Ù†Ø§Ù‡Ú¯Ø§Ù‡ Ø§Ù…Ù†",
        ...
        if gold_bias["bias"] == "bullish" and currency_bias["bias"] == "bullish" and stock_bias["bias"] == "bearish":
            result.append(
                _signal_description(
                    "Stagflation",
                    "Ø±Ú©ÙˆØ¯ ØªÙˆØ±Ù…ÛŒ",
    ...
    def _analyze_signals_only(
            ...
            result.append(
                {
                    ...
                    "signal": "DEFENSIVE",
                    "source": "signal_count",
                    "description": "Ø·Ù„Ø§ â†‘ + Ø³Ù‡Ø§Ù… â†“ â€” Ø¨Ø§Ø²Ø§Ø± Ø±ÛŒØ³Ú©â€ŒÚ¯Ø±ÛŒØ².",
                    "strength": 0.8,
                }
            )
        elif gold_bias["bias"] == "bearish" and stock_bias["bias"] == "bullish":
            result.append(
                {
                    ...
                    "signal": "AGGRESSIVE",
                    "source": "signal_count",
                    "description": "Ø·Ù„Ø§ â†“ + Ø³Ù‡Ø§Ù… â†‘ â€” Ø¨Ø§Ø²Ø§Ø± Ø±ÛŒØ³Ú©â€ŒÙ¾Ø°ÛŒØ±.",
                    "strength": 0.8,
                }
            )
        if gold_bias["bias"] == "bullish" and crypto_bias["bias"] == "bullish":
            result.append(
                {
                    ...
                    "signal": "HEDGE",
                    "source": "signal_count",
                    "description": "Ø·Ù„Ø§ â†‘ + Ø±Ù…Ø²Ø§Ø±Ø² â†‘ â€” Ù¾ÙˆØ´Ø´ ØªÙˆØ±Ù…ÛŒ.",
                    "strength": 0.7,
                }
            )
            ...
            result.append(
                {
                    ...
                    "signal": "CAUTION",
                    "source": "signal_count",
                    "description": "Ú©Ø§Ù„Ø§ â†‘ + Ø§Ø±Ø² â†“ â€” ÙØ´Ø§Ø± ØªÙˆØ±Ù…ÛŒ ÙˆØ§Ø±Ø¯Ø§ØªÛŒ.",
                    "strength": 0.65,
                }
            )
        all_bearish = all(
        ...
        if (
            all_bearish
            and len([b for b in [gold_bias, stock_bias, crypto_bias, currency_bias, commodity_bias] if b["total"] > 0])
            >= 3
        ):
            result.append(
                {
                    ...
                    "signal": "CASH",
                    "source": "signal_count",
                    "description": "Ù‡Ù…Ù‡ Ø¨Ø§Ø²Ø§Ø±Ù‡Ø§ Ù†Ø²ÙˆÙ„ÛŒ â€” Ø±ÛŒØ³Ú© Ø³ÛŒØ³ØªÙ…ÛŒ.",
                    "strength": 0.9,
                }
            )
        ...
        result.append(
            {
                ...
                "signal": "INFO",
                "source": "signal_count",
                "description": "Ø®Ù„Ø§ØµÙ‡ ÙˆØ¶Ø¹ÛŒØª Ø¨Ø§Ø²Ø§Ø±Ù‡Ø§ Ø¨Ø± Ø§Ø³Ø§Ø³ Ø³ÛŒÚ¯Ù†Ø§Ù„â€ŒÙ‡Ø§",
                "strength": 0.0,
                "biases": {
                    "stock": stock_bias,
                    "gold": gold_bias,
...
class QuantSignalOrchestrator:
    ...
    async def generate(
        ...
        logger.info(
            "[pipeline] stage4.5 decision: %.2fs, released=%d rejected=%d",
            time.monotonic() - _t5,
            len(enriched),
            len(rejected),
        )
        ...
        avg_confidence = round(sum(s.confidence for s in enriched) / max(len(enriched), 1), 3) if enriched else 0.0
    ...
    async def _record_outcomes_and_retrain(
        self,
        accuracy_snapshot: dict[str, float],
    ...
    async def _persist_pending_signals(self, signals: list[EnrichedSignal]) -> int:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                pending_keys = {(row[0], row[1], row[2], row[3]) for row in r.fetchall()}
                ...
                if inserted < len(signals):
                    logger.info(
                        "Persisted %d new pending signals (%d already pending / hold skipped)",
                        inserted,
