...
class MultiTimeframeConfirmer:
    ...
    async def confirm_signal(
        ...
        if agreement_ratio >= strong_bar:
            ...
            if primary_direction in ("buy", "sell"):
                ...
                opposing_votes = [
                    v for v in votes if v.direction != primary_direction and v.direction != "hold" and v.strength > 0.3
                ]
    ...
    async def _get_timeframe_vote(
        ...
        """
        try:
            direction, strength = await self._trend_for_timeframe(symbol, market, timeframe)
    ...
    async def _trend_for_timeframe(self, symbol: str, market: str, timeframe: str) -> tuple[str, float]:
