...
from core.indicators import compute_atr as _core_compute_atr
from core.indicators import compute_ema as _core_compute_ema
from core.indicators import compute_rsi as _core_compute_rsi
from core.indicators import compute_sma as _core_compute_sma
...
# Canonical indicator source: core.indicators (single point of testing).
# Wrappers keep the legacy None-on-insufficient-data contract.


def _compute_rsi(closes: list[float], period: int = 14) -> float | None:
    """RSI with Wilder's smoothing (canonical core.indicators)."""
    if len(closes) < period + 1:
        return None
    return _core_compute_rsi(closes, period)


def _compute_sma(values: list[float], period: int) -> float | None:
    if len(values) < period:
        return None
    return _core_compute_sma(values, period)


def _compute_ema(values: list[float], period: int) -> float | None:
    if len(values) < period:
        return None
    return _core_compute_ema(values, period)


def _compute_atr(highs: list[float], lows: list[float], closes: list[float], period: int = 14) -> float | None:
    """ATR with Wilder's smoothing (canonical core.indicators)."""
    return _core_compute_atr(highs, lows, closes, period)
        ...
        try:
            ...
            async with _db.async_session_factory() as session:
                ...
                for g in gold24:
                    ...
                    range_txt = (
                        f" | محدوده: {_fmt_price(low_24h)} - {_fmt_price(high_24h)}" if high_24h and low_24h else ""
                    )
    ...
    async def _signals_currency(self) -> list[MarketSignal]:
        ...
        try:
            ...
            async with _db.async_session_factory() as session:
                ...
                syms_in = [c[0] for c in currencies if re.fullmatch(r"[A-Za-z0-9_]+", c[0] or "")]
            ...
            async with _db.async_session_factory() as session:
                ...
                for und, chains in by_underlying.items():
                    ...
                    if direction == "buy" and best_call:
                        entry = (
                            f"اختیار خرید {und} اعمال {_fmt_price(best_call[2])} — پریمیوم: {_fmt_price(best_call[3])}"
                        )
                        ...
                        entry = (
                            f"اختیار فروش {und} اعمال {_fmt_price(best_put[2])} — پریمیوم: {_fmt_price(best_put[3])}"
                        )
        ...
        try:
            ...
            async with _db.async_session_factory() as session:
                ...
                for com in commodities:
                    ...
                    cat_label = {"precious_metal": "فلزات گرانبها", "base_metal": "فلزات پایه", "energy": "انرژی"}.get(
                        category, category
                    )
