...
class SignalAccuracyTracker:
    ...
    async def evaluate_signal(
        ...
        if target1 is not None:
            if (
                direction == "buy"
                and high_price
                and high_price >= target1
                or direction == "sell"
                and low_price
                and low_price <= target1
            ):
                hit_t1 = True
        if target2 is not None:
            if (
                direction == "buy"
                and high_price
                and high_price >= target2
                or direction == "sell"
                and low_price
                and low_price <= target2
            ):
                hit_t2 = True
    ...
    def _count_unavailable_read(reason: str = "no_database") -> None:
        ...
        try:
            ...
            get_prometheus_exporter().inc("accuracy_tracking_read_unavailable_total", labels={"reason": reason})
