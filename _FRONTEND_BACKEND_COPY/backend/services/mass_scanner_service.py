...
class MassScannerService:
    ...
    async def scan_all(
        ...
        try:
            ...
            logger.info(
                "Built %d indicator×param×signal configs from %d indicators",
                total_configs,
                len({c[0] for c in configs}),
            )
            ...
            for batch_start in range(0, total_symbols, batch_size):
                ...
                logger.info(
                    "Progress: %.1f%% (%d/%d symbols, %d passing configs so far)",
                    progress_pct,
                    symbols_scanned,
                    total_symbols,
                    len(self._results),
            ...
            logger.info(
                "Scan complete: %d configs tested, %d passed, %d best strategies in %.1fs",
                configs_tested,
                len(self._results),
                best_count,
                elapsed,
            ...
            return Result.ok(
                {
                    "batch_id": self._batch_id,
                    **self._stats,
                    "best_per_symbol": {sym: self._result_to_dict(r) for sym, r in self._best_per_symbol.items()},
                }
            )
