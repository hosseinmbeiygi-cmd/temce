...
class AutoRetrainPipeline:
    ...
    async def check_and_retrain(
        ...
        if not should_retrain:
            if trigger == "accuracy_drop" and current_accuracy < self.ACCURACY_THRESHOLD:
                should_retrain = True
                logger.info(
                    "Accuracy %.1f%% below threshold %.1f%%. Retraining %s.",
        ...
        if not should_retrain:
            logger.info("Market %s accuracy %.1f%% above threshold. Skipping retrain.", market, current_accuracy * 100)
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                if len(all_features) < self.MIN_SAMPLES_FOR_RETRAIN:
                    ...
                    logger.warning(
                        "Not enough training samples: %d < %d", len(all_features), self.MIN_SAMPLES_FOR_RETRAIN
                    )
                    return Result.ok(report)
                ...
                if train_result.success:
                    report.models_retrained = [cfg["model_name"] for cfg in self._ml.DEFAULT_MODELS.get(market, [])]
                else:
                ...
                logger.info(
                    "Retrain complete for %s: retrained %d models in %.1fs",
                    market,
                    len(report.models_retrained),
    ...
    async def retrain_all_markets(self, force: bool = False) -> Result[list[RetrainReport]]:
