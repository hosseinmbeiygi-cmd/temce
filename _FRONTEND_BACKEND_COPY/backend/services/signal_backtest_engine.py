...
class SignalBacktestEngine:
    ...
    async def _fetch_future_prices(
        ...
        try:
            ...
            return [
                {
                    "date": safe_row_str(row, idx=0),
                    "open": row[1] or current_price,
                    "high": row[2] or current_price,
                    "low": row[3] or current_price,
    ...
    async def get_backtest_history(
        ...
        return await tracker.get_accuracy_by_market(market=market, days=days, page=page, page_size=page_size)
