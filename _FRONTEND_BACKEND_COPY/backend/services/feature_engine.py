...
def _load_macroeconomics_config() -> tuple[dict[str, float], dict[str, float]]:
    ...
    try:
        ...
        config_path = os.path.join(os.path.dirname(__file__), "..", "config", "macroeconomics.yaml")
...
class FeatureEngine:
    ...
    def _compute_block_b(self, enriched: dict[str, Any]) -> dict[str, Any]:
        ...
        f["eps_real_growth"] = (
            round((1 + f["eps_nominal_growth"]) / (1 + inflation_decimal) - 1, 4)
            if inflation_decimal > 0
            else f["eps_nominal_growth"]
        )
    ...
    def _compute_block_d(
        ...
        if float_shares_count > 0 and last_price > 0:
            f["float_turnover_pct"] = round(100 * f["trade_value"] / (float_shares_count * last_price), 2)
        else:
    ...
    def _compute_block_h(
        ...
        f["position_size"] = (
            round((0.01 * _portfolio_config["investable_capital"]) / max(risk_per_trade, 1), 0)
            if risk_per_trade > 0
            else 0
        )
    ...
    def _score_technical(
        ...
        if queue_streak >= 2:
            ...
            return round((volume_ratio * 50) + ((100 - min(distance, 100)) * 50), 1)
