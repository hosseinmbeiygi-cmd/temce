...
class ScreenerAIReportService:
    ...
    async def _load_profile(self, session: AsyncSession, symbol: str) -> dict[str, Any] | None:
        r = await session.execute(text("SELECT * FROM screener_profiles WHERE symbol = :sym"), {"sym": symbol})
        row = r.fetchone()
        return dict(row._mapping) if row else None

    async def _load_latest_signal(self, session: AsyncSession, symbol: str) -> dict[str, Any] | None:
        r = await session.execute(
            text("SELECT * FROM screener_signals WHERE symbol = :sym ORDER BY generated_at DESC LIMIT 1"),
            {"sym": symbol},
        )
        row = r.fetchone()
        return dict(row._mapping) if row else None
    ...
    async def generate_market_report(self, limit: int = 25) -> dict[str, Any]:
        ...
        async for session in self._sessions():
            ...
            r = await session.execute(text("SELECT MAX(generated_at) AS latest FROM screener_signals"))
    ...
    def _build_symbol_text(
        ...
        if model:
            ...
            lines.append(
                f"نمره نهایی: {model.get('final_score', 0):.1f} / 100 (تعدیل‌شده: {model.get('adjusted_score', 0):.1f})"
            )
            lines.append(
                f"قیمت فعلی: {_fmt(price)} ریال | "
                f"P/E: {model.get('live_pe', 0):.1f} | "
                f"ضریب P/E به صنعت: {model.get('pe_ratio', 0):.2f}"
            )
            lines.append(f"روند ۲۰ روزه: {model.get('trend_20d', '—')} | جهش حجمی: {model.get('volume_spike', 0):.2f}x")
        ...
        if legal_30d:
            ...
            lines.append(
                f"• خالص خرید حقوقی: {_fmt(net, 0)} ({'+' if net >= 0 else ''}{net / max(float(lb or ls or 1), 1) * 100:.1f}٪ از گردش)"
            )
        ...
        if daily_60:
            ...
            recent_pct = [
                float(d.get("price_last_change_pct") or 0) for d in daily_60 if d.get("price_last_change_pct")
            ]
            if recent_pct:
        ...
        if model:
            ...
            if decision == "خرید":
                lines.append(
                    "این نماد با عبور از آستانه نمره ۷۰ و بدون فیلتر ریسک منفی، کاندید خرید است. "
                    "حد ضرر پیشنهادی و سایز پوزیشن را رعایت کنید و ورود را در چند ترنچ انجام دهید."
                )
            elif decision == "رد_ریسک":
                lines.append(
                    "این نماد به دلیل فعال بودن فیلترهای ریسک منفی رد شده است — حتی با نمره بالا نباید وارد شد."
                )
