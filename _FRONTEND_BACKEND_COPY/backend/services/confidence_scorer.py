...
class ConfidenceScorer:
    ...
    async def compute_confidence(
        ...
        factors.historical_accuracy = await self._get_historical_accuracy(market=market, source=source, symbol=symbol)
        ...
        if ml_prediction and models_used:
            factors.model_agreement = self._compute_model_agreement(ml_prediction, direction, models_used)
        else:
    ...
    async def _get_historical_accuracy(self, market: str, source: str, symbol: str = "") -> float:
    ...
    async def _get_recent_performance(self, market: str, source: str, symbol: str, days: int = 30) -> float:
        ...
        try:
            ...
            async with async_session_factory() as session:
                ...
                if row and row[0] is not None:
                    real_accuracy = float(row[0])
                    total = row[1] or 0
                    blended = (bootstrap_prior * BOOTSTRAP_PRIOR_WEIGHT + real_accuracy * total) / (
                        BOOTSTRAP_PRIOR_WEIGHT + total
                    )
