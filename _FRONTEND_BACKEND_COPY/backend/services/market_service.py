...
class MarketService:
    ...
    async def _get_historical_from_quotes_table(
        ...
        try:
            ...
            quotes_result = await self.quote_repo.get_range(instrument_id, start_dt, end_dt, timeframe="1d")
    ...
    async def get_historical_quotes(
        ...
        if self._client:
            try:
                ...
                if live_data:
                    filtered = [
                        d for d in live_data if d.get("date", "") >= start_date and d.get("date", "") <= end_date
                    ]
                    if filtered:
                        return Result.ok(filtered)
            except Exception:
    ...
    async def get_candles(
        ...
        if self._brsapi:
            try:
                raw = await self._brsapi.get_candlesticks(symbol, candle_type=candle_type, limit=limit)
                if raw:
    ...
    @staticmethod
    def _macd(data: list[float], fast: int, slow: int, signal: int) -> tuple[list[float], list[float], list[float]]:
    ...
    @staticmethod
    def _bollinger(data: list[float], period: int, stddev: float) -> tuple[list[float], list[float], list[float]]:
    ...
    async def get_batch_sparklines(self, symbols: list[str], limit: int = 30) -> Result[dict[str, list[float]]]:
        """
                ...
                return Result.ok(
                    {
                        "summary": {"total_symbols": len(commodities)},
                        "sub_markets": [
                            {"name": cat["category"], "count": cat["count"], "instruments": []} for cat in categories
                        ]
                        if categories
                        else [
                            {"name": "Commodity", "count": len(commodities), "instruments": commodities[:20]},
