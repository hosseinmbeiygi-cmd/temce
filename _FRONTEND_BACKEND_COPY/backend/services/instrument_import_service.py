...
def parse_xlsx_bytes(content: bytes) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError("openpyxl is required for Excel import. Install with: pip install openpyxl") from exc
...
def row_to_instrument_kwargs(row: dict[str, Any], row_num: int) -> dict[str, Any] | str:
    ...
    for col_name, arg_name, transform, required in COLUMN_MAP:
        ...
        try:
            ...
            return f"Row {row_num} ({symbol}): failed to parse '{col_name}'='{s}': {exc}"
    ...
    async def import_from_bytes(
        ...
        *,
        ...
        if isinstance(rows[0], dict) and "symbol" not in rows[0]:
            return Result.fail(
                "File must have a 'symbol' column. Found columns: " + ", ".join(sorted(str(k) for k in rows[0]))
            )
