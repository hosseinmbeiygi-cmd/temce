...
def price_commodity_option(params: CommodityOptionParams) -> OptionPrice:
    ...
    # Guard invalid spot/strike: math.log(S / K) raises and gamma's denominator
    # (S * sigma * sqrt(T)) would divide by zero — never emit inf/nan downstream.
    # Black-76 path uses the futures price as the underlying, so guard it too.
    if S <= 0 or K <= 0 or (params.futures_price is not None and params.futures_price <= 0):
        return OptionPrice(
            model=PRICING_MODEL_BLACK_SCHOLES,
            price=0.0,
            intrinsic_value=0.0,
            parameters={"model": "commodity_bs", "error": "invalid_inputs", "S": S, "K": K},
        )
    ...
    # Degenerate-but-valid params (e.g. T~0, sigma~1e-6) can still blow greeks
    # up to inf/nan; clamp so downstream JSON serialization never breaks.
    ...
    if not math.isfinite(delta):
        delta = 0.0
    if not math.isfinite(theta_annual / 365.0):
        theta_annual = 0.0
