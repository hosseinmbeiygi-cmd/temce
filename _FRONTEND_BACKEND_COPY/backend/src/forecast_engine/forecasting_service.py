"""نسخه ادغام‌شده forecasting_service — از BrsApi_Forecasting/services/forecasting_service.py با adapter به core/cache."""
...
from src.forecast_engine.exceptions import FairValueNotApplicableError, NoDataError, OutlierDataError, StaleDataError
from src.forecast_engine.redis_adapter import get_previous_ema, get_raw_price, set_ema
from src.forecast_engine.symbol_registry import get_symbol_meta, is_fair_value_applicable
...
async def _get_fresh_reference(symbol: str) -> float:
    ...
    if age > STALE_DATA_MAX_AGE_SECONDS:
        raise StaleDataError(f"داده‌ی {symbol} با {int(age)} ثانیه تأخیر، قدیمی است.")
...
def calculate_fair_value(symbol: str, xau_price: float, usd_price: float) -> float:
    ...
    if meta.purity is None:
        raise FairValueNotApplicableError(f"عیار برای «{symbol}» ثبت نشده.")
