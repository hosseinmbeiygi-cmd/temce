      ...
      {!isLoading && !error && data && (
        <>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatCard title="TOTAL PROFILES" value={data.profiles.total_profiles.toLocaleString()} sub="from symbols table" />
              <StatCard title="WITH EPS" value={data.profiles.with_eps.toLocaleString()} sub={data.profiles.total_profiles > 0 ? `${((data.profiles.with_eps / data.profiles.total_profiles) * 100).toFixed(0)}% coverage` : undefined} />
              <StatCard title="WITH NET PROFIT" value={fmtNum(data.profiles.with_profit)} sub={`avg: ${fmtNum(data.profiles.avg_eps)}`} accent />
              <StatCard title="WITH GROSS MARGIN" value={data.profiles.with_margin.toLocaleString()} sub={data.profiles.avg_gross_margin > 0 ? `avg: ${fmtPct(data.profiles.avg_gross_margin)}` : undefined} />
            </div>
            {/* Profile fill bars */}
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <StatCard title="TOTAL SIGNALS" value={data.signals.total_signals.toLocaleString()} sub="generated by model" />
              <StatCard title="BUY SIGNALS" value={data.signals.buy_count.toLocaleString()} sub={data.signals.total_signals > 0 ? `${((data.signals.buy_count / Math.max(data.signals.total_signals, 1)) * 100).toFixed(0)}% of total` : undefined} accent />
