...
function useLiveMarketData() {
  ...
  const { prices: wsPrices, connected: wsConnected, error: wsError, latencyMs } = useMarketWebSocket(trackedSymbols);
  ...
  const bySymbol = useMemo(() => {
    ...
    if (wsConnected) {
      for (const [symbol, row] of wsPrices.entries()) {
        result.set(symbol, { ...result.get(symbol), ...row });
      }
    }
    return result;
  }, [query.data?.rows, wsConnected, wsPrices]);
  return {
    bySymbol,
...
function LiveStatusBadge({ connected, hasRestData, latencyMs }: { connected: boolean; hasRestData: boolean; latencyMs: number | null }) {
  const latencyLabel = latencyMs === null ? "— ms" : `${faNumber(latencyMs)} ms`;
  const label = connected ? `LIVE · ${latencyLabel}` : hasRestData ? "POLLING · آخرین دریافت" : "در انتظار داده";
  return (
...
function LiveMarketLayer({ live }: { live: ReturnType<typeof useLiveMarketData> }) {
  ...
  const cards = [
    { label: "سکه امامی", unit: "تومان", value: priceOf("IR_COIN_EMAMI") !== null ? (priceOf("IR_COIN_EMAMI") as number) / 10 : null, change: get("IR_COIN_EMAMI")?.change_percent ?? null, live: live.wsConnected || Boolean(get("IR_COIN_EMAMI")) },
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-line bg-card px-4 py-3 shadow-[var(--shadow-card)]">
        ...
        <LiveStatusBadge connected={live.wsConnected} hasRestData={live.bySymbol.size > 0} latencyMs={live.latencyMs} />
  ...
  return (
    <div className="rounded-2xl border border-line bg-card p-4 shadow-[var(--shadow-card)] lg:p-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        ...
        <div className="flex gap-1 rounded-xl bg-soft p-1" role="tablist" aria-label="بازهٔ نمودار">
          {(["1D", "1W"] as const).map((tf) => (
            <button key={tf} type="button" role="tab" aria-selected={timeframe === tf} data-state={timeframe === tf ? "active" : "inactive"} onClick={() => setTimeframe(tf)} className={`rounded-lg px-3 py-1.5 text-[10px] font-bold ${timeframe === tf ? "bg-card text-primary-600 shadow-sm" : "text-ink-3"}`}>
              {tf === "1D" ? "روزانه" : "هفتگی"}
            </button>
          ))}
        </div>
...
function StructuredSignalCard({ history, live }: { history: ReturnType<typeof useGoldHistory>; live: ReturnType<typeof useLiveMarketData> }) {
  ...
  const signal = useMemo(() => {
    ...
    const atrValue = atr(history.highs, history.lows, closes);
    if (!Number.isFinite(last) || atrValue === null || !Number.isFinite(atrValue)) return null;
  ...
  }, [history, xauusd.closes, live.bySymbol, sekke]);
  if (!signal) {
    return (
    ...
    <div className="rounded-2xl border border-line bg-card p-4 shadow-[var(--shadow-card)]">
      ...
      <div className="space-y-2">
        {sources.map(([key, label]) => {
          ...
          const ok = status?.status === "ok" || status?.status === "fresh";
          ...
          return (
            <div key={key} className="flex items-center justify-between rounded-xl bg-soft/70 px-3 py-2 text-xs">
              ...
              <span className={ok ? "text-up" : "text-warn"}>{ok ? "سالم" : missing ? "ناموجود" : error ? "خطا" : status ? "قدیمی" : "—"}</span>
            </div>
          );
        })}
...
function PortfolioModule({ trades, remove, live }: { trades: Trade[]; remove: (id: string) => void; live: ReturnType<typeof useLiveMarketData> }) {
  ...
  const positions = useMemo(() => {
    ...
    for (const trade of sorted) {
      ...
      if (trade.side === "SELL") {
        if (existing) {
          const sold = Math.min(trade.quantity, existing.quantity);
          existing.cost = existing.quantity > 0 ? existing.cost * (existing.quantity - sold) / existing.quantity : existing.cost;
      ...
      if (existing) {
        ...
        existing.cost = (existing.cost * existing.quantity + trade.quantity * trade.entryPrice) / totalQty;
...
export default function MultiAssetPage() {
  ...
  const portfolioValue = useMemo(() => {
    ...
    for (const trade of sorted) {
      ...
      if (existing) {
        ...
        grouped.set(trade.symbol, { quantity: trade.quantity, cost: trade.entryPrice });
      }
    }
    let total = 0;
    let known = true;
    for (const [symbol, entry] of grouped) {
      const price = priceForSymbol(symbol, live, fundCache);
  ...
  return (
    <AppLayout title="بازار طلا" subtitle="تحلیل یکپارچه طلا، سهام، کریپتو و سبد — همهٔ داده‌ها از BrsApi.ir" sidebar={false}>
      <div className="space-y-5">
        <section className="rounded-3xl border border-primary-500/20 bg-gradient-to-br from-primary-950 via-card to-card p-5 text-white shadow-[var(--shadow-card-hover)] lg:p-6">
          <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
            <div>
              ...
              <p className="mt-1 text-xs text-brand-200/80">آخرین همگام‌سازی طلا: {formatDateTime(goldSync?.last_fetched)} — وضعیت: {goldSync?.status === "ok" || goldSync?.status === "fresh" ? "تازه" : goldSync?.status === "missing" || goldSync?.status === "unknown" ? "ناموجود" : goldSync?.status === "error" ? "خطا" : goldSync ? `${faNumber((goldSync.age_minutes ?? 0) / 60, 1)} ساعت پیش` : "—"}</p>
            </div>
        ...
        <nav className="flex gap-2 overflow-x-auto rounded-2xl border border-line bg-card p-2" role="tablist" aria-label="بخش‌های بازار طلا">
          {([
            ...
            <button key={key} type="button" role="tab" aria-selected={tab === key} data-state={tab === key ? "active" : "inactive"} onClick={() => setTab(key)} className={`flex min-w-[110px] flex-1 items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold transition ${tab === key ? "bg-primary-600 text-white shadow" : "text-ink-3 hover:bg-soft hover:text-ink"}`}>
