...
import { fmt, fmtPct, riskColor, riskBg, riskLabel, marketLabel, PayoffDiagram } from "./helpers";
...
export default function StrategyAnalyzer() {
  ...
  const { data: strategiesResp, isLoading } = useQuery({
    ...
    queryFn: () => apiGet<{ success: boolean; data: StrategyInfo[] }>("/api/v1/options/strategies"),
    staleTime: 300_000,
  });

  const analyzeMutation = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      apiPost<{ success: boolean; data: StrategyAnalysis }>("/api/v1/options/analyze", body),
          ...
          <div className="space-y-1">
            {filtered.map((s) => (
              <button
                ...
                className="w-full text-right p-2.5 rounded-xl transition-all text-xs "
              >
