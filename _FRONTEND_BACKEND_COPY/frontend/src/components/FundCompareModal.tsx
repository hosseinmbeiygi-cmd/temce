...
export default function FundCompareModal({ funds, onClose, onRemove }: FundCompareModalProps) {
  ...
  const entries: { label: string; num?: (a: FundAnalysis, f: Fund) => number | null; render: (a: FundAnalysis, f: Fund) => React.ReactNode }[] = [
    {
      label: "امتیاز کل",
      num: (a) => a.scores.total,
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl border border-surface-700/50 bg-surface-900 shadow-2xl">
        ...
        <div className="p-4 overflow-x-auto">
          <table className="w-full text-right" dir="rtl">
            ...
            <tbody>
              {entries.map((entry) => {
                const nums = analyses.map((a, i) => (entry.num ? entry.num(a, funds[i]) : null));
                ...
                return (
                  <tr key={entry.label} className="border-b border-surface-800/30">
                    ...
                    {analyses.map((a, i) => {
                      const v = entry.num ? entry.num(a, funds[i]) : null;
                      const isBest = !tie && v !== null && best !== null && v === best;
                      const isWorst = !tie && v !== null && worst !== null && v === worst;
                      return (
                        <td
                          key={funds[i]?.symbol ?? i}
                          className={`px-3 py-2.5 text-center rounded ${isBest ? "bg-accent-emerald/10 ring-1 ring-inset ring-accent-emerald/30" : isWorst ? "bg-accent-rose/10 ring-1 ring-inset ring-accent-rose/30" : ""}`}
                        >
