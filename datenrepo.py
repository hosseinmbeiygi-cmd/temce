    ...
    def handle_scraping_error(self, error: Exception, page_url: str, html_snapshot: str = None) -> int:
        ...
        if severity_check := self._check_error_severity(error):
            alert_data = {
                ...
                'message': f"Scrapping failed for {page_url}: {str(error)}",
...
if __name__ == "__main__":
    ...
    audit_data = {
        ...
        'resource_id': 'tsetmc_mkt_20250101_20250110',
        'details': 'Successfully collected market data',
        'ip_address': '192.168.1.100',
        'logged_at': datetime.now().isoformat()
    }
    try:
        audit_id = repo.log_audit(audit_data)
        print(f"Audit log saved with ID: {audit_id}")
    except DatabaseError as e:
