...
class PredictionRepository:
    ...
    async def list_by_batch(self, batch_id: str) -> Result[list[dict[str, Any]]]:
        ...
        stmt = (
            select(MlPredictionModel)
            .where(MlPredictionModel.batch_id == batch_id)
            .order_by(MlPredictionModel.symbol)
        )
    ...
    async def list_recent(
        self, limit: int = 500, symbol: str | None = None
    ...
    async def list_batches(
        ...
        count_stmt = select(sa_func.count()).select_from(
            select(MlPredictionModel.batch_id).distinct().subquery()
        )
