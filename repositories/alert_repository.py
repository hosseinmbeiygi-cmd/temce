...
class _AlertDbRepo(DbRepository[Alert, AlertModel]):
    ...
    async def get_history(self, alert_id: str) -> Result[list[AlertHistory]]:
        stmt = select(AlertHistoryModel).where(AlertHistoryModel.alert_id == alert_id).order_by(desc(AlertHistoryModel.triggered_at))
        result = await self.session.execute(stmt)
        rows = result.scalars().all()
        items = [
            AlertHistory(
    ...
    async def list_history(self, page: int = 1, page_size: int = 100) -> Result[PaginatedResult[AlertHistory]]:
        ...
        stmt = select(AlertHistoryModel).order_by(desc(AlertHistoryModel.triggered_at)).offset((page - 1) * page_size).limit(page_size)
        result = await self.session.execute(stmt)
        rows = result.scalars().all()
        items = [
            AlertHistory(
