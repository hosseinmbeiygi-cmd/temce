...
class GeneratedStrategyRepository:
    ...
    async def save(self, data: dict[str, Any]) -> Result[str]:
        ...
        orm = GeneratedStrategyModel(
            ...
            entry_params=json.dumps(data.get("entry_params", {}), ensure_ascii=False) if data.get("entry_params") else None,
            entry_condition=data.get("entry_condition"),
            exit_indicator=data.get("exit_indicator"),
            exit_params=json.dumps(data.get("exit_params", {}), ensure_ascii=False) if data.get("exit_params") else None,
            exit_condition=data.get("exit_condition"),
            filter1_indicator=data.get("filter1_indicator"),
            filter1_params=json.dumps(data.get("filter1_params", {}), ensure_ascii=False) if data.get("filter1_params") else None,
            filter1_condition=data.get("filter1_condition"),
            filter2_indicator=data.get("filter2_indicator"),
            filter2_params=json.dumps(data.get("filter2_params", {}), ensure_ascii=False) if data.get("filter2_params") else None,
    ...
    async def list_top(
        ...
        return Result.ok(PaginatedResult(
            items=items, total=total, page=offset // limit + 1,
    ...
    async def get_indicator_stats(self) -> list[dict[str, Any]]:
        ...
        ).where(GeneratedStrategyModel.status == "active").group_by(GeneratedStrategyModel.entry_indicator)

        rows = (await self._session.execute(stmt)).all()
        return [
            {"indicator": r[0], "count": r[1], "avg_score": round(r[2] or 0, 2),
             "avg_return": round(r[3] or 0, 2), "avg_sharpe": round(r[4] or 0, 2)}
            for r in rows
        ]
