        ...
        async for session in get_session():
            ...
            if existing is not None:
                ...
                logger.info(
                    ...
                    existing.version, existing.id, feat_count, svc_count,
                )
            else:
                # Perform the seed
                ...
                if seeded is not None:
                    ...
                    logger.info(
                        ...
                        seeded.version, seeded.id, feat_count, svc_count,
                    )
            ...
            if result["decision_count"] > 0:
                ...
                run_result = await session.execute(
                    select(func.count(func.distinct(DecisionResult.run_id)))
                )
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if seed_info.get("arch_version"):
        logger.info("Decision Engine — architecture v%s | decisions: %d | runs: %d",
                     seed_info["arch_version"], seed_info["decision_count"], seed_info["run_count"])
