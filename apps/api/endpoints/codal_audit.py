...
_ALLOWED_SORT = {
    "symbol", "health_score", "roe", "roa", "net_margin", "gross_margin",
    "current_ratio", "debt_to_equity", "revenue", "net_profit", "total_assets",
    "revenue_growth",
}
...
async def get_audit_summary(
    ...
    count_result = await session.execute(
        text(f"SELECT COUNT(*) as cnt FROM {_TABLE}{wsql}"), params
    )
...
async def get_audit_stats(
    ...
    hdist_result = await session.execute(
        text("SELECT health_classification, COUNT(*) as cnt, AVG(health_score) as avg_score "
    ...
    top_roe_result = await session.execute(
        text("SELECT symbol, health_score, health_classification, roe, roa, net_margin, "
    ...
    fd_result = await session.execute(
        text("SELECT forensic_risk, COUNT(*) as cnt FROM codal_audit_summary "
