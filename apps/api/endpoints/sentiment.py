...
@router.post("/train", summary="Train sentiment model", description="Train the Persian sentiment ML model with labelled data")
async def train_sentiment_model(
    body: dict[str, Any] = Body(..., examples=[{"texts": ["بازار خوب", "سقوط شدید"], "labels": ["positive", "negative"]}]),
) -> ApiResponse[dict[str, Any]]:
    try:
        texts = body.get("texts", [])
        labels = body.get("labels", [])
        if not texts or not labels or len(texts) != len(labels):
            return ApiResponse[dict[str, Any]](success=False, data=None, error={"message": "texts and labels arrays must be non-empty and same length"})
        service = _get_service()
        result = service.train(texts, labels)
        return ApiResponse[dict[str, Any]](success=result.get("success", False), data=result, error={"message": result.get("error")} if not result.get("success") else None)
    except Exception as exc:
        logger.exception("Sentiment model training failed")
        return ApiResponse[dict[str, Any]](success=False, data=None, error={"message": str(exc)})


@router.get("/stats", summary="Sentiment service stats", description="Get statistics about the sentiment analysis service")
