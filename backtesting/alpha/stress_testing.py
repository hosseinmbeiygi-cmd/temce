...
class AlphaStressTest:
    ...
    def __init__(self) -> None:
        self.scenarios: list[StressScenario] = [
            ...
            StressScenario("high_volatility", "2x volatility spike", volatility_multiplier=2.0, spread_multiplier=2.0, impact_multiplier=2.0),
            StressScenario("queue_lock", "Bid/ask limit lock", spread_multiplier=5.0, depth_reduction_pct=0.5),
            StressScenario("flash_crash", "5% flash crash", price_shock_pct=-0.05, volatility_multiplier=3.0, impact_multiplier=3.0),
            StressScenario("combined_shock", "Liquidity + volatility + crash", depth_reduction_pct=0.5, volatility_multiplier=2.0, price_shock_pct=-0.03, spread_multiplier=2.0),
        ]
    ...
    def select_robust_alphas(self, stress_results: dict[str, dict[str, dict[str, float]]], min_sharpe_under_stress: float = 0.5) -> list[str]:
