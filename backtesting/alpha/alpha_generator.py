...
class AlphaGenerator:
    ...
    def generate_all_templates(self, features: dict[str, float]) -> dict[str, float]:
        ...
        if "queue_imbalance_mean" in features:
            alphas["alpha_z_imb"] = self.generate_combination_alpha({**features, "_feature_name": "queue_imbalance"}, "zscore")
        if "spread_mean" in features:
            alphas["alpha_z_spread"] = self.generate_combination_alpha({**features, "_feature_name": "spread"}, "zscore")
        return alphas
