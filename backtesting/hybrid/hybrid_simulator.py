...
class HybridMarketSimulator:
    ...
    async def run(
        ...
        try:
            ...
            logger.info("Hybrid simulation starting: %s events, agents=%s", len(timeline), len(self.agent_engine.agents))
            ...
            logger.info(
                ...
                self._event_count, self._agent_order_count, len(result.fills), result.final_price,
            )
            return Result.ok(result)
    ...
    def _build_market_state(self, event: MarketEvent) -> dict[str, Any]:
        ...
        if state:
            inst_state.update({
                ...
                "volume": state.volume,
                "midpoint": (state.best_bid + state.best_ask) / 2 if state.best_bid > 0 and state.best_ask > 0 else 0,
