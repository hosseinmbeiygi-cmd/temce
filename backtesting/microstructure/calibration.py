...
class MicrostructureCalibrator:
    def calibrate_from_events(
        ...
        if len(trade_timestamps) > 1:
            ...
            if isinstance(span, (int, float)):
                total_seconds = max(span, 1)
            elif hasattr(span, "total_seconds"):
                total_seconds = max(span.total_seconds(), 1)
            else:
                total_seconds = 1.0
        ...
        if len(trade_volumes) > 1:
            log_volumes = np.log(trade_volumes)
            params.trade_volume_std = float(np.std(log_volumes))
        ...
        if quotes:
            ...
            for i in range(1, min(len(quotes), 10000)):
                q_prev = quotes[i - 1]
                q_curr = quotes[i]
                prev_bid = q_prev.get("bid_volume", 0) or 0
                curr_bid = q_curr.get("bid_volume", 0) or 0
                prev_ask = q_prev.get("ask_volume", 0) or 0
                curr_ask = q_curr.get("ask_volume", 0) or 0
    ...
    def calibrate_batch(
        ...
        results: list[SymbolMicrostructureParams] = []
        for symbol, (quotes, trades) in data.items():
