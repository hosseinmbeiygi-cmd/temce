...
class RegimeAwareNoiseTrader(HybridNoiseTrader):
    ...
    def __init__(self, agent_id: str, weight: float = 1.0, min_size: int = 100, max_size: int = 2000, market_order_prob: float = 0.3, intensity: float = 0.5) -> None:
    ...
    def decide(self, market_state: dict) -> list | None:
        intensity_mult = {"panic": 2.0, "low_liquidity": 1.5, "queue_lock": 0.5, "trend": 1.3, "normal": 1.0, "mean_reverting": 0.8}
...
class RegimeAwareTrendFollower(HybridTrendFollower):
    ...
    def __init__(self, agent_id: str, weight: float = 1.0, lookback: int = 20, order_size: int = 1000, threshold_pct: float = 0.005) -> None:
