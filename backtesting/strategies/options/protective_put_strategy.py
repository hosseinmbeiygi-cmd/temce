...
class ProtectivePutStrategy(BaseStrategy):
    def __init__(self, put_strike: float = 0.0, put_premium: float = 0.0, instrument_id: str = "") -> None:
        super().__init__(name="ProtectivePut")
    ...
    def on_bar(self, bar: dict[str, Any]) -> list[OrderEvent]:
        ...
        elif self.put_strike > 0 and price < self.put_strike:
            orders.append(
                OrderEvent(
                    instrument_id=self.instrument_id,
                    side=OrderSide.SELL,
