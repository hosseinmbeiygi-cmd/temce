...
class StrategyBlueprint:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "entry": {"indicator": self.entry_indicator, "params": self.entry_params, "condition": self.entry_condition},
            "exit": {"indicator": self.exit_indicator, "params": self.exit_params, "condition": self.exit_condition},
            "filter1": {"indicator": self.filter1_indicator, "params": self.filter1_params, "condition": self.filter1_condition} if self.filter1_indicator else None,
            "filter2": {"indicator": self.filter2_indicator, "params": self.filter2_params, "condition": self.filter2_condition} if self.filter2_indicator else None,
                        ...
                        for entry_params in self._param_combos(entry_spec):
                            for exit_params in self._param_combos(exit_spec):
                                if count >= offset and count < offset + batch_size:
                                    batch.append(StrategyBlueprint(
                                        entry_indicator=entry_spec.id,
                                        entry_params=entry_params,
                                        entry_condition=entry_cond,
                                        exit_indicator=exit_spec.id,
