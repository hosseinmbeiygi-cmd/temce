...
class CorporateActionsEngine:
    ...
    def register_action(self, action: CorporateAction) -> None:
        ...
        logger.debug("Registered corporate action: %s %s on %s", action.action_type, action.symbol, action.effective_date)
    ...
    def register_lifecycle(self, symbol: str, first_seen: date, last_seen: date, delisted: bool = False) -> None:
        ...
        if symbol in self._survivorship:
            ...
            self._survivorship[symbol] = SurvivorshipEntry(
                symbol=symbol, first_seen=first_seen, last_seen=last_seen, delisted=delisted,
            )
    ...
    def build_survivorship_free_universe(
        self, all_symbols: list[str], on_date: date
    ) -> list[str]:
