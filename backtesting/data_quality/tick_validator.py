...
class TickValidator:
            ...
            if timestamp and self._last_timestamp:
                if hasattr(timestamp, "timestamp") and hasattr(self._last_timestamp, "timestamp"):
                    ...
                    if diff < 0:
                        ...
                        report.drift_events.append({"index": i, "type": "gap", "diff_seconds": diff, "expected_missing": expected - 1})

            self._price_history.append(price)
            self._last_timestamp = timestamp

        # Quality score
        total_issues = report.n_bad_ticks + report.n_timestamp_drifts + report.n_missing_events + report.n_price_anomalies
    ...
    def repair_tick(self, tick: dict[str, Any], prev_tick: dict[str, Any] | None, next_tick: dict[str, Any] | None) -> dict[str, Any]:
    ...
    def estimate_missing_events(self, timestamps: list[datetime], expected_interval_seconds: float = 1.0) -> list[dict[str, Any]]:
