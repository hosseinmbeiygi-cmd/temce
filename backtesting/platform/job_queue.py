        ...
        while self._running:
            ...
            try:
                ...
                if job.retry_count < job.max_retries:
                    ...
                    job.started_at = None
                    logger.warning("[%s] Job %s failed (retry %d/%d): %s",
                                   name, job_id, job.retry_count, job.max_retries, exc)
                    await self._queue.put(job_id)
                else:
                    job.status = JobStatus.FAILED
                    job.error = str(exc)
                ...
                logger.info("[%s] Job %s finished: %s (%.1fs)",
                            name, job_id, job.status.value, elapsed)
