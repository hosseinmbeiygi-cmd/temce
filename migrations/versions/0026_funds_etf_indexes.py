...
def upgrade() -> None:
    ...
    created = {r[0] for r in conn.execute(
        text("SELECT indexname FROM pg_indexes WHERE tablename IN "
             "('brsapi_symbol_snapshots', 'brsapi_ime_funds')")
    ).fetchall()}

    if "idx_snap_sector" not in created:
        conn.execute(text(
            "CREATE INDEX idx_snap_sector ON brsapi_symbol_snapshots (sector)"
        ))
    ...
    if "idx_ime_fund_symbol" not in created:
        conn.execute(text(
            "CREATE INDEX idx_ime_fund_symbol ON brsapi_ime_funds (symbol)"
        ))
