...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("market_type", sa.String(20), nullable=False, server_default="bours",
                  comment="نوع بازار: bours / farabours / base_market"),

        # ── 5 Queue Features ──────────────────────────────────
        sa.Column("queue_status", sa.String(20), nullable=False, index=True,
                  comment="وضعیت صف: BUY_QUEUE / SELL_QUEUE / NONE"),
        sa.Column("queue_volume_ratio", sa.Float(), nullable=False, server_default="0",
                  comment="نسبت حجم صف به کل سفارشات (0-1)"),
        sa.Column("queue_days_streak", sa.Integer(), nullable=False, server_default="0",
                  comment="تعداد روزهای متوالی در صف (امروز + تاریخچه)"),
        sa.Column("queue_type_change", sa.String(20), nullable=False, server_default="NO_CHANGE",
                  comment="نوع تغییر صف: NEW_BUY_QUEUE / NEW_SELL_QUEUE / QUEUE_BROKEN / NO_CHANGE"),
        sa.Column("distance_to_limit", sa.Float(), nullable=False, server_default="0",
                  comment="فاصله تا سقف/کف دامنه (درصد)"),

        # ── Metadata ───────────────────────────────────────────
        sa.Column("last_price", sa.Float(), nullable=True,
                  comment="آخرین قیمت معامله‌شده"),
        sa.Column("limit_up", sa.Float(), nullable=True,
                  comment="سقف مجاز روزانه (tmax)"),
        sa.Column("limit_down", sa.Float(), nullable=True,
                  comment="کف مجاز روزانه (tmin)"),
        sa.Column("queue_buy_volume", sa.BigInteger(), nullable=True,
                  comment="حجم سفارشات خرید باقی‌مانده در صف"),
        sa.Column("queue_sell_volume", sa.BigInteger(), nullable=True,
                  comment="حجم سفارشات فروش باقی‌مانده در صف"),

        # ── Adjusted Scores ─────────────────────────────────────
        sa.Column("adjusted_liquidity", sa.Float(), nullable=True,
                  comment="S_L تعدیل‌شده (نقدشوندگی)"),
        sa.Column("adjusted_technical", sa.Float(), nullable=True,
                  comment="S_T تعدیل‌شده (تکنیکال)"),
        sa.Column("adjusted_orderflow", sa.Float(), nullable=True,
                  comment="S_O تعدیل‌شده (جریان پول)"),
        sa.Column("adjusted_penalty", sa.Float(), nullable=True,
                  comment="Penalty تعدیل‌شده (جریمه)"),
        sa.Column("liquidity_delta", sa.Float(), nullable=True,
                  comment="تغییر S_L نسبت به پایه"),
        sa.Column("technical_delta", sa.Float(), nullable=True,
                  comment="تغییر S_T نسبت به پایه"),
        sa.Column("orderflow_delta", sa.Float(), nullable=True,
                  comment="تغییر S_O نسبت به پایه"),
        sa.Column("penalty_delta", sa.Float(), nullable=True,
        ...
        # ── Interpretation ─────────────────────────────────────
        sa.Column("interpretation", sa.dialects.postgresql.JSONB(), nullable=True,
