...
async def main():
    ...
    async with engine.connect() as conn:
        ...
        print("-" * 70)
        for table, _join_col, _has_ins_id, _desc in TABLES:
            # Check table exists
            r = await conn.execute(
        ...
        for table, _join_col, _has_ins_id, _desc in TABLES:
            ...
            if has_symbol:
                ...
                _, linked_sym, orphaned_sym = total, 0, total
            ...
            print(
                f"  {icon} {table:35s} {total:>6} rows | "
                f"linked: {total_linked:>6} ({pct:5.1f}%) | "
                f"orphaned: {total_orphaned:>4}"
            )

            if orphans and total_orphaned > 0:
                print(f"       orphaned symbols: {', '.join(orphans[:5])}" f"{'...' if len(orphans) > 5 else ''}")
