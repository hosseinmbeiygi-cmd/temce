...
export default function AnalyticsPanel() {
  ...
  const greeksMutation = useMutation({
    mutationFn: (p: typeof greeksParams) => {
      const qs = new URLSearchParams({ S: String(p.stock_price), K: String(p.strike), T: String(p.time_to_expiry), sigma: String(p.volatility), option_type: p.option_type, r: "0.15" });
      return apiGet<{ success: boolean; data: GreeksResult }>(`/options/greeks?${qs.toString()}`);
    },
    onError: (err: Error) => toast.error(err.message),
  });

  const arbitrageMutation = useMutation({
    mutationFn: (p: typeof arbParams) => {
      const qs = new URLSearchParams({ call_price: String(p.call_price), put_price: String(p.put_price), stock_price: String(p.stock_price), strike: String(p.strike), time_to_expiry: String(p.time_to_expiry) });
      return apiGet<{ success: boolean; data: ArbitrageParity }>(`/options/analytics/arbitrage/parity?${qs.toString()}`);
    },
    onError: (err: Error) => toast.error(err.message),
  });

  const volatilityMutation = useMutation({
    mutationFn: (p: typeof volParams) => apiPost<{ success: boolean; data: VolatilityAnalysis }>("/options/analytics/volatility", { options_chain: [{ strike: p.strike, price: p.option_price, type: p.option_type }], stock_price: p.stock_price, historical_vol: 0.35 }),
    onError: (err: Error) => toast.error(err.message),
  });

  const portfolioMutation = useMutation({
    mutationFn: (p: typeof portfolioParams) => apiPost<{ success: boolean; data: PortfolioAnalysis }>("/options/analytics/portfolio", { positions: JSON.parse(p.positions), stock_price: p.stock_price }),
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 bg-surface-800/50 rounded-xl p-1 border border-surface-700/50">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${tab === t.id ? "bg-primary-600/30 text-primary-300" : "text-surface-400 hover:text-surface-200"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === "greeks" && (
        <Card title="\u0645\u062d\u0627\u0633\u0628\u0647 \u06cc\u0648\u0646\u0627\u0646\u200c\u0647\u0627">
          ...
          <div className="flex gap-2 mb-4">
            <button onClick={() => setGreeksParams(p => ({ ...p, option_type: "call" }))} className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${greeksParams.option_type === "call" ? "bg-primary-600 text-white border-primary-500" : "bg-surface-800 text-surface-400 border-surface-700/30"}`}>Call</button>
            <button onClick={() => setGreeksParams(p => ({ ...p, option_type: "put" }))} className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${greeksParams.option_type === "put" ? "bg-primary-600 text-white border-primary-500" : "bg-surface-800 text-surface-400 border-surface-700/30"}`}>Put</button>
          </div>
          <button onClick={() => greeksMutation.mutate(greeksParams)} disabled={greeksMutation.isPending} className="w-full bg-primary-600 hover:bg-primary-500 text-white font-bold py-2.5 rounded-xl text-xs transition-all disabled:opacity-50">{greeksMutation.isPending ? "در حال محاسبه..." : "محاسبه یونان‌ها"}</button>
