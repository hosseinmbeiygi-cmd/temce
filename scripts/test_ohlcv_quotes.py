...
async def main() -> None:
    ...
    from core.database import get_session, init_database, close_database
    ...
    async for session in get_session():
        ...
        ohlcv_result = await svc.get_ohlcv(TEST_SYMBOL, "2025-01-01", "2025-12-31")
        if ohlcv_result.success and ohlcv_result.value:
            bars = ohlcv_result.value
            log(f"OK: MarketService returned {len(bars)} OHLCV bars")
            for bar in bars[:3]:
                log(f"  date={bar.get('date')} o={bar.get('open')} h={bar.get('high')} l={bar.get('low')} c={bar.get('close')} v={bar.get('volume')}")
        else:
        ...
        if hist_result.success and hist_result.value:
            ...
            for rec in recs[:2]:
                log(f"  date={rec.get('date')} last={rec.get('price_last')} close={rec.get('price_close')} vol={rec.get('trade_volume')}")
        else:
